...
$estudianteinscripcion = $objetobase->recuperar_datos_tabla("estudianteinscripciontemporal", "idestudianteinscripciontemporal", $_GET['id']);
$horaactual = mktime();
$horaregistro = $_GET['ta'];
if (($horaactual - $horaregistro) > TIEMPOACTIVACION) {
    alerta_javascript("Su tiempo para realizar la activacion de su cuenta expiro,\\n puede registrarse de nuevo para continuar");
} else {
    $tablaes = "estudianteinscripciontemporal";
    $condicion = " and tipodocumento='" . $estudianteinscripcion['tipodocumento'] . "'";
    if ($datosestudiantedocumento = $objetobase->recuperar_datos_tabla("estudiantedocumento", "numerodocumento", $estudianteinscripcion['documento' . $tablaes], $condicion, "", 0)) {
        $datosestudiantegeneral = $objetobase->recuperar_datos_tabla("estudiantegeneral", "idestudiantegeneral", $datosestudiantedocumento["idestudiantegeneral"], "");
        $mensaje = "Usted ya habia estado registrado en un programa" .
                "de La Universidad El Bosque " .
                "\\n su correo inscrito actual es " . $datosestudiantegeneral['emailestudiantegeneral'] .
                " con este puede recuperar su clave ingresandolo a continuacion";
        ...
        $fila['nombrecortoestudiantegeneral'] = $estudianteinscripcion["numerodocumento" . $tablaes];
