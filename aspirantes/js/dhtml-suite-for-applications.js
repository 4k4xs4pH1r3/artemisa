...
let standardObjectsCreated = false // The classes below will check this variable, if it is false, default help objects will be created
...
DHTMLSuite.config = function () {
  let imagePath // Path to images used by the classes.
  let cssPath // Path to CSS files used by the DHTML suite.
...
DHTMLSuite.globalVariableStorage = function () {
  let menuBar_highlightedItems // Array of highlighted menu bar items
  this.menuBar_highlightedItems = new Array()

  let arrayDSObjects // Array of objects of class menuItem.
...
DHTMLSuite.common = function () {
  let loadedCSSFiles // Array of loaded CSS files. Prevent same CSS file from being loaded twice.
  let cssCacheStatus // Css cache status
  let eventEls
  let isOkToSelect // Boolean variable indicating if it's ok to make text selections
  ...
  loadCSS: function (cssFile, prefixConfigPath) {
    ...
    if (!this.loadedCSSFiles[cssFile]) {
      ...
      if (!this.cssCacheStatus) {
        if (cssFile.indexOf('?') >= 0) cssFile = cssFile + '&'
        else cssFile = cssFile + '?'
...
DHTMLSuite.clientInfo = function () {
  let browser // Complete user agent information

  let isOpera // Is the browser "Opera"
  let isMSIE // Is the browser "Internet Explorer"
  var isOldMSIE // Is this browser and older version of Internet Explorer ( by older, we refer to version 6.0 or lower)
  let isFirefox // Is the browser "Firefox"
  let navigatorVersion // Browser version
  var isOldMSIE
}
...
DHTMLSuite.tableWidgetPageHandler = function () {
  let tableRef // Reference to object of class DHTMLSuite.tableWidget
  let targetRef // Where to insert the pagination.

  let txtPrevious // Label - "Previous"
  let txtNext // Label - "Next"
  let txtFirst // Label - "First"
  let txtLast // Label - "last"

  let txtResultPrefix // Prefix : result - default = "Result: "
  let txtResultTo // Text label Result: 1 "to" 10 of 51 - default value = "to"
  let txtResultOf // Text label Result: 1 to 10 "of" 51 - default value = "of"

  let totalNumberOfRows // Total number of rows in dataset
  let rowsPerPage // Number of rows per page.

  let layoutCSS // Name of CSS file for the table widget.
  let activePageNumber // Active page number
  let mainDivEl // Reference to main div for the page handler
  let resultDivElement // Reference to div element which is parent for the result
  let pageListDivEl // Reference to div element which is parent to pages [1],[2],[3]...[Next]

  let objectIndex // Index of this widget in the arrayDSObjects array

  let linkPagePrefix // Text in front of each page link
  let linkPageSuffix // Text behind each page link

  let maximumNumberOfPageLinks // Maximum number of page links.
  let callbackOnAfterNavigate // Callback function - executed when someone navigates to a different page
...
DHTMLSuite.tableWidgetPageHandler.prototype = {
  ...
  __createPageLinks: function () {
    ...
    this.pageListDivEl.innerHTML = '' // Clearing the div element if it allready got content.
    ...
    if (
      ...
      const fl = document.createElement('A') // "first" link
    ...
    const previousLink = document.createElement('A') // "Previous" link
    ...
    const nextLink = document.createElement('A') // "Next" link
    ...
    if (
      ...
      const ll = document.createElement('A') // "Last" link
...
DHTMLSuite.tableWidget = function () {
  let tableWidget_okToSort // Variable indicating if it's ok to sort. This variable is "false" when sorting is in progress
  let activeColumn // Reference to active column, i.e. column currently beeing sorted
  let idOfTable // Id of table, i.e. the <table> tag
  let tableObj // Reference to <table> tag.
  let widthOfTable // Width of table	(Used in the CSS)
  let heightOfTable // Height of table	(Used in the CSS)
  let columnSortArray // Array of how table columns should be sorted
  let layoutCSS // Name of CSS file for the table widget.
  let noCssLayout // true or false, indicating if the table should have layout or not, if not, it would be a plain sortable table.
  let serversideSort // true or false, true if the widget is sorted on the server.
  let serversideSortAscending
  let tableCurrentlySortedBy
  let serversideSortFileName // Name of file on server to send request to when table data should be sorted
  let serversideSortNumberOfRows // Number of rows to receive from the server
  let serversideSortCurrentStartIndex // Index of first row in the dataset, i.e. if you move to next page, this value will be incremented
  let serversideSortExtraSearchCriterias // Extra param to send to the server, example: &firstname=Alf&lastname=Kalleland
  let pageHandler // Object of class DHTMLSuite.tableWidgetPageHandler
  let rowClickCallBackFunction // Row click call back function.
  let objectIndex // Index of this object in the DHTMLSuite.variableStorage.arrayDSObjects array
...
DHTMLSuite.tableWidget.prototype = {
  ...
  addNewColumn: function (columnContent, headerText, sortMethod) {
    ...
    const tableObj = document.getElementById(this.idOfTable) // Reference to the <table>
    const tbody = tableObj.getElementsByTagName('TBODY')[0] // Reference to the <tbody>
    const thead = tableObj.getElementsByTagName('THEAD')[0] // Reference to the <tbody>

    const bodyRows = tbody.rows // Reference to all the <tr> inside the <tbody> tag
    const headerRows = thead.rows // Reference to all <tr> inside <thead>
  ...
  __parseHeaderCell: function (cellRef) {
    if (!this.noCssLayout) {
      ...
      cellRef.style.cursor = 'pointer' // No CSS layout -> just set cursor to pointer/hand.
    }

    const refToThis = this // It doesn't work with "this" on the line below, so we create a variable refering to "this".
  ...
  __parseDataContentFromServer: function (ajaxIndex) {
    ...
    const rows = content.split('|||') // Create an array of each row
    for (let no = 0; no < rows.length; no++) {
  ...
  __sortTable: function (obj, howToSort) {
    ...
    if (obj.getAttribute('direction') || obj.direction) {
      ...
      let curDir = 'ascending' // How to sort the table
    ...
    const sortMethod = this.columnSortArray[indexThis] // N = numeric, S = String
...
DHTMLSuite.dragDropSimple = function (propertyArray) {
  let divElement
  let dragTimer // -1 no drag, 0-4 = initializing , 5 = drag in process
  let cloneNode
  this.cloneNode = true
  ...
  let mouse_y
  let positionSet
  let dragHandle // If a specific element is specified to be a drag handle.
...
DHTMLSuite.dragDrop = function () {
  let mouse_x // mouse x position when drag is started
  let mouse_y // mouse y position when drag is started.

  let el_x // x position of dragable element
  let el_y // y position of dragable element

  let dragDropTimer // Timer - short delay from mouse down to drag init.
  let numericIdToBeDragged // numeric reference to element currently being dragged.
  let dragObjCloneArray // Array of cloned dragable elements. every
  let dragDropSourcesArray // Array of source elements, i.e. dragable elements.
  let dragDropTargetArray // Array of target elements, i.e. elements where items could be dropped.
  let currentZIndex // Current z index. incremented on each drag so that currently dragged element is always on top.
  let okToStartDrag // Variable which is true or false. It would be false for 1/100 seconds after a drag has been started.
  ...
  let moveBackBySliding // Variable indicating if objects should slide into place moved back to their location without any slide animation.
  let dragX_allowed // Possible to drag this element along the x-axis?
  let dragY_allowed // Possible to drag this element along the y-axis?
  ...
  let drag_minY
  let drag_maxY
  let dragInProgress // Variable which is true when drag is in progress, false otherwise
...
DHTMLSuite.dragDrop.prototype = {
  ...
  __processSlideByPixels: function (
    ...
    if (currentX != targetX || currentY != targetY) {
      ...
      // Slide completed. Make absolute positioned element invisible and original element visible
      this.dragObjCloneArray[numId].style.display = 'none'
...
const refToTabViewObjects = new Array() // Reference to objects of this class.
...
DHTMLSuite.tabView = function () {
  let textPadding // Tab spacing
  let strictDocType // Using a strict document type, i.e. <!DOCTYPE>

  let DHTMLSuite_tabObj // Reference to div surrounding the tab set
  let activeTabIndex // Currently displayed tab(index - 0 = first tab)
  let initActiveTabIndex // Initially displayed tab(index - 0 = first tab)
  let ajaxObjects // Reference to ajax objects
  let tabView_countTabs
  let tabViewHeight
  let tabSetParentId // Id of div surrounding the tab set.
  let tabTitles // Tab titles
  let width // width of tab view
  let height // height of tab view
  let layoutCSS
  let outsideObjectRefIndex // Which index of refToTabViewObjects refers to this object.
  let maxNumberOfTabs
...
DHTMLSuite.tabView.prototype = {
  ...
  getReferenceToDivElementByTitle: function (tabTitle) {
    const index = this.getTabIndexByTitle(tabLabel) // Get index of tab
    if (index != -1) {
  ...
  createNewTab: function (
    ...
    const index = this.getTabIndexByTitle(tabTitle) // Get index of tab
    if (index != -1) {
    ...
    const div = document.createElement('DIV') // Create new tab content div.
    ...
    const tabId = this.__initializeAndParseTabs(true, tabTitle, closeButton) // Call the init method in order to create tab header and tab images
...
DHTMLSuite.JSDragDropTree.prototype = {
  ...
  init: function () {
    ...
    const menuItems = DHTMLSuite_tree.getElementsByTagName('LI') // Get an array of all menu items
...
DHTMLSuite.dynamicContent.prototype = {
  ...
  loadContent: function (divId, url, functionToCallOnLoaded) {
    ...
    if (url.indexOf('?') >= 0) {
      ...
      let string = url.substring(url.indexOf('?')) // Extract get variables
...
DHTMLSuite.slider = function () {
  let width // Width of slider
  let height // height of slider
  let targetObj // Object where slider will be added.
  let sliderWidth // Width of slider image. this is needed in order to position the slider and calculate values correctly.
  let sliderDirection // Horizontal or vertical
  let functionToCallOnChange // Function to call when the slider is moved.
  let layoutCss
  let sliderMaxValue // Maximum value to return from slider
  let sliderMinValue // Minimum value to return from slider
  let initialValue // Initial value of slider
  let sliderSize // Size of sliding area.
  let directionOfPointer // Direction of slider pointer;
  let slideInProcessTimer // Private variable used to determine if slide is in progress or not.
  let indexThisSlider // Index of object in the DHTMLSuite.sliderObjects array
  let numberOfSteps // Hardcoded steps
  let stepLinesVisibility // Visibility of lines indicating where the slider steps are

  let slide_event_pos // X position of mouse when slider drag starts
  let slide_start_pos // X position of slider when drag starts
  let sliderHandleImg // Reference to the small slider handle
  let sliderName // A name you can use to identify a slider. Useful if you have more than one slider, but only one onchange event.
  let sliderValueReversed // Variable indicating if the value of the slider is reversed(i.e. max at left and min at right)
...
DHTMLSuite.slider.prototype = {
  ...
  __initializeSliderDrag: function (e) {
    if (document.all) e = event
    DHTMLSuite.commonObj.__setTextSelOk(false)
    const numIndex = this.id.replace(/[^0-9]/gi, '') // Get index in the DHTMLSuite.sliderObject array. We get this from the id of the slider image(i.e. "this").
  ...
  __setPositionFromClick: function (e) {
    ...
    if (sliderObj.numberOfSteps) {
      ...
      let mod = handlePos % modValue // Calculate modulus
      if (mod > modValue / 2) mod = modValue - mod
      else mod *= -1 // Should we move the slider handle left or right?
...
DHTMLSuite.modalMessage = function (props) {
  let url // url of modal message
  let htmlOfModalMessage // html of modal message
  let domRef // Reference to DOM element

  let divs_transparentDiv // Transparent div covering page content
  let divs_content // Modal message div.
  let iframeEl // Iframe element used to cover select boxes in ie
  let layoutCss // Name of css file;
  let width // Width of message box
  let height // Height of message box
  let isModal // Is the modal message modal ?

  let existingBodyOverFlowStyle // Existing body overflow css
  let dynContentObj // Reference to dynamic content object
  let cssClassOfMessageBox // Alternative css class of message box - in case you want a different appearance on one of them
  let shadowDivVisible // Shadow div visible ?
  let shadowOffset // X and Y offset of shadow(pixels from content box)
...
DHTMLSuite.dynamicTooltip = function () {
  let x_offset_tooltip // X Offset tooltip
  let y_offset_tooltip // Y offset tooltip
  let ajax_tooltipObj
...
DHTMLSuite.dynamicTooltip.prototype = {
  ...
  displayTooltip: function (externalFile, inputObj, staticContent) {
    ...
    if (!this.ajax_tooltipObj) {
      ...
      const leftDiv = document.createElement('DIV') /* Create arrow div */
      ...
      const contentDiv =
        document.createElement('DIV') /* Create tooltip content div */
...
DHTMLSuite.infoPanel = function () {
  let xpPanel_slideActive // Slide down/up active?
  let xpPanel_slideSpeed // Speed of slide
  let xpPanel_onlyOneExpandedPane // Only one pane expanded at a time ?
  let savedActivePane
  let savedActiveSub
  ...
  let arrayOfPanes
  let dynamicContentObj
  let paneHeights // Array of info pane heights.
...
DHTMLSuite.infoPanel.prototype = {
  ...
  init: function () {
    ...
    for (let no = 0; no < this.arrayOfPanes.length; no++) {
      // Loop through panes
      const tmpDiv = document.getElementById(this.arrayOfPanes[no][0]) // Creating reference to pane div.
      ...
      if (nameOfCookie) {
        ...
        if (cookieValue) panelDisplayed = cookieValue == 1 // Cookie value exists? -> Expand or collapse pane.
      }
...
DHTMLSuite.menuModelItem = function () {
  let id // id of this menu item.
  let itemText // Text for this menu item
  let itemIcon // Icon for this menu item.
  let url // url when click on this menu item
  let parentId // id of parent element
  let separator // is this menu item a separator
  let jsFunction // Js function to call onclick
  let depth // Depth of this menu item.
  let hasSubs // Does this menu item have sub items.
  let type // Menu item type - possible values: "top" or "sub".
  let helpText // Help text for this item - appear when you move your mouse over the item.
  let state
  let submenuWidth // Width of sub menu items.
  let visible // Visibility of menu item.
...
DHTMLSuite.menuModel = function () {
  let menuItems // Array of menuModelItem objects
  let menuItemsOrder // This array is needed in order to preserve the correct order of the array above. the array above is associative
  // And some browsers will loop through that array in different orders than Firefox and IE.
  let submenuType // Direction of menu items(one item for each depth)
  let mainMenuGroupWidth // Width of menu group - useful if the first group of items are listed below each other
...
DHTMLSuite.menuModel.prototype = {
  ...
  addItem: function (
    ...
    try {
      ...
      alert(
        'Error: You need to include dhtmlSuite-menuModel.js in your html file'
      )
    }
  ...
  addItemsFromMarkup: function (ulId) {
    ...
    for (var no = 0; no < liTags.length; no++) {
      ...
      let parentId = 0 // Default parent id
      ...
      const itemIcon = liTags[no].getAttribute('itemIcon') // Item icon is set from the itemIcon attribute of the <li> tag.
      let url = aTag.href // url is set to the href attribute of the <a> tag
      if (url == '#' || url.substr(url.length - 1, 1) == '#') url = '' // # = empty url.

      const jsFunction = liTags[no].getAttribute('jsFunction') // jsFunction is set from the jsFunction attribute of the <li> tag.

      const submenuWidth = false // Not set from the <li> tag.
...
DHTMLSuite.menuItem = function () {
  let layoutCSS
  let divElement // the <div> element created for this menu item
  let expandElement // Reference to the arrow div (expand sub items)
  let cssPrefix // Css prefix for the menu items.
  let modelItemRef // Reference to menuModelItem
...
DHTMLSuite.menuItem.prototype = {
  ...
  createItem: function (menuModelItemObj) {
    ...
    const div = document.createElement('DIV') // Create main div
  ...
  setIcon: function (newPath) {
    ...
    if (this.modelItemRef.type == 'top') {
      ...
      let img = div.getElementsByTagName('IMG')[0] // Find the image
      if (!img) {
    ...
    if (this.modelItemRef.type == 'sub') {
      ...
      document.getElementById(this.divElement).style.backgroundImage =
        "url('" + newPath + "')" // Set backgroundImage for the main div(i.e. menu item div)
    }
  },
  // }}}
  // {{{ setText()
...
DHTMLSuite.menuBar = function () {
  let menuItemObj
  let layoutCSS // Name of css file
  let menuBarBackgroundImage // Name of background image
  let menuItem_objects // Array of menu items - html elements.
  let menuBarObj // Reference to the main dib
  let menuBarHeight
  let menuItems // Reference to objects of class menuModelItem
  let highlightedItems // Array of currently highlighted menu items.
  let menuBarState // Menu bar state - true or false - 1 = expand items on mouse over
  let activeSubItemsOnMouseOver // Activate sub items on mouse over	(instead of onclick)

  let submenuGroups // Array of div elements for the sub menus
  let submenuIframes // Array of sub menu iframes used to cover select boxes in old IE browsers.
  let createIframesForOldIeBrowsers // true if we want the script to create iframes in order to cover select boxes in older ie browsers.
  let targetId // Id of element where the menu will be inserted.
  let menuItemCssPrefix // Css prefix of menu items.
  let cssPrefix // Css prefix for the menu bar
  let menuItemLayoutCss // Css path for the menu items of this menu bar
  let objectIndex // Global index of this object - used to refer to the object of this class outside
  this.cssPrefix = 'DHTMLSuite_'
...
DHTMLSuite.menuBar.prototype = {
  ...
  __expandGroup: function (idOfParentMenuItem) {
    ...
    if (DHTMLSuite.clientInfoObj.isOpera) {
      /* Opera fix in order to get correct height of sub menu group */
      var subDiv =
        groupRef.getElementsByTagName('DIV')[0] /* Find first menu item */
      subDiv.className = subDiv.className.replace(
        '_over',
        '_over'
      ) /* By "touching" the class of the menu item, we are able to fix a layout problem in Opera */
    }
  },
  ...
  __activateMenuElements: function (e, firstIteration, parentMenuItemObject) {
    ...
    const numericId = parentMenuItemObject.id.replace(/[^0-9]/g, '') // Get a numeric reference to current menu item.
    const state = this.menuItem_objects[numericId].getState() // Get state of this menu item.
    ...
    DHTMLSuite.variableStorage.menuBar_highlightedItems[
      DHTMLSuite.variableStorage.menuBar_highlightedItems.length
    ] = this.menuItem_objects[numericId] // Save this menu item in the array of highlighted elements.
  ...
  __createMenuItems: function () {
    ...
    for (let no = 0; no < this.menuItemObj.menuItemsOrder.length; no++) {
      ...
      try {
        ...
        alert(
          'Error: You need to include dhtmlSuite-menuItem.js in your html file'
        )
      }
      ...
      ].createItem(this.menuItems[indexThis]) // Create div for this menu item.
      ...
      if (
        ...
        ref.setAttribute(
          'objectRef',
          index
        ) /* saving the index of this object in the DHTMLSuite.variableStorage array as a property of the tag - We need to do this in order to avoid circular references and thus memory leakage in IE */
      ...
      let target = this.menuBarObj // Temporary variable - target of newly created menu item. target can be the main menu object or a sub menu group(see below where target is updated).
      ...
      if (
        ...
        /* Main menu item group width set */
      ...
      if (this.menuItems[indexThis].depth == '1') {
        ...
        if (
          ...
          /* Type = "top" - menu items side by side */
    ...
    for (let no = 0; no < this.menuItemObj.menuItemsOrder.length; no++) {
      ...
      document.getElementById(this.menuItem_objects[id].divElement).className =
        document
          ...
          .className.replace('_regular', '_regular') // Nothing is done but by "touching" the class of the menu items in Opera, we make them appear correctly
    }
  },
...
DHTMLSuite.paneSplitterModel = function (arrayOfProperties) {
  let panes // Array of paneSplitterPaneModel objects
  let collapseButtonsInTitleBars // Place collapse button inside title bars ?
...
DHTMLSuite.paneSplitterPaneModel = function (inputArray) {
  let id // Unique id of pane, in case you want to perform operations on this particular pane.
  let position // Position, possible values: "West","East","North","South" and "Center"
  let size // Current size of pane(for west and east, the size is equal to width, for south and north, size is equal to height)
  let minSize // Minimum size(height or width) of the pane
  let maxSize // Maximum size(height or width) of the pane.
  let resizable // Boolean - true or false, is the pane resizable
  let visible // Boolean - true or false, is the pane visible?
  let scrollbars // Boolean - true or false, visibility of scrollbars when content size is bigger than visible area(default = true)
  let contents // Array of paneSplitterContentModel objects
  let collapsable // Boolean - true or false, is this pane collapsable
  let state // State of a pane, possible values, "expanded","collapsed"; (default = expanded)
  let callbackOnCollapse // Call back function - if ony function name is give, a reference to this model will be passed to the function with that name
  let callbackOnHide // Call back function - if ony function name is give, a reference to this model will be passed to the function with that name
  let callbackOnShow // Call back function - if ony function name is give, a reference to this model will be passed to the function with that name
  let callbackOnExpand // Call back function - if ony function name is give, a reference to this model will be passed to the function with that name
  let callbackOnSlideOut // Call back function - if ony function name is give, a reference to this model will be passed to the function with that name
  let callbackOnSlideIn // Call back function - if ony function name is give, a reference to this model will be passed to the function with that name
  let callbackOnCloseContent // Call back function - if ony function name is give, a reference to this model will be passed to the function with that name
  let callbackOnBeforeCloseContent // Call back function - if ony function name is give, a reference to this model will be passed to the function with that name
  let callbackOnTabSwitch // Call back function - if ony function name is give, a reference to this model will be passed to the function with that name
  let callbackOnResize // Call back function - called whenever a tab has been resized manually by dragging the "handle".
...
DHTMLSuite.paneSplitterContentModel = function (inputArray) {
  let id // Unique id of pane, in case you want to perform operations on this particular pane.
  let htmlElementId // Id of element on the page - if present, the content of this pane will be set to the content of this element
  let title // Title of pane
  let tabTitle // If more than one pane is present at this position, what's the tab title of this one.
  let closable // Boolean - true or false, should it be possible to close this pane
  let contentUrl // Url to content - used in case you want the script to fetch content from the server. the path is relative to your html page.
...
DHTMLSuite.paneSplitterPane = function (parentRef) {
  let divElement // Reference to a div element for the content
  let divElCollapsed // Reference to the div element for the content ( collapsed state )
  let divElCollapsedInner // Reference to the div element for the content ( collapsed state )
  let contentDiv // Div for the content
  let headerDiv // Reference to the header div
  let titleSpan // Reference to the <span> tag for the title
  let paneModel // An array of paneSplitterPaneView objects
  let resizeDiv // Div for the resize handle
  let tabDiv // Div for the tabs
  let divTransparentForResize // This transparent div is used to cover iframes when resize is in progress.
  var parentRef // Reference to paneSplitter object

  let divClose // Reference to close button
  let divCollapse // Reference to collapse button
  let divExpand // Reference to expand button
  let divRefresh // Reference to refresh button

  let slideIsInProgress // Internal variable used by the script to determine if slide is in progress or not
  let reloadIntervalHandlers // Array of setInterval objects, one for each content of this pane

  let contentScrollTopPositions // Array of contents scroll top positions in pixels.
  ...
  let activeContentIndex // Index of active content(default = 0)
  this.activeContentIndex = false
  this.slideIsInProgress = false
  let objectIndex // Index of this object in the variableStorage array
  this.objectIndex = DHTMLSuite.variableStorage.arrayDSObjects.length
  DHTMLSuite.variableStorage.arrayDSObjects[this.objectIndex] = this
}
DHTMLSuite.paneSplitterPane.prototype = {
  ...
  loadContent: function (
    ...
    for (let no = 0; no < this.paneModel.contents.length; no++) {
      if (this.paneModel.contents[no].id == idOfContentObject) {
        ...
        const ajaxWaitMsg = this.parentRef.waitMessage // Please wait message
  ...
  setContentTitle: function (idOfContent, newTitle) {
    const contentModelIndex = this.paneModel.__getIndexById(idOfContent) // Get a reference to the content object
  ...
  __createCollapsedPane: function () {
    ...
    let buttonSuffix = 'Vertical' // Suffix to the class names for the collapse and expand buttons
  ...
  __autoSlideInPane: function (e) {
    if (document.all) e = event
    const state = this.paneModel.__getState() // Get state of pane
  ...
  __slidePane: function (e, forceSlide) {
    ...
    let src = DHTMLSuite.commonObj.getSrcElement(e) // Get a reference to the element triggering the event
  ...
  __processSlideByPixels: function (
    ...
    let repeatSlide = true // Repeat one more time ?
  ...
  __createHeaderBar: function () {
    const ind = this.objectIndex // Making it into a primitive variable
    const pos = this.paneModel.getPosition() // Get position of this pane
    let buttonSuffix = 'Vertical' // Suffix to the class names for the collapse and expand buttons
    if (pos == 'west' || pos == 'east') buttonSuffix = 'Horizontal'
    if (pos == 'center') buttonSuffix = ''
  ...
  __updateHeaderBar: function (index) {
    if (index === false) {
      ...
      try {
        ...
        } else this.divCollapse.style.display = 'none' // Make collapse button visible for all panes except center
      } catch (e) {}
  ...
  __addOneContentDiv: function (contentObj, onCompleteJsCode) {
    const htmlElementId = contentObj.htmlElementId // Get a reference to content id
    const contentUrl = contentObj.contentUrl // Get a reference to content id
    const refreshAfterSeconds = contentObj.refreshAfterSeconds // Get a reference to content id
    if (htmlElementId) {
      try {
  ...
  __createAContentDivDynamically: function (contentObj) {
    const d = new Date() // Create unique id for a new div
    let divId =
      'dynamicCreatedDiv__' +
      d.getSeconds() +
      (Math.random() + '').replace('.', '')
...
DHTMLSuite.paneSplitter = function () {
  let dataModel // An object of class DHTMLSuite.paneSplitterModel
  let panes // An array of DHTMLSuite.paneSplitterPane objects.
  let panesAssociative // An associative array of panes. used to get a quick access to the panes
  let paneContent // An array of DHTMLSuite.paneSplitterPaneView objects.
  let layoutCSS // Name/Path of css file

  let horizontalSplitterSize // Height of horizontal splitter
  let horizontalSplitterBorderSize // Height of horizontal splitter

  let verticalSplitterSize //

  let paneSplitterHandles // Associative array of pane splitter handles
  let paneSplitterHandleOnResize

  let resizeInProgress // Variable indicating if resize is in progress

  let resizeCounter // Internal variable used while resizing (-1 = no resize, 0 = waiting for resize)
  let currentResize // Which pane is currently being resized ( string, "west", "east", "north" or "south"
  let currentResize_min
  let currentResize_max

  let paneSizeCollapsed // Size of pane when it's collapsed ( the bar )
  let paneBorderLeftPlusRight // Sum of border left and right for panes ( needed in a calculation)

  let slideSpeed // Slide of pane slide
  let waitMessage // Ajax wait message
  let collapseExpandButtons // Reference to collapse and expand buttons
...
DHTMLSuite.paneSplitter.prototype = {
  ...
  isUrlLoadedInPane: function (id, url) {
    const ref = this.__getPaneReferenceFromContentId(id) // Get a reference to the pane where the content is.
    if (ref) {
      // Pane found
   ...
   */
  setRefreshAfterSeconds: function (idOfContentObject, refreshAfterSeconds) {
    const ref = this.__getPaneReferenceFromContentId(idOfContentObject) // Get a reference to the pane where the content is.
    if (ref) {
      // Pane found
  ...
  __createPanes: function () {
    const dataObjects = this.dataModel.getItems() // An array of data source objects, i.e. panes.
    for (let no = 0; no < dataObjects.length; no++) {
      ...
      this.panesAssociative[dataObjects[no].position.toLowerCase()] =
        this.panes[index] // Save this pane in the associative array
    }
  },
  // }}}
  // {{{ __collapseAPane()
  ...
  __initResizePane: function (e, pos) {
    if (document.all) e = event
    const obj = DHTMLSuite.commonObj.getSrcElement(e) // Reference to element triggering the event.
  ...
  __positionResizeHandles: function () {
    ...
    let heightHandler =
      this.panesAssociative.center.divElement.offsetHeight + 1 // Initial height
    var topPos = 0
    if (this.panesAssociative.center) {
      topPos +=
...
DHTMLSuite.textEditModel = function (inputArray) {
  let labelId // Id of label for editable element.
  let targetId // Id of editable element.
  let serversideFile // If individual serverside file should be used for this option
  let optionObj // Reference to object of class DHTMLSuite.listModel
...
DHTMLSuite.textEdit = function () {
  let layoutCSS // Name of css file
  let elements // Array of editable elements
  let elementsAssociative // Associative version of the array above - need two because of conflicts with Prototype library when using for in loops.
  let serversideFile // Path to file on the server where changes are sent.
  let objectIndex
  let inputObjects // Array of inputs or select boxes
...
DHTMLSuite.textEdit.prototype = {
  ...
  addElement: function (inputArray) {
    ...
    try {
      ...
      alert(
        'Error: You need to include dhtmlSuite-textEditModel.js in your html file'
      )
    }
  ...
  init: function () {
    ...
    for (let no = 0; no < this.elements.length; no++) {
      ...
      if (el) {
        ...
        if (obj.listModel) {
          /* List model exists - create select box */
  ...
  __exitEditMode: function (e) {
    ...
    } // Send ajax request when changes has been made.
...
DHTMLSuite.contextMenu = function () {
  ...
  let menuItems
  let menuObject // Reference to context menu div
  let layoutCSS
  let menuUls // Array of <ul> elements
  let width // Width of context menu
  let srcElement // Reference to the element which triggered the context menu, i.e. the element which caused the context menu to be displayed.
  let indexCurrentlyDisplayedMenuModel // Index of currently displayed menu model.
  let menuBar
...
DHTMLSuite.mediaCollection = function () {
  let mediaObjects // Array of DHTMLSuite.mediaModel objects
...
DHTMLSuite.floatingGallery = function () {
  let collectionModel // Reference to an object of class DHTMLSuite.mediaCollection
  let layoutCSS
  let divElement // Reference to parent div element.
  let divElementImageBoxes // Array of div elements for the images
  let idOfParentElementToGallery // Id of parent element

  // Strings - callback functions.
  let callBackFunction_onClick // Call back on click
  let callBackFunction_onDblClick // Call back on dbl click
  let callBackFunction_onMouseOver // Call back on mouse over
  let callBackFunction_onMouseMove // Call back mouse move

  let imageSelectionObj // Object of class DHTMLSuite.imageSelection
  let objectIndex
...
DHTMLSuite.floatingGallery.prototype = {
  ...
  deleteImageFromGallery: function (idOfImage) {
    ...
    if (retId) {
      // media model image exists with this id ?
      const obj = document.getElementById(retId)
      DHTMLSuite.discardElement(obj)
    } else {
      // id doesn't match id in media collection model. loop through div elements and check each one by id.
      for (let no = 0; no < this.divElementImageBoxes.length; no++) {
        if (this.divElementImageBoxes[no].id == idOfImage) {
          // Match found
          let mediaRefId =
            this.divElementImageBoxes[no].getAttribute('mediaRefId') // get a media reference.
          if (!mediaRefId) {
...
DHTMLSuite.imageSelection = function () {
  let layoutCSS
  let callBackFunction_onDrop // Call back function on drop
  let objectIndex
  ...
  let collectionModelReference // Reference to media collection ( data source for images )
  let destinationEls // Array of destination elements.
  let currentDestEl // Reference to current destination element

  let selectionStatus // -1 when selection isn't in progress, 0 when it's being initialized, 5 when it's ready
  let dragStatus // -1 = drag not started, 0-5 = drag initializing, 5 = drag in process.
  let startCoordinates
  let selectionResizeInProgress // variable which is true when code for the selection area is running
  let selectionStartArea // Selection can only starts within this element
  let selectionOrDragStartEl // Element triggering drag or selection
...
DHTMLSuite.imageEnlarger.prototype = {
  ...
  __makeElementDragable: function () {
    try {
      ...
      alert(
        'You need to include DHTMLSuite-dragDropSimple.js for the drag feature'
      )
    }
  },
  // }}}
...
DHTMLSuite.calendarLanguageModel = function (languageCode) {
  let monthArray // An array of months.
  let monthArrayShort // An array of the months, short version
  let dayArray // An array of the days in a week
  let weekString // String representation of the string "Week"
  let todayString // String representatinon of the string "Today"
  let todayIsString // String representation of the string "Today is"
  let timeString // String representation of the string "Time"
...
DHTMLSuite.calendarModel = function (inputArray) {
  let initialDay // Initial day (i.e. day in month)
  let initialMonth // Initial month(1-12)
  let initialYear // Initial Year(4 digits)
  let initialHour // Initial Hour(0-23)
  let initialMinute // Initial Minute

  let displayedDay // Currently displayed day
  let displayedMonth // Currently displayed month
  let displayedYear // Current displayed year
  let displayedMinute // Currently displayed Minute
  let displayedHour // Current displayed Hour
  let languageCode // Current language code
  let languageModel // Reference to object of class DHTMLSuite.calendarLanguageModel
  let invalidDateRange // Array of invalid date ranges
  let weekStartsOnMonday // Should the week start on monday?
...
DHTMLSuite.calendarModel.prototype = {
  ...
  __createDefaultModel: function (inputArray) {
    ...
    if (inputArray) {
      /* Initial date data sent to the constructor ? */
...
DHTMLSuite.calendar = function (propertyArray) {
  let id // Unique identifier - optional
  let divElement
  let divElContent // Div element for the content inside the calendar
  let divElHeading
  let divElNavBar
  let divElMonthView // Div for the main view - weeks, days, months etc.
  let divElMonthNInHead
  let divElYearInHeading
  let divElBtnPreviousYear // Button - previous year
  let divElBtnNextYear // Button - next year
  let divElBtnPrvMonth // Button - previous Month
  let divElBtnNextMonth // Button - next Month
  let divElYearDropdown // Dropdown box - years
  let divElYearDropdownParentYears // Inner div inside divElYearDropdown which is parent to all the small year divs.
  let divElHourDropdownParentHours // Inner div inside divElYearDropdown which is parent to all the small year hours.
  let divElHourDropdown // Drop down hours
  let divElMinuteDropdownParent // Inner div inside divElYearDropdown which is parent to all the small year Minutes.
  let divElMinuteDropdown // Drop down Minutes
  let divElTodayInNavBar // Today in navigation bar.
  let divElHrInTimeBar // Div for hour in timer bar
  let divElMinInTimeBar // Div for minute in timer bar
  let divElTimeStringInTimeBar // Div for "Time" string in timer bar

  let iframeEl
  let iframeElDropDowns
  let calendarModelReference // Reference to object of class calendarModel
  let objectIndex
  let targetReference // Where to insert the calendar.
  let layoutCSS
  let isDragable // Is the calendar dragable - default = false
  let referenceToDragDropObject // Reference to object of class DHTMLSuite.dragDropSimple
  let scrollInYearDropDownActive // true when mouse is over up and down arrows in year dropdown
  let scrollInHourDropDownActive // true when mouse is over up and down arrows in hour dropdown
  let scrollInMinuteDropDownActive // true when mouse is over up and down arrows in minute dropdown
  let yearDropDownOffsetInYear // Offset in year relative to current displayed year.
  let hourDropDownOffsetInHour // Offset in hours relative to current displayed hour.
  let minuteDropDownOffsetInHour // Offset in minute relative to current displayed minute.

  let displayCloseButton // Display close button at the top right corner
  let displayNavigationBar // Display the navigation bar ? ( default = true)
  let displayTodaysDateInNavigationBar // Display the string "Today" in the navigation bar(default = true)
  let displayTimeBar // Display timer bar - default = false;

  let posRefToHtmlEl // reference to html element to position the calendar at
  let positioningOffsetXInPixels // Offset in positioning when positioning calendar at a element
  let positioningOffsetYInPixels // Offset in positioning when positioning calendar at a element
  let htmlElementReferences
  let minuteDropDownInterval // Minute drop down interval(interval between each row in the minute drop down list)

  let numberOfRowsInMinuteDropDown // Number of rows in minute drop down. (default = 10)
  let numberOfRowsInHourDropDown // Number of rows in hour drop down. (default = 10)
  let numberOfRowsInYearDropDown // Number of rows in year drop down. (default = 10)
  ...
  let callbackFunctionOnDayClick // Name of call back function to call when you click on a day
  let callbackFunctionOnClose // Name of call back function to call when the calendar is closed.
  let callbackFunctionOnMonthChange // Name of call back function to call when the month is changed in the view
  let dateOfToday
...
DHTMLSuite.calendar.prototype = {
  ...
  setDisplayedYear: function (year) {
    const success = this.calendarModelReference.__setDisplayedYear(year) // Year has actually changed
  ...
  setDisplayedMonth: function (month) {
    const success = this.calendarModelReference.__setDisplayedMonth(month) // Month have actually changed
    this.__populateCalHeading()
    this.__populateMonthView()
    if (success) this.__handleCalendarCallBack('monthChange')
  },
  ...
  __populateDropDownMonths: function () {
    ...
    const ind = this.objectIndex // Get a reference to this object in the global object array.
    const months = this.calendarModelReference.__getMonthNames() // Get an array of month name according to current language settings
  ...
  __populateMinutesInsideDropDownMinutes: function () {
    ...
    if (60 / this.minuteDropDownInterval < this.numberOfRowsInMinuteDropDown) {
      ...
      if (startMinute < 0) {
        /* Start minute negative - adjust it and change offset value */
...
DHTMLSuite.windowModel = function (arrayOfProperties) {
  ...
  let icon // Icon of window
  let isDragable // Window is dragable ? (default = true)
  let isResizable // Window is resizable ?
  let isMinimizable // Window is minimizable ?
  let isClosable // Window is closable
  let xPos // Current x position of window
  let yPos // Current y position of window
  let width // Current width of window
  let height // Current height of window
  let cookieName // Name of cookie to store x,y,width,height,state,activeTab and zIndex
  let state // State of current window(minimized,closed etc.)
  let activeTabId // id of active tab
  let tabsVisible // Tabs are visible? If not, we will only show a simple window with content and no tabs.
  let zIndex // Current z-index of window.
  let minWidth // Minimum width of window
  let maxWidth // Maximum width of window
  let minHeight // Minimum height of window
  let maxHeight // Maximum height of window.
  let isVisible // Is window visible

  let callbackOnClose // Callback to execute on close
  let windowsTheme
  let windowContents // Array of DHTMLSuite.windowTabModel objects.
...
DHTMLSuite.windowWidget = function (windowModel) {
  ...
  let divElement // parent element of divElInner and eventual iframe(old MSIE)
  let divElInner // Div element for all the content of an iframe
  let divTitleBar // Div element for the title bar.
  let divElContent // Div for the content
  let divCloseButton // Close button
  let divMinimizeButton // Minimize button
  let divStatusBarTxt // Text element in status bar
  let divTitleIcon // Icon div
  let divResizeHandle // Div element for the resize handle
  let iframeEl // Iframe element used to cover select boxes in IE.
  let divElementTitle_txt
  let referenceToDragDropObject // Reference to object of class dragDropSimple.
  let contentDivs // Array of div elements for the content
  let resizeObj // Reference to object of class DHTMLSuite.resize
  let slideSpeed // Slide speed when window is "sliding" into position by a windowCollection call.
...
DHTMLSuite.windowWidget.prototype = {
  ...
  __createTitleBar: function () {
    ...
    const buttonDiv = document.createElement('DIV') // Creating parent div for buttons
  ...
  __initiallyPopulateContentArea: function () {
    const contentObjects = this.windowModel.__getContentObjects() // Get an array of content objects
  ...
  __clearActiveAndInactiveStylingFromTabs: function () {
    const cells = this.divElementTabRow.getElementsByTagName('TD') // Get an array of <td> elements.
    const divs = this.divElementTabRow.getElementsByTagName('DIV') // Get an array of <td> elements.
...
DHTMLSuite.windowCollection = function () {
  ...
  let divWindowsArea // Eventual div where the windows should be positioned
...
DHTMLSuite.resize = function (propertyArray) {
  ...
  let mouseStartPos // Position of mouse pointer when the resize process starts
  let initElementSize
  let currentResizeDirection // In which direction are we currently resizing the element(example: "west","east","southeast")
  let classNameOfResizeHandles // Class name of resize handles, in case they are created dynamically
  let layoutCSS
...
DHTMLSuite.colorSlider = function (propertyArray) {
  let divElement
  let layoutCSS
  let colorHelper // Object of class DHTMLSuite.colorUtil
...
DHTMLSuite.colorSlider.prototype = {
  ...
  __getHorizColor: function (i, width, height) {
    const sWidth = width / 7 // "section" width
    const C = i % width // column
    const R = Math.floor(i / (sWidth * 7)) // row
    const c = i % sWidth // column in current group
    let r, g, b, h

    const l = (255 / sWidth) * c // color percentage
...
DHTMLSuite.colorWidget = function (propertyArray) {
  ...
  let circleOffsetSize // Size of small circle / 2 rounded
  let posdivElPalette
  let parentRef
...
DHTMLSuite.formValidator.prototype = {
  ...
  __initiallyParseAForm: function () {
    ...
    const vElements = new Array() // Array of elements to parse
  ...
  __isInputValid: function (inputRef, circular) {
    ...
    if (elType == 'text') {
      ...
      if (pat.indexOf('/') == -1) pat = this.masks[pat]
      else pat = eval(pat)
