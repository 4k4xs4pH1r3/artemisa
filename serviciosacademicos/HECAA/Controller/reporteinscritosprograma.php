<?php   
    ...
    if($codigoperiodo)
    {
        ...
        $html1 = "Error,Programa,Año,Semestre,Tipo_Documento,Numero_Documento,PRO_CONSECUTIVO,ID_MUNICIPIO\r\n";
        $htmlregistro ='';
        ...
        if(!empty($Inscritos))
        {
            foreach($Inscritos as $valores)
            {
                ...
                $codigocarrera = $valores['codigocarrera'];   

				$carreraHecaa = $funciones->carreraregistro($db,$codigocarrera);			
                ...
                if($numerodoc != '')
                {
                    ...
                    $htmlRegistro.=$validacion['Error'].",".$validacion['programa'].",".$anio.",".$periodo.",".$validacion['TipoDocumento'].",'".$datosestudiante['numerodocumento'].",".$carreraHecaa.",11001\r\n";     
        ...
        $funciones->logReporte($db, "InscritosPrograma", $usuario, $contador, $codigoperiodo);
        
        $html = $html1.$htmlRegistro;
        
        header("Content-Type: text/csv");
        header("Content-Disposition: attachment; filename=Reporte_InscritosPrograma_".$codigoperiodo.".csv");// Disable caching
