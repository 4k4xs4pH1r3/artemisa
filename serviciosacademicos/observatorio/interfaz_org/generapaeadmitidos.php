...
if ($tipo2=='R') {
     $query_carrera = "SELECT ee.codigoestudiante, ee.codigoperiodo, e.codigocarrera, eg.idestudiantegeneral,
                     ...
                     <td> 
                        <?php
                             ...
                             $sqlpi="select * 
                                    ...
                                    where r.codigoestudiante='".$dt['idestudiantegeneral']."'    ";
                          //  echo $sqlpi; 
                             $data_pi= $db->Execute($sqlpi);
                             $E_data = $data_pi->GetArray();
                             $E_data=$E_data[0];
                            // print_r($E_data);
                             $idpi=$E_data['idobs_registro_riesgo'];
                             $cat=count($E_data);
                             if ($cat==0){
                                ?>
                                 <button type="button" id="crear" name="crear" title="Realizar Entrevista" onclick="updateForm3('form_registro_riesgo.php?tipo=Prueba&codigoestudiante=<?php echo $dt['codigoestudiante'] ?>&id_res=<?php echo $dt['idobs_registro_riesgo'] ?>')"><img src="../img/agregar.png" width="20px" height="20px"  /></button>
                                <?php
                                }else{
                                   ?>
