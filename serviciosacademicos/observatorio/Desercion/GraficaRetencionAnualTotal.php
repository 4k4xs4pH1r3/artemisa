...
//var_dump (is_file('../../mgi/datos/templates/template.php'));die;
    ...
    <html>
       ...
       <?PHP 
        ...
        //echo '<pre>';print_r($D_PeriododAnual);die;
       ...
       // echo '<pre>';print_r($C_Datos);
        ...
        /****************************************/
        for($i=0;$i<count($D_PeriododAnual);$i++){//for
                    
                    $Periodo = $D_PeriododAnual[$i];
                    
                    $R_Total[]  = $C_Desercion->SumaTotalAnual($Periodo);
                    $C_Datos['desercionperiodo'][] = $Periodo;
                    
                    
                    
        }//for
        
        //echo '<pre>';print_r($R_Total);die;
        
        
        for($j=0;$j<count($R_Total);$j++){
            
            $T_Retenido = $R_Total[$j]['TotalMatriculados']-$R_Total[$j]['TotalDesercion'];
                    
            $T_PorcentajeRetnido  = (($T_Retenido*100)/$R_Total[$j]['TotalMatriculados']);
            
            $C_Datos['Porcentaje'][] = number_format($T_PorcentajeRetnido,'2','.','.');
        ...
        $MyData->addPoints($C_Datos['Porcentaje'],$NombreCarrera." ".$P_Periodo);
        //$MyData->addPoints($D_Total['Porcentaje'],"Desercion Pregrado".$P_Periodo."-".$Ac_Periodo);
        $MyData->setSerieTicks("Permanencia Pregrado Total".$P_Periodo,4);   
        ...
        $MyData->setPalette(" Permanencia Total Anual ".$P_Periodo,array("R"=>44,"G"=>87,"B"=>0));
        ...
        */
        ...
        $AxisBoundaries = array(0=>array("Min"=>$K,"Max"=>$N));
