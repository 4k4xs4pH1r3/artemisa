...
//var_dump (is_file('../../mgi/datos/templates/template.php'));die;
    ...
    <html>
       ...
       <?PHP 
        ...
        $CodigoBusqueda  = $CodigoPeriodo.'1';
        
        $PeriodoAnual   = $C_Desercion->PeriodosAnuales($CodigoBusqueda);
        
        //echo '<pre>';print_r($PeriodoAnual);die;
        ...
        for($i=0;$i<count($D_Periodo);$i++){
            ...
            for($j=0;$j<count($C_Carrera);$j++){//for
                ...
                $Valor_Actual = $C_DesercionCostos->CostoActual($Carrera_id,$Periodo);
            
                $S_CostoA    = $S_CostoA+$Valor_Actual;
            }//for Carreras
            
            $C_Valores['Periodos'][]=$Periodo;
            $C_Valores['Valores'][]=$S_CostoA;//number_format($S_CostoA,'0','.','.');
        }//for periodos
       ...
       /* echo '<pre>';print_r($C_Datos);
        ...
        $MyData->addPoints($C_Valores['Valores'],"Costo Deserción Institucional Por Periodo".$P_Periodo."-".$Ac_Periodo);
        //$MyData->setSerieTicks("Desercion Pregrado".$P_Periodo."-".$Ac_Periodo,4);   
        
        $MyData->setAxisName(0,"Costo Desercion");
        $MyData->addPoints($C_Valores['Periodos'],"Labels");
        $MyData->setSerieDescription("Labels","Periodo");
        $MyData->setAbscissa("Labels");
        $MyData->setPalette($NombreCarrera." Deserción ".$P_Periodo."-".$Ac_Periodo,array("R"=>44,"G"=>87,"B"=>0));
        ...
        */
        $myPicture->setGraphArea(60,80,650,600);
