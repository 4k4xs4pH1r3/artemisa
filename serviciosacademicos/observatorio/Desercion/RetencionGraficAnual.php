...
//var_dump (is_file('../../mgi/datos/templates/template.php'));die;
    ...
    <html>
        ...
        */ 
        ...
        for($i=0;$i<count($C_Datos['Matriculados']);$i++){
            
            $Retencion   = $C_Datos['Matriculados'][$i]-$C_Datos['Desercion'][$i];
            
            $Porcentaje  = (($Retencion*100)/$C_Datos['Matriculados'][$i]);
            
            $D_Datos['Porcentaje'][]  = number_format($Porcentaje,'2','.','.');
            
        }//for
        
        for($j=0;$j<count($D_Total['Matriculados']);$j++){
            
            $T_Retencion = $D_Total['Matriculados'][$j]-$D_Total['Desercion'][$j];
            
            $T_Porcentaje = (($T_Retencion*100)/$D_Total['Matriculados'][$j]);
            
            $T_Total['Porcentaje'][] = number_format($T_Porcentaje,'2','.','.');
            
        }//for
       // echo '<pre>';print_r($D_Datos);
        /***************************************/
        ...
        $N=max($D_Datos['Porcentaje']);
        $K=max($T_Total['Porcentaje']);
        ...
        $MyData->addPoints($D_Datos['Porcentaje'],$NombreCarrera." ".$P_Periodo."-".$Ac_Periodo);
        $MyData->addPoints($T_Total['Porcentaje'],"Desercion Pregrado".$P_Periodo."-".$Ac_Periodo);
        $MyData->setSerieTicks("Permanencia Pregrado".$P_Periodo."-".$Ac_Periodo,4);   
        
        $MyData->setAxisName(0,"Permanencia  %");
        $MyData->addPoints($C_Datos['Periodo'],"Labels");
        $MyData->setSerieDescription("Labels","Periodo");
        $MyData->setAbscissa("Labels");
        $MyData->setPalette($NombreCarrera." Permanencia ".$P_Periodo."-".$Ac_Periodo,array("R"=>44,"G"=>87,"B"=>0));
        ...
        $myPicture->drawText(10,16,"Retencion Anual",array("FontSize"=>9,"Align"=>TEXT_ALIGN_BOTTOMLEFT));
