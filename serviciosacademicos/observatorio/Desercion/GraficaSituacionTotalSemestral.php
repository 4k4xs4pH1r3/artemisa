...
//var_dump (is_file('../../mgi/datos/templates/template.php'));die;
    ...
    MainJson();
        ...
        $T_Desercion    = $_REQUEST['T_Desercion'];
        
        $arrayP = str_split($CodigoPeriodo, strlen($CodigoPeriodo)-1);
        
        $P_Periodo=$arrayP[0]."-".$arrayP[1];
        
        $C_Datos        = $C_Desercion->CausasDesercionTotalSemestral($CodigoPeriodo,$T_Desercion);
        
       
        
        //$C_Datos        = $C_Desercion->CausasDesercion($CodigoPeriodo,$CodigoCarrera);
...
//$Settings = array("StartR"=>209, "StartG"=>250, "StartB"=>231, "EndR"=>111, "EndG"=>3, "EndB"=>138, "Alpha"=>50);
$Settings = array("StartR"=>209, "StartG"=>250, "StartB"=>231, "EndR"=>111, "EndG"=>3, "EndB"=>138, "Alpha"=>50);
...
$PieChart->draw3DPie(450,250,array("Radius"=>200,"DataGapAngle"=>18,"DataGapRadius"=>6,"Border"=>TRUE,"DrawLabels"=>TRUE,"WriteValues"=>PIE_VALUE_NATURAL));


$PieChart->drawPieLegend(20,450,array("Mode"=>LEGEND_HORIZONTAL,"Style"=>LEGEND_NOBORDER,"Alpha"=>20));
