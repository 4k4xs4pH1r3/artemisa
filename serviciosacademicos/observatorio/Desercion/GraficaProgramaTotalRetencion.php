...
for($i=1;$i<count($C_Cadena);$i++){
    /***********************************/
        ...
        $C_Datos        = $C_Desercion->CadenaProRetencion($CodigoPeriodo,$C_Cadena[$i],'1');
        
        for($j=0;$j<count($C_Datos);$j++){
            
            $C_Porcentaje = $C_Desercion->CadenaProRetencion($CodigoPeriodo,$C_Cadena[$i]);
        }
        ...
        // echo '<pre>';print_r($C_Porcentaje['Desercion']);
         ...
         $Valor = number_format(($C_Suma/$n),'2','.','');
...
$Periodo_Actual = $C_Desercion->Periodo('Actual','','');
        ...
        /***************************************/
//
//$D_Total        = $C_Desercion->CadenaInstitucional($CodigoPeriodo);
//echo '<pre>',print_r($C_Array); die;
/*******************************************/
//echo '<pre>';print_r($Cadena);
...
/*************************************************/
 ...
 $MyData->setAbscissaName("Programas Pregrado");
...
$myPicture->drawText(10,16,"Permanencia Para Programas Del Periodo ".$P_Periodo."  ".$Ac_Periodo,array("FontSize"=>9,"Align"=>TEXT_ALIGN_BOTTOMLEFT));
//
