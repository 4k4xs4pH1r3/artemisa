...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	...
	if ( isset( $_GET['iSortCol_0'] ) )
	{
		...
		for ( $i=0 ; $i<intval( $_GET['iSortingCols'] ) ; $i++ )
		{
			...
			{
                                if(empty($sTableNickname)){
                                    $sOrder .= "".$aColumns[ intval( $_GET['iSortCol_'.$i] ) ]." ".
                                            mysql_real_escape_string( $_GET['sSortDir_'.$i] ) .", ";
                                } else {                                    
                                    $sOrder .= "".$sTableNickname.".".$aColumns[ intval( $_GET['iSortCol_'.$i] ) ]." ".
                                            mysql_real_escape_string( $_GET['sSortDir_'.$i] ) .", ";
                                }
			}
		}
	...
	if ( isset($_GET['sSearch']) && $_GET['sSearch'] != "" )
	{
		...
		for ( $i=1 ; $i<count($aColumns) ; $i++ )
		{
                    ...
                    $j = $i - 1;
                    if ( (isset($_GET['bSearchable_'.$j]) && $_GET['bSearchable_'.$j] == "true" ) && $_GET['sSearch'] != '' )
                    {
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
                ...
                for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			if ( $columna == $sIndexColumn && !$gotID){
                            $row[DT_RowId]= 'row_'.$aRow[ $columna ];
                             $gotID = true;
