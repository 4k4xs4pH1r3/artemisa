<?php
   ...
   $db =writeHeader("Movilidad<br> estudiantil",true,"PAE",1);
   /* include("./menu.php");
    writeMenu(0);*/
   $tipo=$_REQUEST['tipo'];
    ...
    function Buscar(){
        ...
        if (modalidad==0 && nestudiante==''){
           ...
           $('#result').html('<blink>Cargando...</blink>');
            $.ajax({//Ajax
                      type: 'POST',
                      url: 'buscar_internalizacion_academica.php',
                      async: false,
                      //dataType: 'json',
                      data:({modalidad:modalidad, nestudiante:nestudiante, carrera:carrera }),
