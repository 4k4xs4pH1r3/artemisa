   ...
   // echo '<table border="1">';
		    ...
		    foreach ($data->sheets[0]['cells'] as $llave => $valor)
		    {
		      ...
		      // $idMaterias = array("14","15","10");
		       $notas = array($valor[7],$valor[9],$valor[8]);
					...
					if( $idPruebaEstado != ""){
						...
						//foreach( $idMaterias as $idMateria ){ 	
							//foreach( $notas as $nota ){
								...
								$sqlActualiza = "UPDATE detalleresultadopruebaestado
													SET 
													 notadetalleresultadopruebaestado = '$notas[$k]'
													WHERE
														idresultadopruebaestado = $idPruebaEstado
													AND idasignaturaestado = ".$idMateria["idasignaturaestado"]."";
								$actualizaResultado = $db->Execute( $sqlActualiza );
