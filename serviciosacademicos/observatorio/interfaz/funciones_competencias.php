...
$db = writeHeaderBD();
		...
		$SQL='SELECT 
		 ...
		 foreach($arreglosMaterias as $key=>$resultadosMateria){
			...
			//por materia maximo y el minimo
			$puntajeMinimo = $arreglosMaterias[$key][0]["resultado"]; 
			$puntajeMaximo = $arreglosMaterias[$key][count($arreglosMaterias[$key])-1]["resultado"]; 	
			$rangoValor = ($puntajeMaximo - $puntajeMinimo)/5; 
			
			$maxRango = $puntajeMinimo+$rangoValor;
			
			foreach($resultadosMateria as $resultadoEstudiante){				
				
				if($resultadoEstudiante["resultado"]<=$maxRango){
					$quintilActual = $quintil;
				} else {
					$maxRango = $maxRango + $rangoValor;
					$quintil++;	
					$quintilActual = $quintil;				
				}
