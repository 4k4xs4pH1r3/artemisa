...
ini_set('display_errors', '0');
   ...
   if($tipo=='R'){
       $tit='Estu. Remitidos<br>Seg. Psicol&oacute;gico';
   }else if($tipo=='P'){
       $tit='Estu. Atendidos<br>Seg. Psicol&oacute;gico';
   }
   $db =writeHeader($tit,true,"seguimiento",1,'Seguimiento Psicologico');
    ...
    function Buscar(){
        ...
        if (modalidad==0){
           ...
           $('#result').html('<blink>Cargando...</blink>');
            $.ajax({//Ajax
                      type: 'POST',
                      url: 'buscar_remision_psicologico.php',
                      async: false,
                      //dataType: 'json',
