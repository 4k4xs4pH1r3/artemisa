<?php
   ...
   $db =writeHeader("Estudiantes por <br> Admitir",true,"Admisiones",1);
   /* include("./menu.php");
    writeMenu(0);*/
   $tipo=$_REQUEST['tipo'];
   ...
   $entity2 ->sql_where = "cedula_usuario= '".$n_doc."' and modulo='Coordinador - Entrevista'";
   //$entity2->debug = true;
   $dataD2 = $entity2->getData();
   $total=  count($dataD2);
   $modulo=$dataD2[0]['modulo'];
   
   //if ($total>0 && $modulo=='Coordinador - Entrevista'){
    ?>
<script>
           ...
           $('#result').html('<blink>Cargando...</blink>');
            $.ajax({//Ajax
                      ...
                      data:({periodo:periodo, modalidad:modalidad, facultad:facultad,
                             nestudiante:nestudiante, carrera:carrera, tipo:tipo, Utipo:'coordinador', tipo2:tipo2, vtipo:'Adm' }),
                   ...
                   <?PHP
                    ...
                    <?PHP
                          ...
                          $reg_programa =$db->Execute($query_carrera);
                               ...
                               $query_carrera = "SELECT nombrecarrera, codigocarrera 
                               FROM carrera 
                               where codigofacultad='".$_SESSION['codigofacultad']."'  ";
                                $reg_programa =$db->Execute($query_carrera);
                           }
                         ...
                         // echo $query_carr.'<br>';
						   ...
						   $SQL_D='SELECT 
									...
									$query_carrera = $query_carrera.' UNION ('.$SQL_D.')';
                                 ...
                                 $query_carrera = "SELECT c.nombrecarrera, c.codigocarrera 
                                                   FROM carrera c
                                                   where c.codigocarrera='".$_SESSION['codigofacultad']."'";
...
<?php   
  /* }else{
       echo "Este Usuario no tiene rol de Cordinador de Entrevistas";
   }*/
writeFooter();
        ?>  