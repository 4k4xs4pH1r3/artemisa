...
WHERE r.codigoestado=100 and pi.idobs_registro_riesgo not in (select idobs_registro_riesgo FROM obs_tutorias)
     ".$wp." ".$wm." ".$wf."
     ".$wd." ".$we." ".$ws."
     ".$wn." ".$wca." ".$wcf."
     ".$wcp." ".$woc." ".$wc."
...
group by e.numerodocumento order by r.fechacreacion desc";
    ...
    $query_carrera = "SELECT r.idobs_registro_riesgo, e.numerodocumento, nombresestudiantegeneral, apellidosestudiantegeneral,  
       nombredocente, apellidodocente, d.numerodocumento as numerodocumentodocente,
es.semestre, es.codigocarrera, c.nombrecarrera, m.nombremodalidadacademica, fa.nombrefacultad, n_tutoria,
f.nombrecausas as causas, t.nombretipocausas,  tr.nombretiporiesgo, observacionesregistro_riesgo, intervencionregistro_riesgo, rp.idobs_tutorias, 
rp.objetivotutoria, rp.estrategiatutoria, rp.recomendaciontutoria , rp.fechacreacion, rp.usuariocreacion
 FROM obs_tutorias as rp 
            ...
            ?>
                 <tr>
                     <td> 
                         ...
                         if ($tipo!='R'){
                            ?>
                              <button type="button" id="crear" name="crear" title="Crear Tutoria" onclick="updateForm3('form_tutorias.php?id_res=<?php echo $dt['idobs_registro_riesgo'] ?>&tipotutoria=1')"><img src="../img/agregar.png" width="20px" height="20px"  /></button>
                          
                         <?php
                         }else{
                             ?>
                            <button type="button" id="editar" name="editar" title="Editar" onclick="updateForm3('form_tutorias.php?id=row_<?php echo $dt['idobs_tutorias'] ?>&id_res=<?php echo $dt['idobs_registro_riesgo'] ?>&tipotutoria=1')"><img src="../img/editar.png" width="20px" height="20px"  /></button>
                            <!--<button type="button" id="eliminar" name="eliminar" title="Eliminar" onclick="deleteRegistro('tutorias','<?php echo $dt['idobs_form_tutorias'] ?>','listar_primera_instancia.php')"><img src="../img/eliminar.png" width="20px" height="20px"  /></button>-->
                         <?php
                          }
                         ?>
                     </td>
