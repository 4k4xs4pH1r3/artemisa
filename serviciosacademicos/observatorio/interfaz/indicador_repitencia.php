...
error_reporting(E_ALL);
   ...
   $db =writeHeader("Ind Rep<br> Acad&eacute;mica",true,"PAE",1);
   /* include("./menu.php");
    writeMenu(0);*/
   $tipo=$_REQUEST['tipo'];
    ...
    function Buscar(){
        ...
        if (modalidad==0){
            ...
            alert("Debe escoger la carrera");
                 ...
                 $("#codigocarrera").focus();
        /*}else if (materia==''){
            alert("Debe escoger la materia");
                 $('#codigomateria').css('border-color','#F00');
                 $('#codigomateria').effect("pulsate", {times:3}, 500);
                 $("#codigomateria").focus();*/
        }else if (periodo==''){
           ...
           $('#result').html('<blink>Cargando...</blink>');
            $.ajax({//Ajax
                      ...
                      data:({periodo:periodo, modalidad:modalidad, materia:materia,
                             nestudiante:nestudiante, carrera:carrera, tipo:'2' }),
