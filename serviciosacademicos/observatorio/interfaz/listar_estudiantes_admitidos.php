<?php
    ...
    $db =writeHeader("Entrevista",true,"Amidiones",1);
    $tipo=$_REQUEST['tipo'];
    ...
    $entity2 ->sql_where = "cedula_usuario= '".$n_doc."' and modulo='Entrevistador' ";
...
<script>
    ...
    {
    // Smart Tab
        //$('#tabs').smartTab({autoProgress: true,stopOnFocus:false,transitionEffect:'vSlide'});
	});
</script>
...
<div id="container" style="margin-left: 70px;">
    <div id="tabs">
        ...
        <div id="tabs-1">
            ...
            <tr>
                <td>
                    <label class="titulo_label"><b>Modalidad Acad&eacute;mica:</b></label>
                </td>
                <td>
                    <?php
            ...
            <tr>
                ...
                <td colspan="3">
                    <div  id="carreraAjax" style="display: none;"></div>
                </td>
            </tr>
            <?PHP
            }else 
            {
               ...
               if(!$_SESSION['codigofacultad'])
               {
                   $query_carrera='SELECT  c.nombrecarrera, c.codigocarrera
                                FROM 
                                grupo g INNER JOIN materia m ON m.codigomateria=g.codigomateria INNER JOIN carrera c ON c.codigocarrera=m.codigocarrera
                                WHERE
                                ...
                                UNION
                                SELECT c.nombrecarrera, c.codigocarrera
                                FROM 
                                Obs_AsignarCarrera  a INNER JOIN carrera c ON c.codigocarrera=a.id_Carrera INNER JOIN docente d ON d.iddocente=a.id_Docente
                                WHERE
               ...
               {
                   $query_carrera = "SELECT c.nombrecarrera, c.codigocarrera FROM carrera c where c.codigocarrera='".$_SESSION['codigofacultad']."' UNION SELECT 
                                        c.nombrecarrera,
                                        c.codigocarrera
                                        FROM 
                                        Obs_AsignarCarrera  a INNER JOIN carrera c ON c.codigocarrera=a.id_Carrera  INNER JOIN docente d ON d.iddocente=a.id_Docente
                                        ...
                                        a.codigoperiodo<='".$_SESSION['codigoperiodosesion']."' 
										AND
										(a.CodigoPeriodoFinal>='".$_SESSION['codigoperiodosesion']."' OR a.CodigoPeriodoFinal IS NULL) GROUP BY  c.codigocarrera";
                }
                // echo $query_carrera;
                $reg_programa =$db->Execute($query_carrera);           
                if($reg_programa->EOF)
                {                
                    $SQL_D='SELECT c.nombrecarrera, c.codigocarrera FROM 
                                        Obs_AsignarCarrera  a INNER JOIN carrera c ON c.codigocarrera=a.id_Carrera INNER JOIN docente d ON d.iddocente=a.id_Docente
                                        WHERE
                                        d.numerodocumento="'.$_SESSION['numerodocumento'].'"
                                        AND
