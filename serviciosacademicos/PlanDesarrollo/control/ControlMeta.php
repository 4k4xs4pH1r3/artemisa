	...
	class ControlMeta{
		...
		public function buscarMetaSecundariaId( $txtIdMetaSecundaria ) {
			...
			return $metaSecundaria;
                 ...
                 public function buscarMetaSecundariaFecha( $txtIdMeta, $fecha) {
			$metaSecundaria = new MetaSecundaria( $this->persistencia );
			return $metaSecundaria->consultarMetaSecundaria(  $txtIdMeta,0, $fecha ); 
		}
		...
		public function metaProyecto ( $codigoFacultad , $codigoCarrera , $idProyecto , $idIndicador , $idLinea , $idPrograma ) {
			...
			return $meta->verMetas( $codigoFacultad , $codigoCarrera , $idProyecto , $idIndicador , $idLinea , $idPrograma );
                ...
                public function metaProyectoAvance ( $codigoFacultad , $codigoCarrera , $idProyecto , $idIndicador , $idLinea , $idPrograma , $periodo ) {
			$meta = new Meta( $this->persistencia );
			return $meta->verMetasAvance( $codigoFacultad , $codigoCarrera , $idProyecto , $idIndicador , $idLinea , $idPrograma , $periodo );
		}
		
		/**
		 * Eliminar Meta Secundaria
	 	 * @param   int $idPersona, int $txtIdMeta
		 * @access public
		 * @return boolean
		 */
		public function eliminarMeta( $idPersona, $txtIdMeta ){
			$metaPrincipal = new Meta( $this->persistencia );
			$metaPrincipal->eliminaMetaPrincipal( $idPersona , $txtIdMeta );
			return $metaPrincipal;
		}
                
                public function  alcanceMetasSecundarias ( $metaPrincipal ){
                        $metaSecundaria = new MetaSecundaria( $this->persistencia );
                        $metaSecundaria->alcanceMetasSecundarias( $metaPrincipal );
                        return $metaSecundaria;
                } 
	}
?>