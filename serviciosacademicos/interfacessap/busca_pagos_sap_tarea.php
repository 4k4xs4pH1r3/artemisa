...
<form action="busca_pagos_sap.php" method="post">
...
<?php
 ...
 if (1)
  {
 $mesanterior=mes_anterior(date("m"),date("Y"));
 $mesanterior=mes_anterior($mesanterior["mes"],$mesanterior["anio"]);
 $mesanterior=mes_anterior($mesanterior["mes"],$mesanterior["anio"]);
 if($mesanterior["mes"]<10){
     $mesanterior["mes"]="0".$mesanterior["mes"];
 }
 $fechatresmesesatras=$mesanterior["anio"]."-".$mesanterior["mes"]."-01";
       ...
       and fechaordenpago between '".$fechatresmesesatras."' and '".date("Y-m-d")."'";
       //echo $query_ordenesm;
       //exit();
