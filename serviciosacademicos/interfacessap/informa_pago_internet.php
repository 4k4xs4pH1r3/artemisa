...
//$numeroordenpago = 1136315;
   ...
   $estadoconexionexterna = mysql_db_query($database_sala, $query_estadoconexionexterna) or die("ACA$query_estadoconexionexterna<br>".mysql_error());
   ...
   $row_estadoconexionexterna = mysql_fetch_array($estadoconexionexterna);
    ...
    $query_logpagos = "select l.FIName
    from LogPagos l
    where l.Reference1 = '$numeroordenpago'
    and StaCode = 'OK'";
    //and dop.codigoconcepto = '151'
   //echo "sdas $query_ordenes<br>";
   $logpagos = mysql_db_query($database_sala,$query_logpagos) or die("$query_logpagos<br>".mysql_error());
   $totalRows_logpagos = mysql_num_rows($logpagos);
   $row_logpagos = mysql_fetch_array($logpagos);
   if($totalRows_logpagos == '') {
       exit();
   }

    if($row_logpagos['FIName'] == "MASTERCARD" || $row_logpagos['FIName'] == "VISA")
    	$numerocuenta = "1110050615";
    else
        $numerocuenta = "1110050610";
	...
	//echo "$database_sala<br>Numero00000000000000000"   .$numeroordenpago."<br>";
	//CREA CUENTA CORREO
