...
class ManagerEntity {
    ...
    function __construct($entity) {
        ...
        if(file_exists(dirname(__FILE__)."/".$file) or file_exists(dirname(__FILE__)."\"".$file)){
            if(file_exists(dirname(__FILE__)."/".$file)){
                $classfile = dirname(__FILE__)."/".$file; 
                include $classfile;                
            }else{
                $classfile = dirname(__FILE__)."\"".$file;
                include $classfile;                  
...
//        } // foreach
    ...
    function deleteRecord (){
      ...
      $where  = rtrim($where, ' AND ');
      $query = "UPDATE $this->prefix$this->tablename SET codigoestado='200' WHERE $where";
      if ($this->debug == true){
                echo $query;
                exit();
            }
