...
if(isset($_REQUEST['naenviar']))
{
	...
	//print_r($_REQUEST);
	$year = substr($_SESSION["codigoperiodosesion"], 0,-1)-1;
        $filtrodocumento = "";
	...
	if($_REQUEST['nacodigomodalidadacademica'] != 0 && $_REQUEST['nacodigocarrera'] != 0)
	{
		...
		c.nombrecarrera, pr.semestreprematricula as semestre, 
		eg.emailestudiantegeneral, u.usuario, l.tmpclavelogcreacionusuario, est.nombreestado as estado
		from estudiante e, estudiantegeneral eg, carrera c, logcreacionusuario l, (select usuario,numerodocumento,idusuario,codigotipousuario,codigorol from usuario) u, prematricula pr, estudiantedocumento ed,
		estado est 
		...
		and l.codigoestado not like '3%'
		AND pr.codigoestudiante=e.codigoestudiante
		AND pr.codigoperiodo='".$_SESSION["codigoperiodosesion"]."'
		AND l.fechalogcreacionusuario>='".$year."-01-01 00:00:00'
		AND pr.codigoestadoprematricula in (10,11,40,41) 
		AND est.codigoestado=l.codigoestado  
		$filtrodocumento";
	}
...
<script language="JavaScript">
	...
	//echo substr($_SESSION["codigoperiodosesion"], 0,-1);
	$pager = new ADODB_Pager($db,$sqlini.$sqlfin);
	$pager->Filter($sqlini,$sqlfin,$array_campos,$linkadd);
	//$pager->totalsColumns = true;
	$pager->Render($rows_per_page,'width="100%" border="1" cellpadding="1" cellspacing="0" bordercolor="#E9E9E9"', array('ID General','Documento','Nombre','Carrera','Semestre', 'Correo', 'Usuario', 'Clave Temporal Asignada','Estado'));
?>
<br>
<input type="submit" value="Enviar" name="naenviar"><input type="button" value="Restablecer" onClick="window.location.href='listadoprimeraclave.php'">
<?php
