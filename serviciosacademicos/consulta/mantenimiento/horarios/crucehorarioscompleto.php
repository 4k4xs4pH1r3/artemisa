...
$query="select distinct e.codigoestudiante,eg.numerodocumento,eg.apellidosestudiantegeneral,
		eg.nombresestudiantegeneral,c.nombrecarrera from
		prematricula p,horario h,estudiante e ,detalleprematricula dp, grupo g,
		estudiantegeneral eg,carrera c
		where 
		...
		e.codigocarrera=c.codigocarrera and
                c.codigomodalidadacademica='200' and
		p.codigoperiodo='".$_GET['codigoperiodo']."' and
		eg.idestudiantegeneral=e.idestudiantegeneral 
		order by c.nombrecarrera,e.codigoestudiante";
$resultado=$objetobase->conexion->query($query);
echo "<table>";
echo "<tr><td><B>No</B></td><td><B>CARRERA</B></td><td><B>DOCUMENTO</B></td><td><B>APELLIDOS</B></td><td><B>NOMBRES</B></td>
...
//	echo $row['numerodocumento']."<br>";
	...
	echo  "<tr><td>".$i++."</td><td>".$row["nombrecarrera"]."</td><td>".$row['numerodocumento']."</td><td>".$row['apellidosestudiantegeneral'].
