...
<form name="aplicaarp" method="post" action="">
  ...
  <p align="center" class="Estilo3">VALORES PECUNIARIOS - MODIFICAR </p>
    ...
    <tr>
      <td><table width="100%" border="0" align="center" cellpadding="3" bordercolor="#FFFFFF">
        <tr>
          ...
          <td width="49%" bgcolor='#FEF7ED'>
              <?php echo $row_sel_modalidadacademica['nombremodalidadacademica'];?></td>
        </tr>
        <tr>
		...
		<tr>
          ...
          <td bgcolor="#CCDADD" class="Estilo2">
              <input name="masivo" type="hidden" value="no" >
			...
			order by c.nombrecarrera
					";
					//and c.codigocarrera in(select codigocarrera from facturavalorpecuniario where codigoperiodo='".$_GET['codigoperiodo']."')
...
?>
              ...
              <?php if(isset($_POST['masivo']) and $_POST['masivo']=='si' or isset($_POST['Enviar'])){ 
				...
				WHERE 
				c.fechainiciocarrera <= '".$fechahoy."' and c.fechavencimientocarrera >= '".$fechahoy."' and
				c.codigomodalidadacademica = '".$_GET['modalidadacademica']."' 
				
				order by c.nombrecarrera
				";
              	//AND c.codigocarrera IN (SELECT codigocarrera FROM facturavalorpecuniario)
  ...
  <br>
  <?php if (($totalRows_carrerapormodalidad<>0 or $totalRows_sel_codigocarrera<>0) and( isset($_POST['Enviar']) or isset($_POST['selcodigocarrera']) or isset($_POST['Enviar2']) or isset($_POST['codigotipoestudiante']))){ 	
            ...
            <td bgcolor="#CCDADD" class="Estilo2"><div align="center">Selecci&oacute;n:</div></td>
                    ...
                    /*if($_POST['masivo']=='no')*/ {
                    	while($concepto=mysql_fetch_assoc($mostrar_conceptos)){
							...
							<td class='Estilo1'><div align='center'><input type='checkbox'  name='concepto".$concepto['idvalorpecuniario']."'  $chequear value='".$concepto['idvalorpecuniario']."'></div></td>
							</tr>
							";
                    	}
                    }
...
if (isset($_POST['Grabar']) and  $_POST['masivo']=='no'){
	//unset($array_eliminar,$array_actualizar,$array_detallefvp,$idvalorpecuniario);
	$fechaidfacturavalorpecuniario=date("Y-m-d H:i:s");
	$query_busca_factura="select * from facturavalorpecuniario f where codigocarrera='".$_POST['selcodigocarrera']."' and codigoperiodo='".$_GET['codigoperiodo']."'"; //busca si hay algun valor en la tabla facturavalorpecuniario
	$busca_factura=mysql_query($query_busca_factura,$sala);
	$num_rows_buscafactura=mysql_num_rows($busca_factura);
	$factura=mysql_fetch_assoc($busca_factura);
	...
	if ($num_rows_buscafactura > 0)	{//echo "ya hay factura";
		...
		if ($row_verifica_chequeado != 0){
			...
			$query_selecciona_idfvp="select idfacturavalorpecuniario from facturavalorpecuniario f where codigocarrera='".$_POST['selcodigocarrera']."' and codigoperiodo='".$_GET['codigoperiodo']."'";
			...
			//crea un arreglo con los idvalorpecuniario de detallefacturavalorpecuniario
			...
			if($borrartodo)
			{
				$query_actualizar="update detallefacturavalorpecuniario SET codigoestado='200' where idfacturavalorpecuniario='".$factura['idfacturavalorpecuniario']."' and codigotipoestudiante='".$_POST['codigotipoestudiante']."'";
				$actualizartodo=mysql_query($query_actualizar) or die(mysql_error());
				if($actualizartodo){
			...
			if(isset($array_post) and isset($array_detallefvp)){
				...
				//echo "detallefvp",print_r($array_detallefvp);echo "<br>";
				//echo "arraypost",print_r($array_post);echo "<br>";
				//echo "<br>";
				//echo "actualizar",print_r($array_actualizar);echo "<br>";
				//echo "eliminar",print_r($array_eliminar);echo "<br>";


				$query_actualizar_detallefvp="select idfacturavalorpecuniario,iddetallefacturavalorpecuniario from detallefacturavalorpecuniario where idfacturavalorpecuniario ='".$idfvp['idfacturavalorpecuniario']."'
				and codigotipoestudiante = '".$_POST['codigotipoestudiante']."'
				and codigoestado ='100'";
