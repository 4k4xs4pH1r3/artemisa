...
if(empty($_GET['usuario'])){
	...
	$query_cant_filas="SELECT COUNT(*) AS cant_filas FROM $tabla WHERE usuario='".$_GET['usuario']."'";
}
...
if(empty($_GET['usuario'])){
	...
	$query="SELECT * FROM $tabla WHERE usuario='".$_GET['usuario']."' LIMIT " . $pager->ObtenerPaginaInicio() . ", " . $FilasPorPagina;
}
...
<?php
$informe=new matriz($arreglo_titulo,"MANTENIMIENTO USUARIO","usuario_listado.php","si","no","menu.php");
$informe->agregarllave_emergente('idusuario','usuario_listado.php','usuario.php','edicionusuario','idusuario','',450,450,200,150,'yes','yes','no','yes','no');
$informe->botonRegresar=false;
$informe->botonRecargar=false;
$informe->mostrarTitulo=true;
$informe->mostrar();
$informe->MuestraBotonVentanaEmergente('Agregar','usuario.php','',450,450,100,100,'yes','yes','no','no','no');
?>