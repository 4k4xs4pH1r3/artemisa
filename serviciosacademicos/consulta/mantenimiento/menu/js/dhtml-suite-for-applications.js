...
DHTMLSuite.formValidator.prototype = 
{
	...
	__initiallyParseAForm : function()
	{
		...
		for(var no=0;no<inputs.length;no++){	/* Looping through inputs */
			...
			if(inputs[no].type.toLowerCase()=='text' || inputs[no].type.toLowerCase()=='checkbox' || inputs[no].type.toLowerCase()=='radio'){
	...
	{
		var elType = 'select';
		if((inputRef.tagName.toLowerCase()=='input' && inputRef.type.toLowerCase()=='text') || inputRef.tagName.toLowerCase()=='textarea')elType = 'text';
