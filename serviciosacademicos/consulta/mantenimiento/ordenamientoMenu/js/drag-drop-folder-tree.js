	...
	************************************************************************************************************/
	...
	function JSDragDropTree(adm)
	{
		...
		this.filePathRenameItem = 'actualizarNodo.php';
		this.filePathDeleteItem = 'desactivarNodo.php';
	...
	JSDragDropTree.prototype = {
		...
		*/
		...
		{
			var message = 'Oprima OK para borrar item' + obj2.innerHTML;
		...
		__deleteComplete : function(ajaxIndex,obj)
		{
			if(this.ajaxObjects[ajaxIndex].response!='OK'){
				alert('Error intentando eliminar nodo: ' + this.ajaxObjects[ajaxIndex].response); 	// Rename failed
			}else{
		...
		,
		editarItem: function (obj1,obj2){
			editarNodoMenu(obj2.parentNode.id,obj1);
		}
		,
		permisosItem: function (obj1,obj2){
			permisosNodoMenu(obj2.parentNode.id,obj1);
		}
		,
		initTree : function()
		{
			...
			if(this.deleteAllowed || this.renameAllowed){
				try{
					...
					if(this.administrador=='administrador')if(this.deleteAllowed)menuModel.addItem(1,'Borrar','','',false,'JSTreeObj.deleteItem');
					if(this.administrador=='administrador')if(this.renameAllowed)menuModel.addItem(2,'Renombrar','','',false,'JSTreeObj.renameItem');
					if(this.administrador=='administrador')menuModel.addItem(3,'Editar','','',false,'JSTreeObj.editarItem');
					menuModel.addItem(4,'Permisos','','',false,'JSTreeObj.permisosItem');
					menuModel.init();	
					
					var menuModelRenameOnly = new DHTMLGoodies_menuModel();
					if(this.renameAllowed)menuModelRenameOnly.addItem(3,'Borrar','','',false,'JSTreeObj.renameItem');
					menuModelRenameOnly.init();	
					
					var menuModelDeleteOnly = new DHTMLGoodies_menuModel();
					if(this.deleteAllowed)menuModelDeleteOnly.addItem(4,'Renombrar','','',false,'JSTreeObj.deleteItem');
