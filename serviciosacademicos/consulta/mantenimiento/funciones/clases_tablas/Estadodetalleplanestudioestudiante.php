<?php
/**
 * Table Definition for estadodetalleplanestudioestudiante
 */
require_once 'DB/DataObject.php';

class DataObjects_Estadodetalleplanestudioestudiante extends DB_DataObject 
{
    ###START_AUTOCODE
    /* the code below is auto generated do not remove the above tag */

    public $__table = 'estadodetalleplanestudioestudiante';    // table name
    public $codigoestadodetalleplanestudioestudiante;    // string(3)  not_null primary_key
    public $nombreestadodetalleplanestudioestudiante;    // string(50)  not_null

    /* Static get */
    function staticGet($k,$v=NULL) { return DB_DataObject::staticGet('DataObjects_Estadodetalleplanestudioestudiante',$k,$v); }

    /* the code above is auto generated do not remove the tag below */
    ###END_AUTOCODE
}
