<?php 
function campotexto_valida_post_bd($nombrevar,$validacion,$mensaje,$tamano,$tablaexistente,$indicetablaexistente,$valorindicetablaexistente,$datotablaexistente)
{ 
	$$tablaexistente = DB_DataObject::factory($tablaexistente);
	//var_dump($indicetablaexistente);
	//var_dump($valorindicetablaexistente);
	$$tablaexistente->get($indicetablaexistente,$valorindicetablaexistente);
	//print_r($$tablaexistente);
	$arreglogrupomateria1=array();
	do {
		if ($$tablaexistente->$indicetablaexistente==$valorindicetablaexistente){
			$arreglogrupomateria1['nombregrupomateria'] = $$tablaexistente->$datotablaexistente;			
		}
	} while ($$tablaexistente->fetch());
?>

<input name="<?php echo $nombrevar;?>" type="text" id="<?php echo $nombrevar;?>" value="<?php echo $arreglogrupomateria1['nombregrupomateria'];?>" size="<?php echo $tamano;?>">
<?php 
...
//if(isset($_POST[$nombrevar])){
	switch ($validacion)
	{
		...
		case "fecha":
			...
			elseif(!ereg("^(2[0-9]{3})-([1-9]{1}|0[1-9]{1}|1[0-2]{1})-([1-9]{1}|[0-2]{1}[1-9]{1}|3[0-1]{1}|10|20)$", $_POST[$nombrevar], $regs))
			{
				$valido['mensaje']=$mensaje;
				$valido['valido'] = 0;
			}
