...
<input name="<?php echo $nombrevar;?>" id="<?php echo $nombrevar;?>" type="text" value="<?php echo $_GET[$nombrevar];?>">
<?php 
...
if(isset($_GET[$nombrevar])){
	switch ($validacion)
	{
		...
		case "hora":
			...
			break;
		case "numero":
			if(!ereg("^[0-9]{0,20}$",$_GET[$nombrevar]))
			{
				$valido['mensaje']=$mensaje;
				$valido['valido'] = 0;
			}
			break;
		case "letras":
			if(!ereg("^[a-zA-ZáéíóúüñÁÉÍÓÚÑÜ]*$",$_GET[$nombrevar]))
			{
				$valido['mensaje']=$mensaje;
				$valido['valido'] = 0;
			}
		...
		case "email":
			...
			break;
		case "nombre":
			if(!ereg("^([a-zA-ZáéíóúüñÁÉÍÓÚÑÜ]+([A-Za-záéíóúüñÁÉÍÓÚÑÜ]*|( [A-Za-záéíóúüñÁÉÍÓÚÑÜ]+)*))*$",$_GET[$nombrevar]))
			{
				$valido['mensaje']=$mensaje;
				$valido['valido'] = 0;
			}
		...
		case "fecha":
			// Para fechas >= a 2000
			//$regs = array();
			if(!ereg("^(2[0-9]{3})-([1-9]{1}|0[1-9]{1}|1[0-2]{1})-([1-9]{1}|[0-2]{1}[1-9]{1}|3[0-1]{1}|10|20)$", $_GET[$nombrevar], $regs))
			{
				$valido['mensaje']=$mensaje;
				$valido['valido'] = 0;
			}
		...
		if($_GET[$nombrevar] < $fechamenos60)
		{
			$valido['mensaje']=$mensaje;
			$valido['valido'] = 0;
		}
		...
		case "fechaant":
			// Para fechas < a 2000
			//$regs = array();
			if(!ereg("^(1[0-9]{3})-([1-9]{1}|0[1-9]{1}|1[0-2]{1})-([1-9]{1}|[0-2]{1}[1-9]{1}|3[0-1]{1}|10|20)$", $_GET[$nombrevar], $regs))
			{
				$valido['mensaje']=$mensaje;
				$valido['valido'] = 0;
			}
...
<textarea name="<?php echo $nombrevar;?>" cols="<?php echo $cols;?>" rows="<?php echo $rows;?>" wrap="VIRTUAL"><?php echo $_GET[$nombrevar];?></textarea>
<?php 
...
<table border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td><input name="<?php echo $nombrevar;?>" <?php echo $accion?> type="radio" value="<?php echo $$nombreobjeto->$dato;?>" <?php if($_GET[$nombrevar]==$$nombreobjeto->$dato){echo "checked";}?>></td>
  </tr>
