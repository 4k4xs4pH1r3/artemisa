...
<head>
	...
	************************************************************************************************************/
	...
	function ventanaAgregaEditaUsuario(){
		agregarUsuariosModelo = new DHTMLSuite.windowModel({windowsTheme:false,id:'agregarUsuarios',title:'Agregar Usuarios',xPos:50,yPos:50,minWidth:100,minHeight:100,width:400,height:300 } );//cookieName:'ventanaRevisionCasos'
...
while ( $item = $menu->fetch() ) {
	...
	if(!$administrador){
		echo ( "<li noDrag='true' id='".$item->id()."'><a href=\"".'#'."\">".$item->name()."</a></li>" );
	}
	else{
		echo ( "<li id='".$item->id()."'><a href=\"".'#'."\">".$item->name()."</a></li>" );
	}
}
?>
</ul>
	...
	<script type="text/javascript">	
	treeObj = new JSDragDropTree('<?php if($administrador){echo 'administrador';}?>');
