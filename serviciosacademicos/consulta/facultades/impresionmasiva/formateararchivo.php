...
$datosestudiante = $db->Execute($query_datosestudiante);
...
else
{
	...
	$selcodigo = $db->Execute($query_selcodigo);
	$totalRows_selcodigo = $selcodigo->RecordCount();
	if($totalRows_selcodigo != "")
	{
		// Espacio vertical de las materias
		$row_selcodigo = $selcodigo->FetchRow();
	...
	*/
	...
	$datosmaterias = $db->Execute($query_datosmaterias);
	$totalRows_datosmaterias = $datosmaterias->RecordCount();
	if($totalRows_datosmaterias != "")
	{
		// Espacio vertical de las materias
		$espaciovmaterias = 2.85;
		while($row_datosmaterias = $datosmaterias->FetchRow())
		{
	...
	{
		$espaciovmaterias = 2.85;
		fputs($archivo,"!R! CALL TITU,13,$espaciovmaterias, SEÑOR ESTUDIANTE USTED DEBE: ;EXIT;");
		$espaciovmaterias = $espaciovmaterias + 0.4;
	...
	$datosdetalles = $db->Execute($query_datosdetalles);
	$totalRows_datosdetalles = $datosdetalles->RecordCount();
	...
	else
	{
		...
		while($row_datosdetalles = $datosdetalles->FetchRow())
		{
			$tamañonumero = strlen($row_datosdetalles['valorconcepto']);
			switch($tamañonumero)
			{
				// Por numero 0.125
				// Por punto 0.1
		...
		$datosfechas = $db->Execute($query_datosfechas);
		$totalRows_datosfechas = $datosfechas->RecordCount();
		...
		else
		{
			...
			$espaciovfechas = 7.55;
			$cuentafechas = 1;
			while($row_datosfechas = $datosfechas->FetchRow())
			{
				switch($cuentafechas)
				{
					case "1":
				...
				$tamañonumero = strlen($row_datosfechas['valorfechaordenpago']);
				switch($tamañonumero)
				{
					// Por numero 0.125
					// Por punto 0.1
			...
			$tamañocodigo = strlen($row_datosestudiante['codigoestudiante']);
			if($tamañocodigo < 8)
			{
				$codigoreferencia = "";
			...
			while($row_datosfechas = $datosfechas->FetchRow())
			{
				...
				$tamañovalor = strlen($row_datosfechas['valorfechaordenpago']);
				//echo "<br>$tamañovalor<br>";
				if($tamañovalor < 8)
				{
