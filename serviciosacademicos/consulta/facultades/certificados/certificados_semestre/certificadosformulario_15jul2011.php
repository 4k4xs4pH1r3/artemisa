...
<form name="form1" method="post" action="">
	...
	and   codigoestado like '1%'";
        ...
        <tr>
            ...
            <?php
            if ($_SESSION['rol'] <> 1 and !isset($_GET['periodos'])) { // rol 1
                $articulo = '51';
                $notanumero = '3.0';
...
if($_GET['tipocertificado'] == "semestre") {
    ...
    while($row_materiashistorico = mysql_fetch_assoc($materiashistorico)) {
        ...
        if($materiapapaito = seleccionarequivalenciapapa($row_materiashistorico['codigomateria'],$codigoestudiante,$sala)) {
            ...
            /*if($row_mejornota['codigomateria'] == 7443)
			{
				print_r($row_mejornota);
				//exit();
			}*/
	      ...
	      and codigomateria like  '".$row_materiashistorico['semestre']."'";
            ...
            $Array_materiashistorico[$row_materiashistorico['codigomateria']] = $row_materiashistorico;
            //$Array_materiashistorico[$row_materiashistorico['semestre']] = $row_semestremateria['semestredetalleplanestudio'];
        }
    ...
    //print_r($Array_materiashistoricoinicial);
    ...
    /*  foreach($Array_materiasperiodo as $codigoperiodo => $Array_materia)
	{
		...
		foreach($Array_materia as $key => $row_materiaperiodo)
		{
          ...
          $totalRows_semestremateria  = mysql_num_rows($semestremateria );
	      $row_semestremateria  = mysql_fetch_assoc($semestremateria );
		  //echo $query_semestremateria,"<br>";
		  //$Array_materiasperiodo[$row_materia['codigoperiodo']][$row_materia['semestre']] = $row_semestremateria['semestredetalleplanestudio'];

			//echo "".$row_materiaperiodo['codigomateria']." => ".$row_materiaperiodo['nombremateria']."<br>";
		}
	}  	 */
    /* echo "<pre>";
	print_r($Array_materiasperiodo);
	echo "</pre>";  */
    //exit();
