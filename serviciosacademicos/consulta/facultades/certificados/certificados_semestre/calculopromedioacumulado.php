...
if(!$solicitud_promedioacumulado)
{
  ...
  if ($_GET['tipocertificado'] == "pasadas")  // Codogo incluido por orden de Secretaria General 12-03-2007 E.G.R
   {
...
do
{
   if($conplandeestudio)
   {
		...
		{	
       		...
       		if($solicitud_promedioacumulado1 <> "")
	    	{
				if($solicitud_promedioacumulado1['notadefinitiva'] > 5)
			 	{		   
			  		$notamayor =  round(($solicitud_promedioacumulado1['notadefinitiva'] / 100),2);	       
			 	}
				else
			 	{
	...
	else
	{
		...
		{		   
			$notamayor =  round(($solicitud_promedioacumulado['notadefinitiva'] / 100),2);	       
		}
		else
		{
...
//if($creditos != "")
//{
	$promedioacumulado = (round($notatotal/$creditos,2));
//}
unset($Arregloequivalencias);
unset($notamateria);
