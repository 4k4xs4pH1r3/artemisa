...
if(isset($_GET['codigo'])) $_SESSION['codigo'] = $_GET['codigo'];
...
<form name="form1" method="post" action="">
	...
	AND c.codigotitulo = ti.codigotitulo";
        ...
        if($solicitud_historico <> "") {// if 1            
            if ($_SESSION['rol'] == 1 or isset($_GET['periodos'])) { 
                ?>
    <p class="Estilo4"><strong>Estudiante:</strong> <?echo $solicitud_historico['nombresestudiantegeneral'],"&nbsp;",$solicitud_historico['apellidosestudiantegeneral']; ?><br>
        <strong>Documento:</strong> <?=$solicitud_historico['numerodocumento']; ?></p>
    ...
    <table width="80%"  border="0" align="center" cellpadding="0">                
        <tr>
            <td colspan="7"><div align="center" class="Estilo1 Estilo2">
                    ...
                    <span class="Estilo4"><strong><a class="Estilo4"style="cursor: pointer" onClick="window.location.href='../../prematricula/matriculaautomaticaordenmatricula.php?programausadopor=facultad'">
                                EL SUSCRITO<br>
                                            ...
                                            if(preg_match("/ESPECIALIZAC|MAESTRIA|DOCTORADO/",$solicitud_historico['nombrecortocarrera'])) {
                                                echo "DIRECTOR DE PROGRAMA<br>";
                                                if(preg_match("/DOCTORADO/",$solicitud_historico['nombrecortocarrera'])) {
                                                    echo "DEL ".$solicitud_historico['nombrecortocarrera']."";
                                                } else {                                                   
                                                    echo "DE LA ".$solicitud_historico['nombrecortocarrera']."";
                                                }
                                            } else {
                                                echo "SECRETARIO ACADÉMICO<br>DE LA FACULTAD DE ".$solicitud_historico['nombrecortocarrera']."";
                                            }
                                            ?>
                            </a></strong></span><br>
                    ...
                    <p align="justify">Que <strong><? echo $solicitud_historico['nombresestudiantegeneral'],"&nbsp;",$solicitud_historico['apellidosestudiantegeneral']; ?></strong>, identificado(a) con <?=$solicitud_historico['nombredocumento']; ?> No. <?= $solicitud_historico['numerodocumento']; ?> de <?=$solicitud_historico['expedidodocumento']; ?><!-- , con c&oacute;digo estudiantil No. <?php //echo $solicitud_historico['codigoestudiante']; ?> -->, curs&oacute;
                                    <?
                                }                                
                                if ($solicitud_historico['codigosituacioncarreraestudiante'] == 400 and $row_tipousuario['codigotipousuariofacultad'] == 200 and !isset($_GET['periodos'])) {
                                    ?>
		    y aprob&oacute; las asignaturas correspondientes al programa de <?=$solicitud_historico['nombrecortocarrera']; ?>, y obtuvo el t&iacute;tulo de <strong><?=$solicitud_historico['nombretitulo']; ?></strong>.</p>
                                <?
                            }
                            else
                            if ($solicitud_historico['codigosituacioncarreraestudiante'] <> 400 and $_SESSION['rol'] <> 1 and !isset($_GET['periodos'])) {
                                ?>
		 las asignaturas abajo relacionadas correspondientes al programa de <?=$solicitud_historico['nombrecortocarrera']; ?>.
                                    <?
                                }
                                if ($_SESSION['rol'] <> 1 and !isset($_GET['periodos'])) {
                                    ?>
        ...
        <tr>
            <td colspan="7"><span class="Estilo1 Estilo2"><strong><? echo $solicitud_historico['nombreperiodo'];?></strong></span></td>
        </tr>
                                <?
                            }
					...
					order by 2, 1";
                            ...
                            ?>
        <tr>
            <td valign="top"><span class="Estilo1 Estilo2"><? echo $solicitud_historico['codigomateria'];?></span></td>
            <td><span class="Estilo1 Estilo2"><? if ($mostrarpapa <> "") echo "$mostrarpapa /  ";
                    echo $solicitud_historico['nombremateria'].$lugaresderotacion;?>
                    <?
                    if ($row_detallemateria <> "") {
                        echo '<table width="100%" border="0" align="left" cellpadding="0">';
                        do {
                        echo "<tr>".'<td class="Estilo10">- '.$row_detallemateria['descripciontemasdetalleplanestudio'].'</td>'."</tr>";
        ...
        <tr>
            ...
            <?if($_GET['concredito'] == 1) { ?>
                    <td><div align="center"><span class="Estilo2 Estilo1"><strong><?=$sumaulas;?></strong></span></div></td>
                    <td><div align="center"><span class="Estilo2 Estilo1"><strong><?= $sumacreditos;?></strong></span></div></td>
             <?} ?>
            <td><div align="center"><span class="Estilo2 Estilo1"><strong>
            <?
            if ($promediosemestralperiodo > 5) $promediosemestralperiodo = number_format(($promediosemestralperiodo / 100),1);
    ...
    ?>
        ...
        <tr>
                ...
                if($carreraestudiante == 10) {
                    echo  '<td colspan="2"><span class="Estilo2 Estilo1"><strong>Promedio Ponderado Acumulado a '.$nombreultimoperiodo.'</strong></span></td>';
                }else {
                    echo '<td colspan="2"><br><span class="Estilo2 Estilo1"><strong>Promedio Ponderado Acumulado </strong></span></td>';
                }
                if($_GET['concredito'] == 1) {        
                    echo "<td>&nbsp;</td><td>&nbsp;</td>";
                }
    ...
    ?>
  	...
  	De conformidad con lo dispuesto   <?=$articulo; ?> , la escala de calificaciones que aplica la Institución est&aacute; dentro de un rango de cero punto cero (0.0) a cinco punto cero (5.0) y la calificación m&iacute;nima aprobatoria es de <?php echo "$notaletra ($notanumero)"; ?> sobre cinco punto cero (5.0).</p>
                <p align="justify" class="Estilo1 Estilo2">La presente certificaci&oacute;n se expide a solicitud del interesado(a), en Bogot&aacute; D.C., a los
                            ...
                            require('convertirnumeros.php');
                            echo $dias;
                            ?> (<?=$dia;?>) d&iacute;as del mes de <?=$meses;?> (<?=$mes;?>) del a&ntilde;o dos mil <?php $day = substr($ano,2,4);
                            require('convertirnumeros.php');
        echo $dias; ?> (<?=$ano;?>).</p>
            ...
            ?>
                        <td>
                            <p class="Estilo1 Estilo2"><strong><a class="Estilo4"style='cursor: pointer' onClick='JavaScript:window.print()' ><?echo $row_responsable['nombresdirectivo'],"&nbsp;",$row_responsable['apellidosdirectivo'];?></a><br>
                            <?=$row_responsable['cargodirectivo']; ?></strong></td>
                            <?
...
if($_GET['tipocertificado'] == "reglamento") {
    if(isset($_GET['periodos'])) {
        $query_selperiodos = "SELECT distinct codigoperiodo from notahistorico  where codigoestudiante = '$codigoestudiante' order by 1";
