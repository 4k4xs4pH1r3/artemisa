...
if(isset($_GET['documento'])){
	...
	$query_buscadiploma="select * from documentograduado where idregistrograduado='".$_GET['idregistrograduado']."' and codigoestado='100' and codigotipodocumentograduado='1'";
	$buscadiploma=mysql_query($query_buscadiploma,$sala) or die(mysql_error());
	$totalrows_buscadiploma=mysql_num_rows($buscadiploma);
	$row_buscadiploma=mysql_fetch_assoc($buscadiploma);
	//echo $totalrows_buscadiploma;
      ...
      <td><table border="0" align="center" cellpadding="0" bordercolor="#003333">
          <tr class="Estilo2">
            ...
            while($row_seliddirectivo = mysql_fetch_assoc($seliddirectivo)){
            	...
            	$chequear="";
            	$query_verifica_chequeado="select * from documentograduado d where d.idregistrograduado='$idregistrograduado' and d.codigotipodocumentograduado='1' and iddirectivo='".$row_seliddirectivo['iddirectivo']."' and codigoestado='100'";
...
{
	if($totalrows_buscadiploma==0){
		$_SESSION["accion"]='Enviar';
		$_SESSION["diploma"]=$_POST;
		...
		if(isset($arraypost) and isset($arraybd))
		{
			...
			foreach ($array_eliminar as $eliminacion => $valeliminar){
				...
				$query_anular_documentograduado_diploma="update documentograduado set codigoestado='200' where idregistrograduado='$idregistrograduado' and iddirectivo='$iddirectivo' and codigotipodocumentograduado='1'";
				$anular_documentograduado_diploma=mysql_query($query_anular_documentograduado_diploma,$sala);
				$query_log_anular_documentograduado_diploma="insert into logdocumentograduado values ('','".$row_buscadiploma['iddocumentograduado']."','1','$iddirectivo','200','1','1')";
				echo $query_log_anular_documentograduado_diploma;
				$log_anular_documentograduado_diploma=mysql_query($query_log_anular_documentograduado_diploma,$sala);
			}
			if($anular_documentograduado_diploma or $log_anular_documentograduado_diploma){echo '<script language="javascript">window.close();</script>';echo '<script language="javascript">window.opener.recargar();</script>';} 

			foreach ($array_actualizar as $actualizacion => $valactualizar){
				$iddirectivo = $array_actualizar[$actualizacion];
				//echo "insert",$iddirectivo;
				$query_insertar_documentograduado_diploma="insert into documentograduado values ('','$idregistrograduado','1','$iddirectivo','100','1','1')";
				...
				$query_log_insertar_documentograduado_diploma="insert into logdocumentograduado values ('','$iddocumentograduado','1','$iddirectivo','100','1','1')";
				$query_log_insertar_documentograduado_diploma;
				$log_insertar_documentograduado_diploma=mysql_query($query_log_insertar_documentograduado_diploma,$sala);
			}
			if($insertar_documentograduado_diploma and $log_insertar_documentograduado_diploma ){echo '<script language="javascript">window.close();</script>';echo '<script language="javascript">window.opener.recargar();</script>';}
		}
		elseif(@$arraypost==0)
		{
			$query_anular_documentograduado_diploma="update documentograduado set codigoestado='200' where idregistrograduado='$idregistrograduado' and codigotipodocumentograduado='1'";
			$anular_documentograduado_diploma=mysql_query($query_anular_documentograduado_diploma,$sala);
			if($anular_documentograduado_diploma){echo '<script language="javascript">window.close();</script>';echo '<script language="javascript">window.opener.recargar();</script>';}
		}
	}
}
?>
