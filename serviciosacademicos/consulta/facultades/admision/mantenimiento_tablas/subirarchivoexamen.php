...
//datos del arhivo
$nombre_archivo = $file['userfile']['name'];
$tipo_archivo = $file['userfile']['type'];
$tamano_archivo = $file['userfile']['size'];
...
if("xls"!=$extension[1])
{
	...
	alert("El archivo debe ser excel '.$tipo_archivo.' ");
	document.location.href="menu_subirexamen.php?codigocarrera='.$_POST['codigocarrera'].'";
	</script>';
	exit();
...
else
{
	...
	if(move_uploaded_file($file['userfile']['tmp_name'], "/tmp/examenadmision.xls"))
	{
...
if($archivo_cargado_ok==true)
{
	...
	$dataexcel->read('/tmp/examenadmision.xls');
	...
	foreach ($array_carga as $llave => $valor)
	{
		...
		$array_datos_estudiante[]=$admisiones->ObtenerDatosEstudiantePartiendoDesdeDocumento($valor['numerodocumento'],$codigocarrera, $codigoperiodo);
	}
	if($depurar==true)
	{
	...
	{
		$array_detalleestudianteadmision=$admisiones->LeerSiHaySalonPreviamenteAsignado($valor['codigoestudiante'],$codigocarrera,$idsubperiodo);
		$array_admision[]=$array_detalleestudianteadmision;
	}
	if($depurar==true)
	{
	 ...
	 */
	$array_datos_actualizados=$admisiones->ActualizarDatosPruebasExamenAdmision($array_admision,$array_carga);
	if($depurar==true)
	{
...
if($todosValidos==false)
{
	...
	echo'<meta http-equiv="REFRESH" content="0;URL=log_subir_examen.php"/>';
}
else
{
	$_SESSION['log_subir_examen']=$suma_C;
