...
while ($planestudio->cantidadsemestresplanestudio >= $cuentasemestre) :
	$query_cojematerias = "select d.codigomateria, m.nombremateria, m.numerohorassemanales, ".
	...
	while($row_cojematerias = $cojematerias->FetchRow()){
		if($row_cojematerias['codigotipomateria'] != 5){
			$detalleplanestudio[$cuentasemestre][] = new detalleplanestudio($planestudio->idplanestudio,
                $row_cojematerias['codigomateria']);
			$maximonuevo++;
		}else{
			$query_materiahija = "SELECT d.idlineaenfasisplanestudio, ".
            " d.codigomateriadetallelineaenfasisplanestudio as codigomateria, ".
			...
			if($totalRows_materiahija != ""){
				while($row_materiahija = $materiahija->FetchRow()){
					$detalleplanestudio[$cuentasemestre][] = new detalleplanestudio($planestudio->idplanestudio,
                        $row_materiahija['codigomateria'], $row_materiahija['idlineaenfasisplanestudio']);
					$maximonuevo++;
				}
			}else{
				$detalleplanestudio[$cuentasemestre][] = new detalleplanestudio($planestudio->idplanestudio,
...
if(isset($detalleplanestudio[$cuentasemestre][$cuentafilas - 1])){
    ...
    if(isset($_SESSION['codigoperiodosesion'])) {
        $mensajemateria = getHorario($detalleplanestudio[$cuentasemestre][$cuentafilas - 1]->codigomateria,
            $_SESSION['codigoperiodosesion'], $_SESSION['codigo']);
    }

    $detalleplanestudio[$cuentasemestre][$cuentafilas - 1]->mostrarDetallePlanEstudioEstudiante($materiasfinal, $conimagen);
    if(isset($nota) && !empty($nota) && isset($credito) && !empty($credito)){
        if(!isset($creditos[$cuentasemestre]) || empty($creditos[$cuentasemestre])){
            $creditos[$cuentasemestre] = "0";
        }
...
<?php
	...
	if($creditos[$cuentasemestre] != 0)
	{
		...
		if(isset($acumulanotas) && !empty($acumulanotas)){
...
<td colspan="3">
    <table border="1" cellpadding="1" cellspacing="0" style="font-size: 12px;" height="30px" width="60%" align="center">
        <tr>
          ...
          <td width="20%" align="center"><?php echo ($creditosVistos==0)?$creditosVistos:"<a href='materiaselectivasplanestudioestudiante.php?cod_estudiante=".
                  $_REQUEST['codigoestudiante']."&solomaterias=true' target='_blank' onClick=\"window.open(this.href, this.target, 'width=800,height=300'); return false;\">".
                  $creditosVistos."</a>"?></td>
	</tr>
        <tr>
