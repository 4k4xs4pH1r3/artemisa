...
if(isset($fechavencimiento) && !empty($fechavencimiento)){
    if(isset($datevencimiento) && $datevencimiento == "0000-00-00 00:00:00"){
        $datevencimiento = "2099-12-31";
    }else{
...
?>
<h2>SELECCION DE COREQUISITOS SENCILLOS </h2>
...
<table width="780" border="1" cellpadding="2" cellspacing="1" bordercolor="#003333">
  ...
  <tr>
	<td align="center">
        <input type="text" name="finiciocorequisito" placeholder="aaaa-mm-dd" value="<?php echo $fechainicio; ?>"
               size="10" onBlur="iniciarinicio(this)" onFocus="limpiarinicio(this)">
	  <font size="1" face="Tahoma"><font color="#FF0000">
	  <?php echo $msgfechainicio; ?>
        </font></font></td>
	<td align="center" colspan="2">
        <input type="text" name="fvencimientocorequisito" placeholder="2999-12-31" value="<?php echo $fechavencimiento;?>"
           size="10" onBlur="iniciarvencimiento(this)" onFocus="limpiarvencimiento(this)">
	  <font size="1" face="Tahoma"><font color="#FF0000">
...
<table width="780" border="1" cellspacing='0' bordercolor='#D76B00'>
    ...
    for($filamateria=1; $filamateria<=($row_cuentamateriassemestre['conteo']+$row_cuentamateriassemestre2['semestredetallelineaenfasisplanestudio']);
        $filamateria++){
        ?>
        <tr>
            <?php
            for($columnamateria=1; $columnamateria<=$row_planestudio['cantidadsemestresplanestudio']; $columnamateria++){
                if(isset($semestre[$columnamateria][$cuentamateria[$columnamateria]])) {
                    ...
                    // Mira si la materia papa  de la materia inicial es la misma que se va a mostrar
                    if ($codmateria != $Varcodigodemateria) {
                        // Mira si la materia anterior o inicial, es decir a la que se quiere ver la información
                        if ($codmateria != $Varcodigodemateriaanterior) {
                            if ($columnamateria == $limite) {
                                // Si la materia a ver informacion y la que se quiere mostrar tienen corequisito doble no la muestra
                                ...
                                " and codigomateriareferenciaplanestudio = '$codmateria' ".
                                " and idlineaenfasisplanestudio = '$idlineaenfasis' ".
                                " and codigotiporeferenciaplanestudio = '201'";
                                $selcorequisitos = $db->GetRow($query_selcorequisitos);
                                ...
                                <td width="<?php echo (780 / $row_planestudio['cantidadsemestresplanestudio']) - 2; ?>"
                                    ...
                                    if (isset($codmateria) && !empty($codmateria)) {
                                        if ($estaEnenfasis == "si") {
                                            ?>
                                            <input type="checkbox" name="<?php echo "cosencillo$codmateria"; ?>"
    ...
    ?>
      <tr>
        <td align="center" colspan="<?php echo $row_planestudio['cantidadsemestresplanestudio'];?>">
            <input type="submit" name="aceptarcorequisitosencillo" value="Aceptar">
    <?php
    if($estaEnenfasis == "no"){
        ?>
        ...
        ?>
        <input type="button" name="regresar" value="Regresar" onClick="window.location.href='materiaslineadeenfasisporsemestre.php?planestudio=<?php echo "$idplanestudio&lineaenfasis=$idlineaenfasis";?>'">
        <?php
    }
    ?>
	</td>
