...
do { 
    ...
    /////////////////////////////////////////////////////////////
    /***********Ajuste nuevo***/
    $codigomateria = $solicitud_creditos['codigomateria'];
    ...
    if ($solicitud_historico <> "") {
        do {
             /*echo '<br>notadefinitiva->'.$solicitud_historico['notadefinitiva'].' <  notaminimaaprobatoria->'.$solicitud_historico['notaminimaaprobatoria'];
             echo '<br>notadefinitiva->'.$solicitud_historico['notadefinitiva'].' >=  notaminimaaprobatoria->'.$solicitud_historico['notaminimaaprobatoria'];
             */
            if ($solicitud_historico['notadefinitiva'] < $solicitud_historico['notaminimaaprobatoria']) {
               ...
               // echo '<pre>';print_r($Arregloequivalencias);
                ...
                foreach($Arregloequivalencias as $key3 => $selEquivalencias) { // foreach
                    if($selEquivalencias <> "") {
                        $query_historico = "SELECT n.notadefinitiva,m.notaminimaaprobatoria
                                            FROM notahistorico n,materia m
                                            WHERE n.codigoestudiante = '".$codigoestudiante."'
                    ...
                    if ($solicitud_historico <> "") {
                        ...
                        // echo '<br><br>--_>'.$cuentamateriaperdida,"perdida<br>";
                        if ($cuentamateriaperdida > 1) {
                            $indicadormateriasperdidas = 1;
                        }
                    }
            
                 //echo "<br><br>indicadormateriasperdidas->".$indicadormateriasperdidas;
                } // foreach*/
        ...
        while($solicitud_historico = mysql_fetch_assoc($res_historico));
        // echo '<br><br>Numero de veces Perdida->'.$cuentamateriaperdida,"perdida<br>";
        if ($cuentamateriaperdida > 1) {
            $indicadormateriasperdidas = 1;
        }
    }
...
//echo '<br>$creditosperdidos->'.$creditosperdidos.' >= $calculoperdida->'.$calculoperdida.' <br> or  $indicadormateriasperdidas->'.$indicadormateriasperdidas.' == 1 <br>  or $promedioacumulado->'.$promedioacumulado.' < 3.3 and '.$row_historicosituacion['codigosituacioncarreraestudiante'].' == 200';
							...
							if (($creditosperdidos >= $calculoperdida) or ($indicadormateriasperdidas == 1) or ($promedioacumulado < 3.3 and $row_historicosituacion['codigosituacioncarreraestudiante'] == 200) ){
                                /*
                                *** codigosituacioncarreraestudiante = 200 --> prueba academica y $promedioacumulado < 3.3 si el promedio es menor a 3.3
                                *** 
                                *** $creditosperdidos >= $calculoperdida --> si los creditos perdidos son menores o iguales a el 50% del total matriculados
                                
                                */
                                if($texto){
                                    $situacionAcademicaNueva = '';    
                                   $situacionAcademicaNueva = $cambiosituacion;
                                }else{
							 ...
							 else if ($promediosemestralperiodo < 3.3 or $promedioacumulado < 3.3)
							 {  
							    $cambiosituacion = 200;
