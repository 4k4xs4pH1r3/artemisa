...
//$this->encuentra_estudiante_orden($numeroorden);
	if($this->encuentra_estudiante_orden($numeroorden)){
		if(!($usuariodatos = $this->objetobase->recuperar_datos_tabla("usuario u","u.numerodocumento",$row["numerodocumento"],"","",0))){
	...
	if($datos_documento=$this->objetobase->recuperar_datos_tabla($tablas,"u.usuario",$nuevousuario,$condicion,"",0)||(@$datos_ldapusuario["count"]>0))
	{
		...
		return false;
 		...
 		/*$mensaje="<b>BIENVENIDO A LA UNIVERSIDAD EL BOSQUE</b><BR><BR>".
			"Adjunto al presente usuario y clave para el ingreso al correo institucional y como usuario de servicios académicos.\n\n
		...
		$this->clave=$clave;
		$this->insertarnuevousuario($nuevousuario);
