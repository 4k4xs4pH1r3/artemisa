...
require_once('conexion_sala.php');
...
while($row_datos = $datos->FetchRow()){
    if($row_datos['codigocarrera']!=10){
        ...
        $pdf->SetY(115);
        $pdf->SetX(45);
        $pdf->CellFit(330,0,$row_datos['nombre'],0,0,'C','','',true,false);
	...
	if(ctype_digit($row_datos['documento']))
        ...
        $pdf->Cell(0,0, str_replace($search, $replace, strtoupper($row_datos['nombredocumento'])).' No. '.$documento.' EXPEDIDA EN '.str_replace($search, $replace, strtoupper($row_datos['expedicion'])),0,2,'C');
        ...
        $pdf->CellFit(360,0, str_replace($search, $replace, strtoupper($row_datos['titulo'])),0,0,'C','','',true,false);
        }

        $pdf->SetFont('Humanst521BTRoman','',12);
        $pdf->SetY(197);
        $pdf->Cell(0,0, str_replace($search, $replace, strtoupper('DADO EN BOGOTÁ D.C., REPÚBLICA DE COLOMBIA, EL DÍA ')).$row_datos['fechagrado'],0,2,'C');
