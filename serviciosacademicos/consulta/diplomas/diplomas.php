...
if(isset($_REQUEST['tipogeneracion']) && $_REQUEST['tipogeneracion']=="generacionmasiva"){
 ...
 $query_datos = "SELECT e.codigoestudiante, e.codigocarrera, rg.idregistrograduado, rg.codigoestado,
                ...
                eg.codigogenero, eg.tipodocumento
...
while($row_datos = $datos->FetchRow()){
    //if($row_datos['codigocarrera']!=10){
		...
		/*Validar si el documento incia en 0*/
		$cadenaDocu=$row_datos['documento'];
		for($i=0;$i<count($cadenaDocu);$i++){
			
			if($cadenaDocu[0] === "0"){
				$extranDoc = $cadenaDocu[0];
			}
		} 
	if(ctype_digit($row_datos['documento']))
        {
	    if($row_datos['tipodocumento']!='03')
	    {
			if($extranDoc != '0'){
...
setlocale(LC_CTYPE,"es_ES");
                 ...
                 * Se agrega una funcion trim al dato $row_datos['expedicion'] para eliminar todos los posibles espacios en blanco
                 * que afecten el correcto display de la informacion a imprimir
                 * @since marzo 22, 2017
                */
		if(strtoupper(utf8_decode($row_datos['nombredocumento']))=="PASAPORTE"){
			$pdf->Cell(0,0, str_replace($search, $replace, strtoupper(utf8_decode($row_datos['nombredocumento']))).' No. '.$documento.' EXPEDIDO EN '.str_replace($search, $replace, strtoupper(utf8_decode(trim($row_datos['expedicion'])))),0,2,'C');
		} else {
			$pdf->Cell(0,0, str_replace($search, $replace, strtoupper(utf8_decode($row_datos['nombredocumento']))).' No. '.$documento.' EXPEDIDA EN '.str_replace($search, $replace, strtoupper(utf8_decode(trim($row_datos['expedicion'])))),0,2,'C');
		}
                /* Fin Modificacion */
        $pdf->SetFont('TrajanPro','',50);
        $pdf->SetY(177);
	 ...
	 * Se agrego el ":" despues de Elaboración Duplicado y se agrego la tilde a elaboración
	...
	*/
	...
	$pdf->Cell(0,0, str_replace($search, $replace, strtoupper(utf8_decode('DADO EN BOGOTÁ D.C., REPÚBLICA DE COLOMBIA, EL DÍA '))).$row_datos['fechagrado'].'. '.utf8_decode('ELABORACIÓN').' DUPLICADO: '.strtoupper($mesesanio[date('n')-1]).' '.date('d').' DE '.date('Y').'.',0,2,'C');
	}
