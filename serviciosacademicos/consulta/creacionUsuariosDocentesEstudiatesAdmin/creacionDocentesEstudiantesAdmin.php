...
function validacionExisteUsuarioDocEstAdm($nombres,$apellidos,$numerodocumento,$espracticante,$db) {    
	...
	if($cont>0) {
		$reg=$row->FetchRow();
                verificaconSalaInterno($reg['tipodocumento'],$nombres,$apellidos,$numerodocumento,$reg['usuario'],$reg['idusuario'],$db);
...
function creacionSALAUsuarioDocEstAdm($tipodocumento,$nombres,$apellidos,$numerodocumento,$usuario,$espracticante,$db) {
    ...
    $sqlUsuarioTipo = "insert into UsuarioTipo (CodigoTipoUsuario, UsuarioId, CodigoEstado) value ('".$codtipousuario."', '".$idusuarioinsert."', '100' );";
    $insertar = $db->Execute($sqlUsuarioTipo);
}

/*
 * @modified Vega Gabriel <vegagabriel@unbosque.edu.do>.
 * Se agrega esta funcion que valida si existen o no registros asociados en otras tablas de SALA,
 * es decir, en el momento de haber sido creado el docente no haya almacenado la informacion respectiva.
 * @since Octubre 2, 2018
 */
function verificaconSalaInterno($tipodocumento,$nombres,$apellidos,$numerodocumento,$usuario,$idusuario,$db) {
	require_once(dirname(__FILE__).'/../../Connections/sala2.php');
	$rutaado = dirname(__FILE__)."/../../funciones/adodb/";
	require_once(dirname(__FILE__).'/../../Connections/salaado.php');
    $querys="select * from permisousuariomenuopcion where idusuario='".$idusuario."' ";
	$row=$db->Execute($querys);
	$cont=$row->RecordCount();
	if($cont==0) {
            $query="insert into permisousuariomenuopcion (idpermisomenuopcion,idusuario,codigoestado) values (251,".$idusuario.",'100')";
            $db->Execute($query);
	}
    $queryt="select * from docente where numerodocumento='".$numerodocumento."' ";
	$rowt=$db->Execute($queryt);
	$contar=$rowt->RecordCount();
	if($contar==0) {
            $query="INSERT INTO `docente` (`codigodocente`, `apellidodocente`, `nombredocente`, `tipodocumento`, `numerodocumento`, 
            `clavedocente`, `emaildocente`, `codigogenero`, `fechanacimientodocente`) 
            VALUES ('".$numerodocumento."', '".$apellidos."', '".$nombres."', '".$tipodocumento."', '".$numerodocumento."', '0', '".$usuario."@unbosque.edu.co', '100', '0000-00-00')";
    ...
    $queryr="select * from UsuarioTipo where UsuarioId='".$idusuario."' ";
	$rowt=$db->Execute($queryr);
	$conta=$rowt->RecordCount();
	if($conta==0) {
            $sqlUsuarioTipo = "insert into UsuarioTipo (CodigoTipoUsuario, UsuarioId, CodigoEstado) value ('".$codtipousuario."', '".$idusuario."', '100' );";
            $insertar = $db->Execute($sqlUsuarioTipo);
	}       
    
}

//funcion para la creacion de usuario y claves en ldap
...
function generacionUsuarioDocEstAdm($tipodocumento,$numerodocumento,$nombres,$apellidos,$email,$espracticante,$db,$admindocente=null) {    
	...
	//generacion de la clave para el usuario
	...
	if($retorno && $siguecreacionusuariogoogle) {
		...
		if($admindocente){
			$query="UPDATE administrativosdocentes SET EmailInstitucional='".$nuevousuario."@unbosque.edu.co' WHERE (numerodocumento='".$numerodocumento."' AND EmailInstitucional IS NULL)";
			$row=$db->Execute($query);
		}
		...
		if($_REQUEST['tipo_creacion']==1) {
			...
			if($tipodocumentocrear && $documentocrear && $apellidoscrear && $nombrescrear && $emailcrear) 
            {
                if(validacionExisteUsuarioDocEstAdm(trim($_REQUEST['nombres']),trim($_REQUEST['apellidos']),trim($_REQUEST['numerodocumento']),trim($_REQUEST['espracticante']),$db)){
                     generacionUsuarioDocEstAdm(trim($_REQUEST['tipodocumento']),trim($_REQUEST['numerodocumento']),trim($_REQUEST['nombres']),trim($_REQUEST['apellidos']), trim($_REQUEST['email']),trim($_REQUEST['espracticante']),$db,$_REQUEST['administrativodocente']);
                }
			}
		} else {
				...
				for($i=1; $i<=$dataexcel->sheets[0]['numRows']; $i++) {
					...
					if($tipodocumentocrear && $documentocrear && $apellidoscrear && $nombrescrear && $emailcrear && $espracticantecrear) {
						if(validacionExisteUsuarioDocEstAdm(trim($dataexcel->sheets[0]['cells'][$i][4]),trim($dataexcel->sheets[0]['cells'][$i][3]),trim($dataexcel->sheets[0]['cells'][$i][2]),trim($dataexcel->sheets[0]['cells'][$i][6]),$db))
							generacionUsuarioDocEstAdm(trim($dataexcel->sheets[0]['cells'][$i][1]),trim($dataexcel->sheets[0]['cells'][$i][2]),trim($dataexcel->sheets[0]['cells'][$i][4]),
									trim($dataexcel->sheets[0]['cells'][$i][3]),trim($dataexcel->sheets[0]['cells'][$i][5]),trim($dataexcel->sheets[0]['cells'][$i][6]),$db,$_REQUEST['administrativodocente']);
					}
				}
			}
		}
