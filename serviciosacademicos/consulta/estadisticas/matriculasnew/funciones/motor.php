 ...
 */
	...
	function agregarllave_drilldown($nombrecolumna,$link_origen,$link_destino,$descripcion="",$llave,$cadenaadicional="",$variable="",$aliasvariable="",$mailto="",$textoglobo="")
	{
		$this->drill=false;
		if($nombrecolumna!="")
		{
				...
				if(is_array($this->array_drill_down))
				{
					foreach ($this->array_drill_down as $llave => $valor)
					{
						if($elemento[0]==$valor['nombrecolumna'])
						{
							...
							{
								echo $cadena.$complemento.$cadenaadicional.$descriptor.$variable.$globo.$final."\n";
							}
							else
							{
								echo $cadena.$complemento.$cadenaadicional.$descriptor.$globo.$final."\n";
							}
	...
	{
		//echo "<h1>$formato</h1>";
		$nombrearchivo = str_replace ( " " , "_" , trim($nombrearchivo) );
	...
	function agregar_llaves_totales($llave,$link_origen="",$link_destino="",$descripcion="",$cadenaadicional="",$variable="",$aliasvariable="",$textoglobo="")
	{
		...
		$this->arreglo_llaves_totales['llave']=$llave;
		if($link_origen!="")
		{
			$this->arreglo_llaves_totales['link_origen']=$link_origen;
		}
		if($link_destino!="")
		{
			$this->arreglo_llaves_totales['link_destino']=$link_destino;
		}
		if($descripcion!="")
		{
			$this->arreglo_llaves_totales['descripcion']=$descripcion;
		}
		if($cadenaadicional!="")
		{
			$this->arreglo_llaves_totales['cadenaadicional']=$cadenaadicional;
		}
		if($variable!="")
		{
			$this->arreglo_llaves_totales['variable']=$variable;
		}
		if($aliasvariable!="")
		{
			$this->arreglo_llaves_totales['aliasvariable']=$aliasvariable;
		}
		if($textoglobo!="")
		{
			$this->arreglo_llaves_totales['textoglobo']=$textoglobo;
		}
	}
