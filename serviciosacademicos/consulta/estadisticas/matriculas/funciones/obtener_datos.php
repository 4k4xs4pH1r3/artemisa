...
ORDER BY c.codigocarrera
	...
	function obtener_colegio_estudiante($codigoestudiante,$codigomodalidadacademica)
	{
		$query="
		SELECT		
		if(ie.idinstitucioneducativa=1,ie.otrainstitucioneducativaestudianteestudio,i.nombreinstitucioneducativa)
		nombreinstitucion
		FROM
		estudiante e, carrera c,estudianteestudio ie
		left join institucioneducativa i on ie.idinstitucioneducativa=i.idinstitucioneducativa
		WHERE
		e.codigoestudiante='$codigoestudiante'
		and e.codigocarrera=c.codigocarrera
		AND ie.idestudiantegeneral=e.idestudiantegeneral
		and ie.idniveleducacion=".($codigomodalidadacademica/100)."";
	//	echo "<br>";
//echo "<BR><H1>MODALIDAD ACADEMICA =</H1>".$codigomodalidadacademica."<BR>";
		$operacion=$this->conexion->query($query);
		$row_operacion=$operacion->fetchRow();
		return $row_operacion['nombreinstitucion'];
	}
...
//		having
	...
	function obtenerDatosOrdenMatricula($codigoestudiante,$retorno="")
	{
		...
		and e.codigoestudiante='".$codigoestudiante."'
		GROUP by e.codigoestudiante";*/
		$query="SELECT min(o2.fechapagosapordenpago) fechacuotamenor,if(min(o2.fechapagosapordenpago) is not null,min(o2.fechapagosapordenpago),o.fechapagosapordenpago) fechapago,o.*
		FROM  detalleordenpago d, estudiante e, carrera c, concepto co, prematricula pr,ordenpago o
		left join ordenpagoplandepago op on op.numerorodenpagoplandepagosap = o.numeroordenpago
		left join ordenpago o2 on o2.numeroordenpago=op.numerorodencoutaplandepagosap AND o2.codigoestadoordenpago LIKE '4%'
		WHERE o.numeroordenpago=d.numeroordenpago
		AND e.codigoestudiante=pr.codigoestudiante
		AND pr.codigoperiodo='$this->codigoperiodo'
		AND e.codigoestudiante=o.codigoestudiante
