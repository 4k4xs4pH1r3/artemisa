...
ORDER BY c.codigocarrera
	...
	function obtener_datos_estudiante_noprematricula($codigoestudiante)
	{
		...
		sce.nombresituacioncarreraestudiante as situacion_carrera_estudiante
		FROM
		estudiante e, estudiantegeneral eg, carrera c, estadocivil ec,
		 ciudad ciu, jornada j, tipoestudiante te, situacioncarreraestudiante sce,
		  genero g, documento tp
		WHERE
		e.idestudiantegeneral=eg.idestudiantegeneral
		AND eg.codigogenero=g.codigogenero
		AND e.codigocarrera=c.codigocarrera
	...
	function obtener_datos_estudiante_preinscripcion($idpreinscripcion,$codigocarrera)
	{
		...
		'Sin Seguimiento',
			if(tes.codigotipoestudianteseguimiento='999',epc.nombreestadopreinscripcionestudiante,tes.nombretipoestudianteseguimiento)
		) Ultimo_Seguimiento,
		count(distinct idpreinscripcionseguimiento) Seguimientos, 
		c.nombrecarrera,
		p.numerodocumento,
		...
		p.emailestudiante
		FROM
		 preinscripcioncarrera pc, carrera c,estadopreinscripcionestudiante epc,preinscripcion p
		left join preinscripcionseguimiento ps on p.idpreinscripcion =ps.idpreinscripcion 
		...
		group by p.idpreinscripcion
		having 
		max(ps.idpreinscripcionseguimiento)= ((select max(idpreinscripcionseguimiento) idpreinscripcionseguimiento from preinscripcionseguimiento pss where p.idpreinscripcion =pss.idpreinscripcion ))
		or max(ps.idpreinscripcionseguimiento) is NULL";
