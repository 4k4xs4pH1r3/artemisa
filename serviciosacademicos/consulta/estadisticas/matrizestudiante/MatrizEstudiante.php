...
/**
 * Description of MatrizEstudiante
 *
 * @author fmunozb
 */
class MatrizEstudiante {
//put your code here
    var $codigocarrera;
    var $codigoperiodo;
    ...
    function MatrizEstudiante($codigocarrera, $codigoperiodo, $codigomodalidadacademicasic, $codigoareadisciplinar) {
    ...
    function setFiltroCarreras() {
        ...
        from estadisticamatriculados e, detalleestadisticamatriculados d
        where e.fechaestadisticamatriculados = date(now())
        and d.idestadisticamatriculados = e.idestadisticamatriculados
    ...
    function getTotalEstudiantesMatriculados() {
        global $db;
    ...
    function getTotalEstudiantesMatriculadosXModalidad($codigomodalidadacademicasic) {
        global $db;
        //$db->debug = true;
        if($this->obtenerDatosEnLinea) {
            $query = "SELECT count(distinct e.codigoestudiante) conteo
            FROM ordenpago o, detalleordenpago d, estudiante e, carrera c, concepto co, facultad f
            WHERE o.numeroordenpago=d.numeroordenpago
            AND e.codigoestudiante=o.codigoestudiante
            AND c.codigocarrera=e.codigocarrera
            AND d.codigoconcepto=co.codigoconcepto
            ...
            AND o.codigoestadoordenpago LIKE '4%'
            AND o.codigoperiodo='$this->codigoperiodo'
            and c.codigomodalidadacademicasic = '$codigomodalidadacademicasic'
            and c.codigofacultad = f.codigofacultad";
        }
        else {
            $query = "select sum(a.conteo) as conteo
    ...
    function getTotalEstudiantesMatriculadosXModalidadXArea($codigomodalidadacademicasic, $codigoareadisciplinar) {
        global $db;
        //$db->debug = true;
        if($this->obtenerDatosEnLinea) {
            $query = "SELECT count(distinct e.codigoestudiante) conteo
    ...
    function getTotalEstudiantesMatriculadosXCarrera($codigocarrera) {
        global $db;
        //$db->debug = true;
        if($this->obtenerDatosEnLinea) {
            $query = "SELECT count(distinct e.codigoestudiante) conteo
    ...
    function getListaNivelesEducativos() {
        global $db;
        $query = "select nombreniveleducacion, pesoniveleducacion
        from niveleducacion
        where idniveleducacion <> 5
        order by 2";
        $sel = $db->Execute($query);
        $totalRows_sel = $sel->RecordCount();
        while($row_sel = $sel->FetchRow()) {
            $niveles[] = $row_sel['nombreniveleducacion'];
        }
    ...
    function getListaParticipacionBienestar() {
        global $db;
        $query = "SELECT nombretipoparticipacionuniversitariaestudiante
        FROM tipoparticipacionuniversitariaestudiante";
        $sel = $db->Execute($query);
        $totalRows_sel = $sel->RecordCount();
        while($row_sel = $sel->FetchRow()) {
            $participaciones[] = $row_sel['nombretipoparticipacionuniversitariaestudiante'];
        }
        $participaciones[] = "No registra";
        return $participaciones;
    }
    ...
    function getListaParticipacionGobierno() {
        global $db;
        $query = "SELECT nombretipoparticipaciongobiernoestudiante
        FROM tipoparticipaciongobiernoestudiante
        where codigoestado like '1%'";
        $sel = $db->Execute($query);
        $totalRows_sel = $sel->RecordCount();
        while($row_sel = $sel->FetchRow()) {
            $participaciones[] = $row_sel['nombretipoparticipaciongobiernoestudiante'];
        }
        $participaciones[] = "No registra";
        return $participaciones;
    }
    ...
    function getListaAsociacion() {
        global $db;
        $query = "SELECT nombretipoasociacionestudiante
        FROM tipoasociacionestudiante";
        $sel = $db->Execute($query);
        $totalRows_sel = $sel->RecordCount();
        while($row_sel = $sel->FetchRow()) {
            $participaciones[] = $row_sel['nombretipoasociacionestudiante'];
        }
        $participaciones[] = "No registra";
        return $participaciones;
    }
    ...
    function getListaReconocimiento() {
        global $db;
        $query = "SELECT nombretiporeconocimientoestudiante
        FROM tiporeconocimientoestudiante";
        $sel = $db->Execute($query);
        $totalRows_sel = $sel->RecordCount();
        while($row_sel = $sel->FetchRow()) {
            $reconocimientos[] = $row_sel['nombretiporeconocimientoestudiante'];
        }
    ...
    function getListaHistoricoEstudiante() {
        global $db;
