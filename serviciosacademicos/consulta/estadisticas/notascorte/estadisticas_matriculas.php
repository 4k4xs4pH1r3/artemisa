...
foreach ($carreras as $llave_carreras => $valor_carreras)
{
	...
	if(in_array('Inscritos_Evaluados',$arraymenuopcion))
	$array_datos[$contador]['Inscritos_Evaluados']=$datos_matriculas->ObtenerDatosCuentaOperacionPrincipalInscritosEvaluado($_SESSION['codigoperiodo_reporte'],$valor_carreras['codigocarrera'],153,'conteo');	
	if(in_array('a_seguir_inscritos_vs_matriculados_nuevos',$arraymenuopcion))
	...
	if(in_array('a_seguir_Prematriculados',$arraymenuopcion))
	...
	if(in_array('a_seguir_No_prematriculados',$arraymenuopcion))
