...
if(isset($codigodocente)&&trim($codigodocente)!='')
...
$condicion=" and idformulariopadre = '".$iditemsicpadre."' 
	...
	and idformulario <> '1'
	and f.codigotipousuario like '5%'
	order by f.pesoformulario
	 ";
$nivel++;
$resultado=$this->objetobase->recuperar_resultado_tabla($tabla,$nombreidtabla,$idtabla,$condicion,"",0);
...
//if($this->codigocarrera){
	$tabla="formulariodocente fd,periodo p";
	$nombreidtabla="fd.codigoestado";
	$idtabla="100";
	$condicion=" and fd.idformulario = '".$iditem."' 
		and fd.iddocente='".$this->codigodocente."'
		and fd.codigoestado like '1%'
		and fd.codigoestadodiligenciamiento in ('200','300')
...
$ramaarray=explode("?",$rama["enlaceingresoformulario"]);
$this->dibujaabreitemli($iditem,$nombrecorto,$descripcionitem,$nivel,$descripcionul,$ramaarray[0]."?idformulario=".$rama["idformulario"]."&".$ramaarray[1]," target='marcocentral' ",$javascriptdetalle,$fotomuestra);
if($tienehijos)
	$this->dibujaabreitemul($descripcionul);
