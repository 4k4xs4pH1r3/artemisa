...
@@session_start();
$direccion = "aspectosFamiliares.php";
...
?>
          ...
          <tr>
           <!--  <td id="tdtitulogris">Edad:</td>
            ...
            <td><input type="text" name="ocupacionestudiantefamilia" size="25" value="<?php if (isset($row_datosgrabados['ocupacionestudiantefamilia'])) echo $row_datosgrabados['ocupacionestudiantefamilia']; else echo $_POST['ocupacionestudiantefamilia'];?>"></td>
		 <!--  </tr>
          <tr>
...
{	 
	...
	/*else if (!eregi("^[0-9]{1,15}$", $_POST['telefono2']) and $_POST['telefono2'] <> "")
	{
	    echo '<script language="JavaScript">alert("Teléfono2 Incorrecto")</script>';		
		$banderagrabar = 1;
	}
	else if (!eregi("^[0-9]{1,15}$", $_POST['celular']) and $_POST['celular'] <> "")
	{
		echo '<script language="JavaScript">alert("Celular Incorrecto")</script>';		
		$banderagrabar = 1;
	}
	else if (!eregi($email,$_POST['email']) and $_POST['email'] <> "")
	{
		...
		telefono2estudiantefamilia = '".$_POST['telefono2']."',
	   	...
	   	*/
	   	...
	   	echo "<script language='javascript'>
	   		window.location.href='aspectosFamiliares.php?".$_SESSION['fppal']."#ancla".$_SESSION['modulosesion']."';
