...
formulario.action="participaciongestionestudiante.php?Nuevo_Registro=1&idformulario=<?php echo $_GET["idformulario"]; ?>";
formulario.submit();
...
/*if($_REQUEST["codigotipoparticipaciongestionestudiante"]=="400")
$formulario->dibujar_fila_titulo("INDICACION: Seleccione de la lista a las que pertenece",'tdtituloencuestadescripcion',"2","align='left'","td");*/
	...
	$condicion=" and d.codigoestado like '1%'
			and d.codigotipoparticipaciongestionestudiante=tp.codigotipoparticipaciongestionestudiante";

	$resultado=$objetobase->recuperar_resultado_tabla("participaciongestionestudiante d,tipoparticipaciongestionestudiante tp ","d.idestudiantegeneral",$_SESSION['sissic_idestudiantegeneral'],$condicion,"",0);
		$fila["Codigo"]="";
	...
	while($row=$resultado->fetchRow()){
		unset($fila);
		$enlacedetalle="<a href='participaciongestionestudiante.php?idparticipaciongestionestudiante=".$row["idparticipaciongestionestudiante"]."&idformulario=".$_GET["idformulario"]."'>".$row["idparticipaciongestionestudiante"]."</a>";
		$fila[]=$enlacedetalle;
		$fila[]=$row["nombretipoparticipaciongestionestudiante"];				$fila[]=$row["fechainicioparticipaciongestionestudiante"];
...
//echo "<h1>ENTRO A ANULAR  idestimulodocente ID=".$muestraformulario."</h1>";
	...
	if(isset($_GET["idparticipaciongestionestudiante"])&&trim($_GET["idparticipaciongestionestudiante"])!=''){
		unset($_POST);
		unset($_GET["idparticipaciongestionestudiante"]);
	}
...
if($datosdedicacionexterna=$objetobase->recuperar_datos_tabla("participaciongestionestudiante","idparticipaciongestionestudiante",$_GET["idparticipaciongestionestudiante"],"","",0)){
...
//echo "<h1>ENTRO 2 A CAMBIAR MUESTRA=".$muestraformulario."</h1>";
	...
	/*if(isset($_REQUEST["codigotipoparticipaciongestionestudiante"])&&count($_REQUEST["codigotipoparticipaciongestionestudiante"])>0){
		$codigotipoparticipaciongestionestudiante=$_REQUEST["codigotipoparticipaciongestionestudiante"];
		if($datosparticipacion=$objetobase->recuperar_datos_tabla("participaciongestionestudiante t","codigotipoparticipaciongestionestudiante",$codigotipoparticipaciongestionestudiante," and idestudiantegeneral=".$_SESSION['sissic_idestudiantegeneral']."",'',0)){
			$_GET["idparticipaciongestionestudiante"]=$datosparticipacion["idparticipaciongestionestudiante"];
		}
		else{
			if(trim($_REQUEST["codigotipoparticipaciongestionestudiante"])!=''){
				unset($_POST["nombreparticipaciongestionestudiante"]);
				$tabla="participaciongestionestudiante";
				$fila["idestudiantegeneral"]=$_SESSION['sissic_idestudiantegeneral'];
				//$fila["codigotipoconsejouniversidad"]="400";
				$fila["codigotipoparticipaciongestionestudiante"]=$_REQUEST["codigotipoparticipaciongestionestudiante"];
				$fila["nombreparticipaciongestionestudiante"]="";
				$fila["codigoestado"]="100";
				
				$objetobase->insertar_fila_bd($tabla,$fila,$imprimir,$condicionactualiza);
				
				$datosparticipacion=$objetobase->recuperar_datos_tabla("participaciongestionestudiante t","codigotipoparticipaciongestionestudiante",$codigotipoparticipaciongestionestudiante," and idestudiantegeneral=".$_SESSION['sissic_idestudiantegeneral']."",'',0);
				$_GET["idparticipaciongestionestudiante"]=$datosparticipacion["idparticipaciongestionestudiante"];
			}
		}
...
//$muestraformulario=1;
$objdibujaformulario=new DibujaFormulario("participaciongestionestudiante",$_GET["idparticipaciongestionestudiante"],$sala,$_GET["idformulario"],$formulario);
...
if($muestraformulario){
	...
	if(isset($_REQUEST["codigotipoparticipaciongestionestudiante"])&&count($_REQUEST["codigotipoparticipaciongestionestudiante"])>0){
		...
		$codigotipoparticipaciongestionestudiante=$objdibujaformulario->recuperarValorCampo("codigotipoparticipaciongestionestudiante");
	}
	...
	$arrayquitacolumnas[3]="codigotipoparticipaciongestionestudiante";
	...
	$formulario->filatmp=$objetobase->recuperar_datos_tabla_fila("tipoparticipaciongestionestudiante t","codigotipoparticipaciongestionestudiante","nombretipoparticipaciongestionestudiante"," codigoestado like '1%'",'',0);

	$formulario->filatmp[""]="Seleccionar";	
	$campo='menu_fila'; $parametros="'codigotipoparticipaciongestionestudiante','".$codigotipoparticipaciongestionestudiante."','onchange=enviarparticipacionuniversitaria();'";
	$formulario->dibujar_campo($campo,$parametros,"Tipo de Participacion","tdtitulogris",'codigotipoparticipaciongestionestudiante','requerido');
	if(isset($codigotipoparticipaciongestionestudiante)&&trim($codigotipoparticipaciongestionestudiante)!=''){

		//if($codigotipoparticipaciongestionestudiante<>"400"){
		...
		$objdibujaformulario->columnas[]="codigotipoparticipaciongestionestudiante";
