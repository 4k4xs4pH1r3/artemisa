...
$_SESSION['estudiantesordencolegio']=$_POST['estudiante'];

if(formato_fecha_mysql($_GET['fechapago'])>=date("Y-m-d")){


echo "<script languaje='javascript'>
        window.open('envioordenescolegio.php?estudiante=".serialize($_POST['estudiante'])."&codigoconcepto=".$_GET['codigoconcepto']."&fechapago=".$_GET['fechapago']."&mesorden=".$_GET['mesorden']."', 'Envio_ordenes', 'width=400, height=250, top=100, left=200, scrollbars=yes,toolbar=no, resizable=yes, status=no,menu=no')
    </script>";
...
for($i=0;$i<count($_POST['estudiante']);$i++){
	//echo "<br>Estudiante=".$_POST['estudiante'][$i].",valorpecuniario=".$datosconcepto['valorpecuniario'];
echo "<h1>".$_POST['estudiante'][$i]."</h1><br>";
$ordenesxestudiante = new Ordenesestudiante($salatmp, $_POST['estudiante'][$i], $_SESSION['codigoperiodoordenescolegio']);
...
$condicion=" and f.codigocarrera = e.codigocarrera
	...
	and e.codigoestudiante = '".$_POST['estudiante'][$i]."'
...
alerta_javascript("Creadas $i ordenes de pensión");*/
}
else{
