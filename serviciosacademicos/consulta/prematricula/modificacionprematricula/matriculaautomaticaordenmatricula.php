...
function calcular_valorcredito($sala, $valordetallecohorte, $codigoestudiante){
    ...
    $query_selplanestudiante= "select p.idplanestudio from planestudioestudiante p, planestudio pe 	where p.codigoestudiante = '$codigoestudiante' and  p.idplanestudio = pe.idplanestudio and pe.codigoestadoplanestudio like '1%' and p.codigoestadoplanestudioestudiante like '1%'";
...
if($totalRows_oiniciales == ""){
    ...
    $query_seltotalcreditossemestre2 = "select sum(d.numerocreditosdetallelineaenfasisplanestudio) as totalcreditossemestre, d.idplanestudio from detallelineaenfasisplanestudio d, lineaenfasisestudiante l where d.idlineaenfasisplanestudio = l.idlineaenfasisplanestudio and now() between  l.fechainiciolineaenfasisestudiante and l.fechavencimientolineaenfasisestudiante  and l.codigoestudiante = '$codigoestudiante' and d.semestredetallelineaenfasisplanestudio = '".$row_iniciales['semestreprematricula']."' and d.codigotipomateria not like '4' group by 2 ";
    ...
    if($totalcreditossemestre2 == ""){
        $query_seltotalcreditossemestre2 = "select sum(d.numerocreditosdetalleplanestudio) as totalcreditossemestre, d.idplanestudio from detalleplanestudio d, planestudioestudiante p where d.idplanestudio = p.idplanestudio and p.codigoestudiante = '$codigoestudiante' and d.semestredetalleplanestudio = ".$row_iniciales['semestreprematricula']." and d.codigoestadodetalleplanestudio like '1%' and d.codigotipomateria like '5%' and p.codigoestadoplanestudioestudiante like '1%' group by 2";
        $seltotalcreditossemestre2 = mysql_db_query($database_sala,$query_seltotalcreditossemestre2) or die("$query_seltotalcreditossemestre2".mysql_error());
        $totalRows_seltotalcreditossemestre2 = mysql_num_rows($seltotalcreditossemestre2);
        $row_seltotalcreditossemestre2 = mysql_fetch_array($seltotalcreditossemestre2);
        $totalcreditossemestre2 = $row_seltotalcreditossemestre2['totalcreditossemestre'];
    ...
    <tr>
        <td id="tdtitulogris">
            ...
            if($_SESSION['MM_Username'] == "estudiante" || $_SESSION['MM_Username'] == "estudianterestringido"){
                ...
                if ($row_estudiante <> ""){
                    ...
                    if ($totalRows_carrera<>0){
                        ?>
                        <!--  <a href="../eva20061/ffacultad.php" id="aparencialinknaranja"><img src="../../../imagenes/evaluacion.gif" width="20" height="25" alt="Evaluaciï¿½n Docente" style="border-color:#FFFFFF"></a>-->
                        <?php
                    }
                }//if
            }//if MM_Username
            ...
            if($paseautoevaluacion){
                echo " <a href='../facultades/validaautoevaluacion.php?codigoestudiante=".$codigoestudiante."'><img src='../../../imagenes/autoevaluacion.jpg' width='20' height='25' alt='Autoevaluacion' style='border-color:#FFFFFF'></a>";
            }
            if($_SESSION['MM_Username'] == "colegio"){
                ?>
                <a href="generarordenescolegio/generarordenescolegio.php"><img src="../../../imagenes/ordenautomaticaindividual.gif" width="20" height="25" alt="Generar Orden Automï¿½tica" style="border-color:#FFFFFF"></a>
                <?php
            }
            /*
            if($_SESSION['MM_Username'] == "adminmedicina" or $_SESSION['MM_Username'] == "decmedicina")
            {
            ?>
            <a href="../facultades/modificahistoricoformulariomedicina.php?codigoestudiante=<?php echo $codigoestudiante; ?>"><img src="../../../imagenes/iconos/modificarhistorico.gif" alt="Modificación Historico de Notas Medicina" style="border-color: rgb(255, 255, 255);" height="25" width="20"></a>
            <?php
            }*/
...
<!--<form name="form1" method="post" action="matriculaautomaticaordenmatricula.php?programausadopor=--><?php //echo $programausadopor;?><!--">-->
    ...
    if($totalRows_oiniciales != "" || $totalRows_oiniciales2 != ""){
        ...
        <p>DATOS GENERALES DE LA MATRICULA </p>
        <table class="table table-bordered" style="font-size: 12px">
            <tr id="trtituloNaranjaInst">
                ...
                <td>Documento</td>
                <td>Género</td>
                <td rowspan="7" align="center" style="background-color: #FFFFFF">
            ...
            <tr id="trtituloNaranjaInst">
                ...
                <td colspan="2">Cr&eacute;ditos Seleccionados: <?php echo $creditoscalculados+$numcreditoelectivaibre; ?></td>
            </tr>
            <tr>
                <td colspan="2"><?php echo $row_iniciales['nombrecarrera'];?></td>
                <td colspan="2"><?php if($nombreplan == "Sin Asignar"){echo "Sin Asignar";} else if($existeinicial){echo $totalcreditossemestre;} else echo "0";?></td>
                <td><?php if($nombreplan == "Sin Asignar"){echo "Sin Asignar";} else if($existeinicial){$creditosobligatorios = $creditoscalculados; echo '<b>Obligatorios:</b> '.$creditosobligatorios;} else echo "0";?></td>
                <td><?php if($nombreplan == "Sin Asignar"){echo "Sin Asignar";} else if($existeinicial){echo '<b>Electivos:</b> '.$numcreditoelectivaibre;} else echo "0";?></td>
            </tr>
            <?php
            ...
            /*
            Esta validacion se agrega con el fin de restar los creditos de electiva libre a los creditos calculados
            para que cuando se muestre la informacion del estudiante en la casilla valor Valor Crédito Adicional no muestre un valor que no corresponde
            la variable $numcreditoelectivalibre se crea en el archivo calculocreditossemestre.php y en ese archivo se encuentra comentado
            el por q de la creacion de esta variable
            la modificacion se realizo el dia 14 de agosto de 2013
            */
            if(isset($numcreditoelectivalibre)){
            ...
            if($creditosadicionales>0){
             ...
             $cobroadicional=number_format(round(($valorcreditootrajornada*$creditosadicionales)+(($valorcreditootrajornada-$valorcreditonormal)*($creditosotrajornada-$creditosadicionales))));
            ...
            <tr id="trtituloNaranjaInst">
                <td></td>
                <td colspan="2" align="center">Valor Matricula</td>
                <td colspan="3"></td>
            </tr>
            <tr id="trtituloNaranjaInst">
                <td>Fecha</td>
            ...
            <tr>
                <td colspan="2"><?php echo "$ ".number_format($valorcreditonormal);?></td>
        ...
        ?>
        <P>NO TIENE REGISTRADAS PREMATRICULAS PARA ESTE PERIODO</P>
...
//        var_dump('ordenesxestudiante: ', $ordenesxestudiante,
//                'ordenesxestudiantepre: ', $ordenesxestudiantepre,
//                'ordenesxestudiante: ', $ordenesxestudiante);
  ...
  ";
    ...
    ///// FIN ///////
    $query_dataestudiante = "select p.idprematricula FROM prematricula p, detalleprematricula pr  where p.codigoestudiante = '$codigoestudiante'  AND p.codigoperiodo = '".$_SESSION['codigoperiodosesion']."' and pr.idprematricula = p.idprematricula AND ( pr.codigomateria IN (16716, 16717) OR pr.codigomateriaelectiva IN (16716, 16717) ) AND pr.codigoestadodetalleprematricula IN (30,10)";
    $dataestudiante = mysql_query($query_dataestudiante, $sala) or die("$query_dataestudiante".mysql_error());
    $totalRows_dataestudiante = mysql_num_rows($dataestudiante);
    if($totalRows_dataestudiante>0)
    {
         ...
         * Se modifica el link de descarga del instructivo de ingles
         * @since Enero 28, 2019
         */
        ?>
