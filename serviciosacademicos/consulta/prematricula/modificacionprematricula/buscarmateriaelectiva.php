...
function cambia_tipo()
{
    ...
    if (tipo == 1)
	{
		window.location.href="buscarmateriaelectiva.php?planestudio='.$idplanestudio.'&busqueda=nombre";
	}
    if (tipo == 2)
	{
		window.location.href="buscarmateriaelectiva.php?planestudio='.$idplanestudio.'&busqueda=codigo";
    }
}
...
function buscar()
{
    ...
    if (busca != 0)
	{
		window.location.href="buscarmateriaelectiva.php?planestudio='.$idplanestudio.'&"+busca;
	}
}
</script>';
?>
<div align="center">
<form name="f1" action="buscarmateriaelectiva.php" method="get">
  <p class="Estilo1 Estilo2"><strong>B&Uacute;SQUEDA DE MATERIA ELECTIVA</strong></p>
...
<?php
	...
	if(isset($_GET['busqueda_nombre']))
	{
		...
		$query_solicitud = "select m.nombremateria, m.codigomateria, c.nombrecarrera, pee.idplanestudio
...
group by codigomateria
	...
	if(!$vacio)
	{
		...
		while($solicitud = mysql_fetch_assoc($res_solicitud))
		{
			...
			echo "<tr>
				<td class='Estilo1'><a href='buscarmateriaelectiva.php?aceptar=$codigomateria&idplan=$idplanestudio'>$codigomateria&nbsp;</a></td>
