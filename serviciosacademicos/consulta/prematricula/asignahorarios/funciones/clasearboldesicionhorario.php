...
//CONSTRUCTOR DE CLASE, INICIA RASTREO Y VARIABLES GLOBALES.
function arbolDesicionHorario($estructurahorarios,$estructuradatosmateria,$codigocarrera,$codigoperiodo,$codigojornada,$objetobase){
	...
	$this->horainicial=mktime(date("H"),date("i"),date("s"));
	
		foreach ($estructurahorarios as $materiai => $grupos){
			foreach ($grupos as $idgrupo => $obj){
				$materiasinicial[$materiai][$idgrupo]=1;
			}
		}
		$i=0;
		$estructuradatosmateria2=$estructuradatosmateria;
		$tmpsemestremenor=9999999;
		foreach ($estructuradatosmateria as $materiai => $datosmateria){
...
//echo "--HORA INICIAL=";
$this->restringejornada[$i]['horainicial']=horaaminutos($row['horainiciodetallecobroexcedentecambiojornada'])+$row['codigodia']*10000;

$this->restringejornada[$i]['horainicialh']=$row['horainiciodetallecobroexcedentecambiojornada'];
//echo "--HORA FINAL=";
$this->restringejornada[$i]['horafinal']=horaaminutos($row['horafinaldetallecobroexcedentecambiojornada'])+$row['codigodia']*10000;

$this->restringejornada[$i]['horafinalh']=$row['horafinaldetallecobroexcedentecambiojornada'];
...
//echo "<br>";
$condicion=" and djc.idjornadacarrera=jc.idjornadacarrera";

$tablas="jornadacarrera jc, detallejornadacarrera djc";
$resultado_jornada= $this->objetobase->recuperar_resultado_tabla($tablas,"jc.codigocarrera",$this->codigocarrera,$condicion,"",0);

$i=0;
while ($row = $resultado_jornada->fetchRow()){
	$this->restringejornada[$i]['dia']=$row['codigodia'];
	//echo "--HORA INICIAL=";
	$this->restringejornada[$i]['horainicial']=horaaminutos($row['horainiciodetallejornadacarrera'])+$row['codigodia']*10000;
	$this->restringejornada[$i]['horainicialh']=$row['horainiciodetallejornadacarrera'];
	//echo "--HORA FINAL=";
	$this->restringejornada[$i]['horafinal']=horaaminutos($row['horafinaldetallejornadacarrera'])+$row['codigodia']*10000;
	$this->restringejornada[$i]['horafinalh']=$row['horafinaldetallejornadacarrera'];
				...
				if(is_array($this->restringejornada)){
						for($ir=0;$ir<count($this->restringejornada);$ir++){
							...
							if($this->restringejornada[$ir]['dia']==$dia){
								...
								/*if($grupo=="35049")
									echo "($horaioniciorestringida,$horafinalrestringida,$horainicial,$horafinal)<BR>";*/
								if(!$this->horascruzadasexhaustivo($horainicial,$horafinal,$horaioniciorestringida,$horafinalrestringida)){
...
function  horascruzadasexhaustivo($horainicial1,$horafinal1,$horainicial2,$horafinal2){
if($horainicial1>=$horainicial2&&$horainicial1<$horafinal2)
	if($horafinal1>$horainicial2&&$horafinal1<=$horafinal2)
		return 1;
...
echo "<br>";
	...
	else
	{
		...
		//Sin cupo Ver Articulo 41 Reglamento Estudiantil
			...
			$this->horafinal=mktime(date("H"),date("i"),date("s"));
			
			if(!empty($pilamaterias)&&(($this->horafinal-$this->horainicial)<120)){
