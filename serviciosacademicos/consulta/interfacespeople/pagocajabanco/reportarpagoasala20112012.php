...
function InformacionCajaBancos($numeroordenpago, $fechapagoordenpago){
    ...
    global $salaobjecttmp;
     ...
     $fechahoy=date("Y-m-d");
//echo "<h1><b>Llamando la funcion</b></h1>";
    /****************************
     ****************************
     **Reporta  los pagos a sala*
     ****************************
     ****************************/
        //$numeroordenpago = 1427803;
    ...
    if(checkdate((int) $mes, (int) $dia, (int) $anio) && $fechapago<=$fechahoy){
		...
		where numeroordenpago = '$numeroordenpago'";                
		 ...
		 AND d.codigotipodetalleordenpago = 2";                
            ...
            VALUES( 0, 'Pago Caja-Bancos','INVOICE_ID =,$numeroordenpago  TRANSFER_DT=$fechapago', 'ERRNUM=0, DESCRLONG=Ok', '$numeroordenpago',now())";
        ...
        //return $param;//new soapval('ERRNUM','xsd:string',$param2);
		   ...
		   // return new soap_fault("Client", "", "Número Orden Pago Incorrecto");
		    $query_logps="INSERT INTO logtraceintegracionps(idlogtraceintegracionps, transaccionlogtraceintegracionps, enviologtraceintegracionps, respuestalogtraceintegracionps, documentologtraceintegracionps, fecharegistrologtraceintegracionps)
		    VALUES( 0, 'Pago Caja-Bancos','INVOICE_ID =,$numeroordenpago  TRANSFER_DT=$fechapago', 'ERRNUM=0, DESCRLONG=ok, orden de pago menor a 1000000, ser reporta como ok para finalizarlas en PS', '$numeroordenpago',now())";
		$logps = $db->Execute($query_logps) or die(mysql_error());
     	       return array(
        	        'ERRNUM' =>0,
                	'DESCRLONG' => 'ok'
		   ...
		   // return new soap_fault("Client", "", "Número Orden Pago Incorrecto");
		    ...
		    enviologtraceintegracionps, respuestalogtraceintegracionps, documentologtraceintegracionps,
		    fecharegistrologtraceintegracionps)
		    VALUES( 0, 'Pago Caja-Bancos','INVOICE_ID =,$numeroordenpago  TRANSFER_DT=$fechapago', 'ERRNUM=1, DESCRLONG=Número Orden Pago no Existe', '$numeroordenpago',now())";
		$logps = $db->Execute($query_logps) or die(mysql_error());
		    return array(
			'ERRNUM' => 1,
			'DESCRLONG' => 'Número Orden Pago no Existe'
			);
	    }
        }
        ...
        $query_logps="INSERT INTO logtraceintegracionps(idlogtraceintegracionps, transaccionlogtraceintegracionps,
            ...
            'ERRNUM=2, DESCRLONG=Fecha incorrecta', '$numeroordenpago',now())";
        $logps = $db->Execute($query_logps) or die(mysql_error());
        return array(
                'ERRNUM' => 2,
