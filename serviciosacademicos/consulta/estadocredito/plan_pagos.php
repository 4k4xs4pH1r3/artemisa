		...
		{ // foreach 2  
		  ...
		  if ($valor1 <> "DOCCC_SAP" and $valor1 <> "DOCPP_SAP" and $valor1 <> "OPPAL" and $valor1 <> "OPPAR" and $valor1 <> "TEXT")
		    { 
			 ...
			 if($valor1 == "FPAGO")
			  {
			   echo "<td colspan='$colspan'><div align='center' class='Estilo13 Estilo14'>",cambiaf_a_sala($total1),"</div></td>";
	          }
			 else
			 if ($valor1 == "VPAGO")
			   {
			    echo "<td colspan='$colspan'><div align='center' class='Estilo13 Estilo14'>",number_format($total1,0),"</div></td>";
			   }
			  else
			  {
			   echo "<td colspan='$colspan'><div align='center' class='Estilo13 Estilo14'>",$total1,"</div></td>";			  
			  }
			} 			
			 ...
			 /*if ($cont == 0)
			   ...
			   if ($fechapago < $hoy)  
			    {			
				  ...
				  saprfc_table_append ($existe,"I_ITEMTAB", array ("I_POSNO"=>"0000000001","I_CLIENT"=>$mandante,"I_IRULE"=>"RULE UB02","I_FROMDAT"=>$fechapago,"I_TODAT"=>$hoy,"I_KEYDAT"=>$hoy,"I_KEYDAY"=>"00","I_INTINT"=>"","I_AMOUNT"=>$value,"I_CURR"=>"COP","I_EXCT"=>"","I_EXCV"=>"","I_BUKRS"=>"UB01"));
		  ...
		  if ($indicadorplancuotapaga == '1' and $text <> 'BLOQUEO')
		   {
             ...
             echo "<td><div align='left' class='Estilo13 Estilo14'><a href='generarorden.php?ordenpago=$ordenpago&cxc=$cxc&cxp=$cxp&fechalimite=$fechalimite&valores=$tmp'>Generar Orden Pago</a></div></td>";
           }
		  else
		  if ($text == 'BLOQUEO')
		   {
