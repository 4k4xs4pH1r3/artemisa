...
menu[0]="<?php	
			$condicion="em.codigotipoempresasalud = t.codigotipoempresasalud and (t.nombretipoempresasalud like 'ERP%' or t.nombretipoempresasalud like 'No%')
			...
			$formulario->menu_fila('arpnueva',$arpnueva,'id=\"arpnueva\" onChange=\"cambiamenu();\" onClick=cambieestadoclickmenu();');

			?>";
nombremenu[0]="arpnueva";
...
echo "<form name=\"form2\" action=\"listadoestudiantesaportesarp.php?Filtrar=Filtrar\" method=\"post\"  >
<input type=\"hidden\" name=\"AnularOK\" value=\"\">
	...
	$formulario->dibujar_fila_titulo('ASIGNACION MASIVA DE INGRESO DE COTIZANTE-ARP POSTGRADO','labelresaltado',"2","align='center'");
...
//	$formulario->dibujar_campo($campo,$parametros,"Mes de cierre","tdtitulogris",'mescierre','');
	...
	unset($formulario->filatmp);
	$formulario->filatmp["1"]="Normal";$formulario->filatmp["2"]="Extemporaneo";$formulario->filatmp["3"]="Retiro";
 	$defecto="NA";
	...
	unset($formulario->filatmp);
	$formulario->filatmp["3"]="Si";$formulario->filatmp["4"]="No"; 
	$defecto="4";
	if($_POST['ingresadoeps']) $ingresadoeps=$_POST['ingresadoeps']; else $ingresadoeps=$defecto;
	$parametros="'ingresadoeps','".$ingresadoeps."',''";
	$campo='menu_fila';
	$formulario->dibujar_campo($campo,$parametros,"Ingresados EPS","tdtitulogris",'ingresado');
	...
	$filtroarp=6;
	$archivoarp="crearretiroaportesarp.php";
	}
	else{
	$filtroarp=5;
	$archivoarp="crearingresoaportesarp.php";
	}
...
if(validacionprocesoactivo($_POST['fechacierre'],$datos_bd,4,0)){
...
";*/
if(isset($_POST['fechacierre'])&&trim($_POST['fechacierre'])!=''){
//echo "Mes de cierre=".$_SESSION['sesion_mescierre'];
unset($_SESSION['sesion_mescierre']);
$mescierre=$_POST['fechacierre'][3].$_POST['fechacierre'][4].$_POST['fechacierre'][5].$_POST['fechacierre'][6].
				$_POST['fechacierre'][7].$_POST['fechacierre'][8].$_POST['fechacierre'][9];
...
//	$row_temp['codigoempresasalud']="<div id='celda_".$i."_6' >".$row_operacion['codigoempresasalud']."$espacio</div>";
	$row_temp['codigoempresasalud']="&nbsp;";
	if(validar_estudiante_arp($mescierre,$datos_estudiante['codigoestudiante'],$datos_bd,$_POST['pagoeps'],$_POST['pagoarp'],$filtroarp))
		if(validar_estudiante_arp($mescierre,$datos_estudiante['codigoestudiante'],$datos_bd,$_POST['pagoeps'],$_POST['pagoarp'],$_POST['ingresadoeps'])){
...
$parametrogeneral="&fechainicio=".$_POST['fechacierre']."&ingresado=".$_POST['ingresado'];
...
<?php
		...
		$motor->agregarllave_drilldown('numerodocumento','listadoestudiantesaportesarp.php','ingresoaportes.php','','codigoestudiante',"",'','','','','onclick= "return ventanaprincipal(this)"');
		$motor->mostrar();
		}
	}
	else
	{
	alerta_javascript("No puede ingresar novedad por que la fecha no esta dentro de un periodo activo");
	}
}
