...
<?php
    ...
    if($objvalidaautoevaluacion->validaEncuestaCompleta()){
		...
		$query_esactua = "select * from actualizacionusuario 
  		where usuarioid='".$_SESSION['idusuariofinalentradaentrada']."'
		...
		if($totalRows_esactua==""){		
		 $query_actualizaestado = "insert into actualizacionusuario (usuarioid, tipoactualizacion, id_instrumento, codigoperiodo, estadoactualizacion,userid,entrydate)
 	         values ('".$_SESSION['idusuariofinalentradaentrada']."',3,0,'".$_SESSION['codigoperiodo_autoenfermeria']."',1,'".$_SESSION['idusuariofinalentradaentrada']."',now())";
		...
		echo "<META HTTP-EQUIV='Refresh' CONTENT='0;URL=../encuestahumanidades/encuestaenfermeria.php?codigoestudiante=". $_GET['codigoestudiante']."&codigotipousuario=".$_REQUEST['codigotipousuario'];
