 ...
 * */
    ...
    $query="select nombrecarrera, sum(cuenta) egresados  from (
select ca.nombrecarrera,count(distinct e.codigoestudiante) cuenta
from registrograduado rg ,estudiante e , carrera ca
where rg.codigoestudiante=e.codigoestudiante
and ca.codigocarrera=e.codigocarrera
and ca.codigomodalidadacademica in (200,300)
and rg.codigoestado like '1%'
group by ca.codigocarrera
union
select ca.nombrecarrera,count(distinct e.codigoestudiante) cuenta
from registrograduadoantiguo rg ,estudiante e , carrera ca
where rg.codigoestudiante=e.codigoestudiante
and ca.codigocarrera=e.codigocarrera
and ca.codigomodalidadacademica in (200,300)
group by ca.codigocarrera
...
$query=" select ca.codigocarrera,ca.nombrecarrera, count(e.codigoestudiante) egresados from 
    estudiantegeneral eg , estudiante e,carrera ca where e.idestudiantegeneral=eg.idestudiantegeneral
    and e.codigosituacioncarreraestudiante in ('104','400')
    and ca.codigocarrera=e.codigocarrera
    and ca.codigomodalidadacademica in (200,300)
    group by ca.codigocarrera
        order by ca.nombrecarrera
    ";
    ...
    while ($rowresultadodocente = $rescarrerasdocente->fetchRow()) {
        ...
        $array_interno[$i]["Egresados"] = $rowresultadodocente["egresados"];

        $sumamatriculados+=$rowresultadodocente["egresados"];
        $i++;
    }
    ...
    while ($row_operacion = $operacion->fetchRow()) {
        ...
        for ($j = 0; $j<count($array_interno); $j++) {
            if($array_interno[$j]["nombrecarrera"]==$row_operacion["nombrecarrera"]) {
                $jmal++;
                $array_interno[$j]["codigocarrera"] = $row_operacion["codigocarrera"];
...
$row_operacion["codigocarrera"] = "";
    ...
    $row_operacion["Egresados"] = $sumamatriculados;
...
unset($filacarreras);
$_REQUEST['idencuesta'] = 52;
$_REQUEST['tablarespuesta'] = "respuestainstitucionalegresado";
