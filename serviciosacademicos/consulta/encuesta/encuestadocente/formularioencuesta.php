...
if(!$objetoconsultapregunta->validausuarioencuesta()){
//exit();
 alerta_javascript("No puede realizar encuesta");
echo "<META HTTP-EQUIV='Refresh' CONTENT='0;URL=ingresoestudiante.php'>";
exit();
}
...
echo  "</pre>";
*/
$datosencuesta=$objetobase->recuperar_datos_tabla("encuesta","idencuesta",$idencuestaaleatorio,"","",0);

echo "<table border=\"1\" cellpadding=\"1\" cellspacing=\"0\" bordercolor=\"#E9E9E9\" width=\"750\">";
/*
<b>La autoevaluacion es un proceso de reflexion interna, de autoestudio colectivo, que permite detectar oportunidades de mejora en aquello que la Universidad quiere hacer, lo que hace, la forma en que controla que lo esta logrando y su capacidad de cambio e innovacion</b>.<br><br>  Este cuestionario tiene como fin obtener informacion actualizada sobre algunos aspectos de la Universidad. A continuacion encontrara una serie de preguntas o frases para que usted responda. Por favor marque con una (X) sobre el lugar que mas se aproxime  o aleje de la frase o respuesta formulada y que usted considere la mas adecuada, teniendo en cuenta su opinion, conocimiento o experiencia.  Recuerde que no hay respuestas malas o buenas y la informacion es absolutamente confidencial. En una cultura de calidad no pensamos en cosas bien o mal hechas. Pensamos en oportunidades de mejora.<br> <b>Su participacion es muy importante, de esta depende el exito del ejercicio de autoevaluacion</b>
*/
if(isset($datosencuesta["titulo1encuesta"])&&trim($datosencuesta["titulo1encuesta"])!=''){

$stringtitulo=$datosencuesta["descripcionencuesta"];

$formulario->dibujar_fila_titulo("<b><BR>".$datosencuesta["titulo1encuesta"]."<BR><BR></b>",'tdtituloencuesta',"2","align='center'","td");

//if($codigotipousuario=="500")
$formulario->dibujar_fila_titulo("<b><br>".$datosencuesta["titulo2encuesta"]."<br><br></b>","tdtituloencuestadescripcion","2","align='left'","td");

$formulario->dibujar_fila_titulo($stringtitulo,'tdtituloencuestadescripcion',"2","align='left'","td");

}
else{
$stringtitulo="<B>La autoevaluación es un proceso de reflexión interna, de autoestudio colectivo, que permite detectar oportunidades de mejora en aquello que la Universidad quiere hacer, lo que hace, la forma en que controla que lo está logrando y su capacidad de cambio e innovación</B>.<BR><BR>Este cuestionario tiene como fin obtener información actualizada sobre algunos aspectos de la Universidad. A continuación encontrará una serie de preguntas o frases para que usted responda. Por favor marque con una (X) sobre el lugar que más se aproxime  o aleje de la frase o respuesta formulada y que usted considere la más adecuada, teniendo en cuenta su opinión, conocimiento o experiencia. 
Recuerde que no hay respuestas malas o buenas y la información es absolutamente confidencial. En una cultura de calidad no pensamos en cosas bien o mal hechas. Pensamos en oportunidades de mejora.<BR> <B>Su participación es muy importante, de ésta depende el éxito del ejercicio de autoevaluación</B>";

	$formulario->dibujar_fila_titulo('<b><BR>PROCESO DE AUTOEVALUACIÓN INSTITUCIONAL<BR><BR></b>','tdtituloencuesta',"2","align='center'","td");

if($codigotipousuario=="500")
$formulario->dibujar_fila_titulo('<b><br>Las preguntas que se encuentran a continuación corresponden a una muestra del instrumento original que está compuesto por 221 preguntas. Esta seleccìón se ha realizado con el fin de facilitar el diligenciamiento.<br><br></b>','tdtituloencuestadescripcion',"2","align='left'","td");
...
foreach($objetoconsultapregunta->preguntasencuesta as $llave=>$idpregunta ){
			...
			$fila["numerodocumento"]=$idusuario;
			$fila["valorrespuestadetalleencuestapreguntadocente"]="";
...
if(isset($_POST["Guardar"])){
	//if($formulario->valida_formulario()){
		foreach($_POST["preguntas"] as $llave=>$idpregunta){
			...
			//$fila["idpregunta"]=$idpregunta;
			$fila["numerodocumento"]=$idusuario;
			$fila["valorrespuestadetalleencuestapreguntadocente"]="".$_POST[$idpregunta];
			$fila["codigoestado"]="100";
...
if(isset($_POST["Finalizar"])){
	...
	if($validacion==false){
		...
		echo "<META HTTP-EQUIV='Refresh' CONTENT='0;URL=formularioencuesta.php?idusuario=".$_GET["idusuario"]."&codigotipousuario=".$_GET["codigotipousuario"]."'>";

	}
	else{
		alerta_javascript("Gracias por su colaboración, sus respuestas son utiles para el mejoramiento de nuestra Institución .\\n Puede continuar");
		
		if(isset($_SESSION["tipoentradasesionencuesta"])&&trim($_SESSION["tipoentradasesionencuesta"])!='')
