...
function encuentra_array_materias($idencuesta, $tablarespuesta, $objetobase, $imprimir=0) {
 ...
 left join carrera c on c.codigocarrera = r.codigocarrera and c.codigomodalidadacademica ='200'
...
and r2.idencuestapregunta=ep2.idencuestapregunta
  ...
  and r.numerodocumento  in (select idestudiantegeneral from estudiante where semestre=1 and idestudiantegeneral =r.numerodocumento)
 ...
 * */
    ...
    $query="select c.nombrecarrera,count( distinct e.codigoestudiante) cuentadocente from ordenpago o, detalleordenpago d, estudiante e, carrera c, concepto co, prematricula pr,estudiantegeneral eg 
...
order by c.nombrecarrera";
    ...
    while ($rowresultadodocente = $rescarrerasdocente->fetchRow()) {
        ...
        $array_interno[$i]["Estudiantes"] = $rowresultadodocente["cuentadocente"];
...
//$row_operacion=$operacion->fetchRow();
    while ($row_operacion = $operacion->fetchRow()) {
        ...
        if(isset($row_operacion["nombrecarrera"])&&
                trim($row_operacion["nombrecarrera"])!=''){
        $codigocarrera = $row_operacion["codigocarrera"];

        $row_operacion["Estudiantes"] = $datosnombresegresado["cuentadocente"];
...
unset($filacarreras);
$_REQUEST['idencuesta'] = 54;
$_REQUEST['tablarespuesta'] = "respuestaestudiantesnuevos";
