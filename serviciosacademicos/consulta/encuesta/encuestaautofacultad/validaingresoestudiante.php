...
/*$query="select codigomodalidadacademica from estudiantegeneral eg join estudiante e on eg.idestudiantegeneral=e.idestudiantegeneral join carrera c on e.codigocarrera=c.codigocarrera where eg.idestudiantegeneral='".$_GET['idusuario']."' group by codigomodalidadacademica";
$operacion = $objetobase->conexion->query($query);
$row_operacion = $operacion->fetchRow();

$url_encuesta="encuestaestudiantesnuevos/encuestaestudiantesnuevos.php";
$condicion_aux="'200'";
$condicion_aux2="AND e.codigoperiodo='20112'";
if($row_operacion['codigomodalidadacademica']==300) {*/
	$_GET["codigotipousuario"]=620;
	$_GET["idencuesta"]=50;
...
if($datosnombresegresado=$objetobase->recuperar_datos_tabla("ordenpago o, detalleordenpago d, estudiante e, carrera c, concepto co, prematricula pr,estudiantegeneral eg","eg.idestudiantegeneral",$_GET['idusuario'],$condicion,'',1)) {
    $siga=1;
...
if($siga) {
    ...
    if($diligenciarencuesta) {
        //echo "IN1";
        //exit();
        echo "<META HTTP-EQUIV='Refresh' CONTENT='0;URL=../$url_encuesta?idencuesta=".$_GET["idencuesta"]."&idusuario=".$_GET["idusuario"]."&codigotipousuario=".$_GET["codigotipousuario"]."&codigocarrera=$codigocarrera'>";
       ...
       // echo "IN2";
        ...
        if($completoencuesta) {
            if($_POST["tipousuario"]!="700") {
                ...
                echo "<META HTTP-EQUIV='Refresh' CONTENT='0;URL=../$url_encuesta?idencuesta=".$_GET["idencuesta"]."&tipoentrada=1&idusuario=".$_GET["idusuario"]."&codigotipousuario=".$_GET["codigotipousuario"]."&codigocarrera=$codigocarrera'>";
                //alerta_javascript("formularioencuesta.php?idusuario=".$_GET["idusuario"]."&codigotipousuario=".$_GET["codigotipousuario"]);
            }
        }
        else
