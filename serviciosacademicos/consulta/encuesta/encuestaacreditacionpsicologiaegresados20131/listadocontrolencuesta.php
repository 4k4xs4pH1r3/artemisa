...
/*	$query="select	 c.codigocarrera
			,c.nombrecarrera
			,count(e.codigoestudiante) egresados
	...
	$query="select	 c.codigocarrera
			,c.nombrecarrera
			,count(distinct e.codigoestudiante) egresados
		from registrograduado rg 
		join estudiante e using (codigoestudiante) 
		join carrera c using(codigocarrera) 
		where codigocarrera in (133,134)
		group by c.codigocarrera
			,c.nombrecarrera
		order by c.nombrecarrera ";
	*/
	echo $query="SELECT	 codigocarrera
			,nombrecarrera
			,COALESCE(nuevos,0)+COALESCE(antiguos,0) AS egresados 
		FROM carrera
		JOIN (	
			...
			From (	select	 distinct 
				...
				where codigocarrera in (133,134,126)
			) s1 
			Group By codigocarrera
		) sub1 USING(codigocarrera)
		LEFT JOIN (	
			Select	 codigocarrera
				,count(*) As antiguos 
			From (	select	 distinct 
				...
				where codigocarrera in (126,133,134) 
			) s2 
			Group By codigocarrera
...
unset($filacarreras);
$_REQUEST['idencuesta'] =79;
$_REQUEST['tablarespuesta'] = "respuestaegresadospsicologia20131";
