...
$query ="select c.nombrecarrera, count(distinct numerodocumentodirectivo) cuentadocente from directivo d join carrera c on d.codigocarrera=c.codigocarrera where c.codigomodalidadacademica like '2%' group by c.codigocarrera order by c.nombrecarrera";


/*
    $query="select c.nombrecarrera, count(distinct numerodocumento) cuentadocente
from (
select g.numerodocumento,m.codigocarrera from
grupo g, materia m, carrera c where g.codigoperiodo='20112'
and g.codigomateria = m.codigomateria
and g.codigoestadogrupo like '1%'
and c.codigocarrera=m.codigocarrera
and c.codigomodalidadacademica like '2%'
and g.numerodocumento <> 1
and m.numerocreditos = (select max(m2.numerocreditos) maxcreditos from grupo g2, materia m2 ,
carrera c2 where  c2.codigocarrera=m2.codigocarrera and g2.codigoperiodo='20112' and
g2.codigomateria = m2.codigomateria and g2.numerodocumento=g.numerodocumento
and c2.codigomodalidadacademica like '2%' ) group by numerodocumento) t,
 carrera c where 1= '1' and t.codigocarrera=c.codigocarrera AND c.codigocarrera in (90,93)
 group by c.codigocarrera
order by c.nombrecarrera";*/
    ...
    while ($rowresultadodocente = $rescarrerasdocente->fetchRow()) {
        ...
        $array_interno[$i]["Docentes"] = $rowresultadodocente["cuentadocente"];

        $sumamatriculados+=$rowresultadodocente["cuentadocente"];
        $i++;
    }
...
//$row_operacion=$operacion->fetchRow();
    ...
    $row_operacion["Docentes_aprox"] = $sumamatriculados;
...
unset($filacarreras);
$_REQUEST['idencuesta'] = 58;
$_REQUEST['tablarespuesta'] = "respuestadirectivos";
...
$motor = new matriz($cantidadestmparray, "HISTORIAL LINEA ENFASIS ", "listadocontrolencuesta.php", 'si', 'si', 'menulistadocontorlencuesta.php', 'menulistadocontorlencuesta.php', false, "si", "../../../");
