...
$query_datos = "SELECT g.nombregenero,eg.idestudiantegeneral, concat(eg.nombresestudiantegeneral, ' ',eg.apellidosestudiantegeneral) as nombre, eg.numerodocumento,  r.tituloregistrograduadoantiguo, r.numeroactaregistrograduadoantiguo, r.fechagradoregistrograduadoantiguo, r.numerodiplomaregistrograduadoantiguo, d.tipodocumento,d.nombredocumento
            ...
            union
            SELECT  g.nombregenero,eg.idestudiantegeneral, concat(eg.nombresestudiantegeneral, ' ',eg.apellidosestudiantegeneral) as nombre, eg.numerodocumento, t.nombretitulo, r.numeroactaregistrograduado, r.fechagradoregistrograduado, r.numerodiplomaregistrograduado,d.tipodocumento,d.nombredocumento
...
?>
    <script type="text/javascript">
        ...
        window.location.href='certificadoegresado.php';
    </script>
<?php
...
/*IDENTIFICADO*/
    ...
    $pdf->SetY(52);
    $pdf->SetX(20);
    $pdf->Cell(0, 0, 'Oficina de Registro y Control Académico ', 0, 2, 'C');
    $pdf->Ln(10);
    $pdf->SetX(20);
    $pdf->Cell(0, 0, 'VERIFICACIÓN DE TÍTULO ', 0, 2, 'C');
    $pdf->SetY(75);
    $pdf->SetX(20);
    $pdf->SetFontSize(12);
    $pdf->MultiCell(0, 5.5, 'La Secretaría General certifica que verificado el sistema académico de la universidad, se encuentra el registro de '.$trato.' '  . $row_datos['nombre'] . ' '.$identificado.' con '. $row_datos['nombredocumento'] . ' Nº ' . $row_datos['numerodocumento'] . ', quien cursó y aprobó los estudios y obtuvo el título respectivo en esta institución conforme a la siguiente información:', 0, 'J');
    $pdf->Ln(10.5);
    $pdf->SetX(25);
    $pdf->SetFontSize(10);
    ...
    $pdf->SetX(25);
    $pdf->celda(25, 5.5, 'DIPLOMA Nº', 150, 'J', 1, 1);
    $pdf->SetY(107.5);
    $pdf->SetFontSize(9);
   $datosestudiante=$row_datos;
    ...
    do {
        ...
        $pdf->Ln(5.5);
        $pdf->SetX(25);
        $pdf->celda(80, 5.5, $row_datos['tituloregistrograduadoantiguo'], 0, 'C', 1, 1);

        if ($pdf->GetStringWidth($row_datos['tituloregistrograduadoantiguo']) > 80) {
    ...
    $pdf->SetX(20);
    $pdf->SetFontSize(12);
    $pdf->MultiCell(0, 5.5, 'Bogotá, D.C., ' . $dias[date('w')] . " " . date("d") . " de " . $meses[$mes] . " de " . date("Y,") . " hora " . $hora, 0, 'J');
    $pdf->Ln(15);
    $pdf->SetX(20);
    ...
    $pdf->SetY(70);
    $pdf->SetX(20);
    $pdf->Cell(0, 0, 'Nº: ' . $consecutivo, 0, 2, 'C');
    $pdf->Output();
}
?>
