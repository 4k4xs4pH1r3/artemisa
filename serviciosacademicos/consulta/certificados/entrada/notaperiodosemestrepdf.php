...
{
	echo $_SESSION['codigo'] = $_GET['codigo'];
}
...
if($_GET['tipocertificado'] == "todo" or $_GET['tipocertificado'] == "pasadas")
{
	...
	echo     $query_historico = "SELECT 	c.nombrecortocarrera,eg.expedidodocumento,ti.nombretitulo,eg.numerodocumento,
...
<?php
	   ...
	   if($row_tipousuario['codigotipousuariofacultad'] == 200 and !isset($_GET['periodos']))
		{
?>
		 EL SUSCRITO SECRETARIO GENERAL</span><br>
...
<?php
	    ...
	    {
?>
	     <span class="Estilo4"><strong><a class="Estilo4"style="cursor: pointer" onClick="window.location.reload('../../prematricula/matriculaautomaticaordenmatricula.php?programausadopor=facultad')">EL SUSCRITO</a></strong></span><br>
...
<?php
		...
		if (! isset ($_GET['totalperiodos']) )
		 {
		   echo $query_historicoperiodos = "SELECT distinct n.codigoperiodo, p.nombreperiodo, e.codigosituacioncarreraestudiante
						...
						if ($solicitud_historico['codigomateriaelectiva'] <> "" and $solicitud_historico['codigomateriaelectiva'] <> "1")
						  {
							...
							$totalRows_mCARGOateriaelectiva1 = mysql_num_rows($materiaelectiva1);
...
?>
    <tr>
    <td colspan="2"><span class="Estilo2 Estilo1"><strong>Promediofdsdfddd Ponderado Semestral Acumulado</strong></span></td>
<?php
                        if($_GET['concredito'] == 1)
                        {
?>
...
<?
	...
	require('../convertirnumeros.php');
	echo $dias;
?> (<?php echo $dia;?>) d&iacute;as del mes de <?php echo $meses;?> (<?php echo $mes;?>) del a&ntilde;o dos mil <?php $day = substr($ano,2,4); require('../convertirnumeros.php'); echo $dias; ?> (<?php echo $ano;?>).</p>
