      ...
      <tr bgcolor='#FEF7ED'>
        ...
        if ($_POST['modalidad'] <> 0)
         {  // if 1  
			...
			$_SESSION['codigodocente']= $_POST['modalidad'];
...
?>
              <select name="especializacion" id="select" onChange="enviar()">
...
?>
              <select name="periodo" onChange="enviar()">
                <option value="0" <?php if (!(strcmp("0", $_POST['grupito']))) {echo "SELECTED";} ?>>Seleccionar</option>
                <?php
				do {  
?>
                <option value="<?php echo $row_periodo['idgrupo']?>"<?php if (!(strcmp($row_periodo['idgrupo'], $_POST['grupito']))) {echo "SELECTED";} ?>>
                <?php $row_periodo['idgrupo']?>
                </option>
                <?php
  ...
  if ($_POST['grupito'] <> 0)
	...
	echo $_POST['grupito'];
	echo "<script>window.location.reload('resultado14.php');</script>";
		} 
?>
