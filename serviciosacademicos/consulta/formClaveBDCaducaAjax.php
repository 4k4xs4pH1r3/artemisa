...
$queryPoliticaClave="SELECT pc.* FROM politicaclave pc WHERE '$fechahoy' between pc.fechadesdepoliticaclave and pc.fechahastapoliticaclave";
...
$queryVerificaClaveCaducada="SELECT cu.* FROM claveusuario cu WHERE cu.idusuario='".$_GET['idusuario']."' ORDER BY cu.idclaveusuario DESC LIMIT 1";
...
if($numRowClaveCaduca > 0){
	...
	//print_r($rowClaveCaduca);die;
        
        /*
         * @modified Andres Ariza <arizaandres@unbosque.edu.co>
         * Se modifican las condiciondes de valicacion de and por || ademas se agrega el estado 500 referente a "Desbloqueo por tecnologia"
         * que junto con 200 (Clave caduco) y 400 (Desbloqueo por el usuario) cubren todas las posibilidades de recuperacion de clave
         * Caso reportado por Diana Rojas - Registro y Control Académico <dianarojas@unbosque.edu.co> Fecha: 27 de marzo de 2018, Caso Mesa  99319
         * @since  Abril 5, 2018
        */
	//if($rowClaveCaduca['codigoestado']==200 and $rowClaveCaduca['codigoindicadorclaveusuario']==400){
        if($rowClaveCaduca['codigoestado']==200 || $rowClaveCaduca['codigoindicadorclaveusuario']==400 || $rowClaveCaduca['codigoindicadorclaveusuario']==400){
		$desbloq=true;
	}
	else{
		$desbloq=false;
