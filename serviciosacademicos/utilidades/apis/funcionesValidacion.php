...
function CreatTokenApp($db,$idusuario,$usuarioname){
        $fecha = date("Y-m-d H:i:s");	
        ...
        $sql = "INSERT INTO `Token` (`idUsuario`, `token`, `FechaCreacion`, `CodigoEstado`, `FechaModificacion`) 
        VALUES ('".$idusuario."', '".$Token."', '".$fecha."', '100', '".$fecha."')";	
        $db->Execute($sql);
    
    return $Token;
}
function autenticarUsuario($usuario,$clave,$segunda=false,$rol=false){
        ...
        if($rol==1){  
            ...
            //echo "<pre>";print_r($autenticacion);
     		...
     		if($autenticacion->autenticacionAutorizada==1){
				...
				foreach($documentos as $doc){					
                 ...
                 $json["token"] = CreatTokenApp($db,$json["idusuario"],$autenticacion->usuario);
            ...
            if($admin){
                ...
                if($autenticacion->autenticacionAutorizada==1){
                    ...
                    $json["tipodocumento"]        = ' ';
                    $json["fullname"]                  = ' ';
                    $json["token"]                      = CreatTokenApp($db,$json["idusuario"],$usuario);
