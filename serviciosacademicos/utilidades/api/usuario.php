    ...
    if (intval($user["codigorol"]) == 1 && intval($user["codigotipousuario"]) == 600 || intval($user["codigotipousuario"]) == 900) {
        ...
        $SQL = 'SELECT c.codigocarrera, c.nombrecarrera  
            ...
            INNER JOIN usuario u ON (u.usuario=uf.usuario) 
             WHERE u.idusuario = ' . $idusuario . '  AND uf.codigoestado = 100  
                    ...
                    if (isset($usuario) && isset($clave) && $usuario != "" && $clave != "") {
                        switch ($Tipo_acceso) {
                            case 1: {/*                                 * **Autenticacion Inicial**** */
                                    $json = autenticarUsuario($usuario, $clave, false, $rol, $plataforma, $devicetoken);
                                }break;
                            case 2: {/*                                 * **Autenticacion Segunda clave***** */
                                    $json = autenticarUsuario($usuario, $clave, true, $rol, $plataforma, $devicetoken);
                                }break;
                        }//switch interno						
                    } else {
                        $json["result"] = "ERROR";
