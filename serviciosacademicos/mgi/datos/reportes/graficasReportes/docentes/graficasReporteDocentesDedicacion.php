...
function getValores($db,$codigoperiodo,$utils){
    ...
    if(!($row!=NULL && count($row)>0)){
        ...
        if($row!=NULL && (count($row)>0)){
            $total1 += $row["numAcademicos14T"];
            $total2 += $row["numAcademicosMT"];
            $total3 += $row["numAcademicos34T"];
            $total4 += $row["numAcademicosTC"];
        }
    ...
    $valores[] = $total3;
    $valores[] = $total4;
    $labelCategorias = array("1/4 Tiempo","1/2 Tiempo","3/4 Tiempo","Tiempo Completo");
           //var_dump($valores);
...
//$myPicture->drawGradientArea(0,0,700,230,DIRECTION_VERTICAL,array("StartR"=>240,"StartG"=>240,"StartB"=>240,"EndR"=>180,"EndG"=>180,"EndB"=>180,"Alpha"=>100));
 ...
 $myPicture->drawLegend(510,12,array("Style"=>LEGEND_NOBORDER,"Mode"=>LEGEND_HORIZONTAL));
   ...
   //$myPicture->drawLegend($ancho-240,9,array("Style"=>LEGEND_NOBORDER,"Mode"=>LEGEND_HORIZONTAL,"FontR"=>255,"FontG"=>255,"FontB"=>255));
    ...
    $myPicture->Render("imagenesGraficas/".$reporte["alias"]."/docentesDedicacion.png"); 
     ?>
        <img alt="Resultados selectividad" src="<?php echo "imagenesGraficas/".$reporte["alias"]."/docentesDedicacion.png?random=".time(); ?>" style="border: 1px solid gray;margin-right: 20px;"/>
 <?php }
