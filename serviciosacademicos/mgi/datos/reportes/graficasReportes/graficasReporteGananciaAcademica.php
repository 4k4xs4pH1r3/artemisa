...
$array_matnuevo=obtenerEstudiantes($db,$periodo,$carrera);
 ...
 if(count($array_matnuevo)>0 && is_array($array_matnuevo)){
    ...
    $data = obtenerMaxSemestreCarrera($db,$carrera);
    ...
    for($i=1; $i<=$maxSemestre; $i++){
        ...
        if($sumaPromedios!=0){
            ...
            $promedios["notas"][] = $sumaPromedios/$totalEstudiantesSemestre;
        }
    }
 }
...
<?php if(!isset($_GET['codigoestudiante']) || $_GET['codigoestudiante']=="") {
 ...
 <?php 
    ...
    $query = "SELECT MAX(p.semestreprematricula) as max from prematricula p 
        INNER JOIN estudiante e ON e.codigoestudiante=p.codigoestudiante AND e.codigocarrera='".$carrera."' 
    where p.codigoestudiante=".$_GET['codigoestudiante']." AND p.codigoestadoprematricula IN (40,41) ";
    $row = $db->GetRow($query);
    $maxSemestre = $row["max"];
    ...
    for($i=1; $i<=$maxSemestre; $i++){
        ...
        $notaspromedio[] = $promediosemestralperiodo;
    }
    /*$query_carrera = "select nombrecarrera
                                from carrera 
                                                    where codigocarrera = '$carrera'";
