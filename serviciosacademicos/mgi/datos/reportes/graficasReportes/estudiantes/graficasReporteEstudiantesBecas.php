...
function getValores($db,$modalidad,$codigoperiodo){
    ...
    while($row_carreras = $carreras->FetchRow()){
        ...
        $sql = "SELECT * from becasbeneficio WHERE codigoestado=100 AND carrera_id='".$row_carreras['codigocarrera']."'";
        $sql .= " AND periodo='".$codigoperiodo."' ORDER BY entrydate DESC";
        $row = $db->GetRow($sql);
        if($row!=NULL && (count($row)>0)){
            $totalAL += $row["academica"];
            $totalE += $row["poblaciones"];
            $totalA += $row["grado"];
            $totalEU += $row["graduandos"];
        }
    ...
    $valores[] = $totalA;
    $valores[] = $totalEU;
    $labelCategorias = array("Becas de excelencia académica","Beca poblaciones especiales","Beca Grado de Honor","Beca graduandos (ECAES)");
           //var_dump($valores);
