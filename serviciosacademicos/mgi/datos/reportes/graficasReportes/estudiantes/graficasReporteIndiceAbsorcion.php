...
$array_matnuevo=$datos_estadistica->obtener_datos_estudiantes_matriculados_nuevos('','arreglo',$modalidad["codigomodalidadacademicasic"]);
$total_matnuevo = count($array_matnuevo);
        $total_admnomat=$datos_estadistica->seguimiento_inscripcionvsmatriculadosnuevos_modalidad($modalidad["codigomodalidadacademicasic"],$codigoperiodo,'conteo',$array_matnuevo);
            ...
            if(($total_admitidos) == 0){
                $indicedos = 0;
                    ...
                    $indicedos=($total_matnuevo/$total_admitidos)*100;	   
            }
            $valores[] = round($indicedos, 2);
            $total_admnomat=0;
            $total_admnoing=0;
            $total_matnuevo=0;
    }
...
function drawGraphic($db,$periodos,$reporte,$modalidad,$fontPath) { 
    ...
    $myPicture->drawText(10,16,"Índice de Absorción",array("FontSize"=>9,"Align"=>TEXT_ALIGN_BOTTOMLEFT));
    ...
    $myPicture->Render("imagenesGraficas/".$reporte["alias"]."/absorcionModalidades.png"); 
    //$myPicture->autoOutput("imagenesGraficas/example.drawLineChart.plots.png"); ?>
        <img alt="Resultados selectividad" src="<?php echo "imagenesGraficas/".$reporte["alias"]."/absorcionModalidades.png?random=".time(); ?>" style="border: 1px solid gray;margin: 0 20px;"/>
 <?php } 
 function drawGraphicModalidad($db,$periodos,$reporte,$modalidad,$carrera,$fontPath) { 
    ...
    /* Render the picture (choose the best way) */
    $myPicture->Render("imagenesGraficas/".$reporte["alias"]."/absorcionModalidadCarrera.png"); 
    //$myPicture->autoOutput("imagenesGraficas/example.drawLineChart.plots.png"); ?>
        <img alt="Resultados selectividad" src="<?php echo "imagenesGraficas/".$reporte["alias"]."/absorcionModalidadCarrera.png?random=".time(); ?>" style="border: 1px solid gray;margin: 0 20px;"/>
 <?php }  
