...
if(empty($_REQUEST['id'])){
    ...
    $url='../reportes/reportes/docentes/viewReporteDedicacionBosquePorNivelFormacion.php';
}else{
    $url='reportes/docentes/viewReporteDedicacionBosquePorNivelFormacion.php';
}
$tipo=1;
	...
	<fieldset>
		<label class="fixedLabel">Mes: <span class="mandatory">(*)</span></label>
                ...
                <?php $utils->getYearsSelect("anio");  ?>
                  <input type="button" value="Consultar" id="consul_report_bnfor1" class="first small" />
		<!--<input type="submit" value="Consultar" class="first small" />-->

	</fieldset>
</form>
		<div id='respuesta_forma1'>
...
if(isset($_REQUEST["mes"]) && isset($_REQUEST["anio"])) { 
    ...
    $nivelFormacion  = $utils->getAll($db,"siq_tipoNivelFormacion","actividad=1 AND codigoestado=100","orden");
    $nivelFormacion1  = $utils->getAll($db,"siq_tipoNivelFormacion","actividad =1 AND codigoestado=100","orden");
    ...
    <table align="center" id="estructuraReporte" class="previewReport" width="92%" >
                    <thead>            
                        <tr class="dataColumns">
                            <th class="column" colspan="9"><span>Dedicación Semanal Docentes<br>(Según categorización Universidad El Bosque), por Mayor Nivel de Formación</span></th>                                    
                        </tr>
...
$(document).ready(function(){
                $("#consul_report_bnfor1").one('click', function() {
                            $.ajax({//Ajax
                              type: 'POST',
                              url: '<?php echo $url?>',
