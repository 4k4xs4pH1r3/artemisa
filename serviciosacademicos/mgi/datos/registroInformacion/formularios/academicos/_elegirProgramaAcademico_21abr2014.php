...
//var_dump($_SESSION);
    ...
    //TOCO CLAVARLE EL OR codigocarrera=6 PARA INCLUIR AL DPTO DE HUMANIDADES
    if(isset($_SESSION['modalidad']) ) {
        getFormacion($_SESSION['modalidad'],$disable);
        $currentdate  = date("Y-m-d H:i:s");
        $query_programa = "SELECT codigocarrera,nombrecarrera FROM carrera WHERE codigomodalidadacademicasic='".$_SESSION['modalidad']."' AND (fechavencimientocarrera>'".$currentdate."' OR codigocarrera='6') ORDER BY nombrecarrera ASC";
        ...
        $currentdate  = date("Y-m-d H:i:s");
        $sql = "SELECT * FROM carrera WHERE codigocarrera=".$_SESSION['codigofacultad']." AND codigomodalidadacademicasic IN (200,300,301,302) AND (fechavencimientocarrera>'".$currentdate."' OR codigocarrera='6')";
        $row = $db->GetRow($sql);
        if(count($row)==0){ 
            getFormacion("");
            getProgramas(array(),"");
        ?>
    <?php } else { 
            $query_programa = "SELECT codigocarrera,nombrecarrera FROM carrera WHERE codigomodalidadacademicasic='".$row["codigomodalidadacademicasic"]."' AND (fechavencimientocarrera>'".$currentdate."' OR codigocarrera='6') ORDER BY nombrecarrera ASC";
            $result =$db->GetAll($query_programa);
