...
if($estaLogueado){
	...
	if((strcmp($action,"getData")==0)){
		...
		if(!isset($_REQUEST["joinField"])){//echo '<pre>';print_r($_REQUEST);
		...
		} else {//echo '<pre>';print_r($_REQUEST);
		...
		if(!isset($_REQUEST["joinField"])){
			...
			"",$_REQUEST["codigocarrera"],null,$_REQUEST["order"],$_REQUEST["modalidad"]);
		} else {
			...
			$_REQUEST["joinField"],$_REQUEST["codigocarrera"],null,null,$_REQUEST["modalidad"]);
		}
		...
		$result = $utils->getDataConsolidada($db,$_REQUEST["entity"],$_REQUEST["campoPeriodo"],$_REQUEST["periodo"],"siq_",$_REQUEST["codigocarrera"],$_REQUEST["planEstudio"],
			$_REQUEST["periodo2"],$_REQUEST["tipo"],$_REQUEST["formato"],$_REQUEST["funcion"],$_REQUEST["idFormulario"], $_REQUEST["pestana"],$_REQUEST["order"],$_REQUEST["modalidad"]);        
	   ...
	   echo json_encode($data);
		...
		$result = $utils->getDataFormFieldsByDate($db,"siq_".$_REQUEST["entity"],$_REQUEST["campoFecha"],$_REQUEST["periodo"],$_REQUEST["codigocarrera"],$_REQUEST["tipo"],$_REQUEST["modalidad"]);
		if(count($result)>0){
			$data = array('success'=> true,'total'=> count($result),'data'=>$result);
			echo json_encode($data);
		} else {
	...
	// Set up associative array
	$data = array('success'=> false,'message'=>'Se ha cerrado la sesión. Por favor salga y vuelva a ingresar a sala.');
