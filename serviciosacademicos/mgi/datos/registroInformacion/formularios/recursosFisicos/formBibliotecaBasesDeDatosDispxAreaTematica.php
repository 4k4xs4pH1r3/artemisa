...
<form action="" method="post" id="forma<?PHP echo $_REQUEST['alias']?>">
	...
	<fieldset>  
		...
		<label for="mes" class="grid-2-12">Año: <span class="mandatory">(*)</span></label>
<?PHP 		
		$utils->getYearsSelect("anio"); 
                 $idForm = "forma".$_REQUEST['alias'];
                 $utils->pintarBotonCargar("popup_cargarDocumento(2,10,$('#".$idForm." #anio').val())","popup_verDocumentos(2,10,$('#".$idForm." #anio').val())");
...
?>
				<tr class="dataColumns category">
					<th colspan="<?php if($aprobacion) echo "2"; else echo "3";?>" class="borderR"><?PHP echo $row2['clasificacionesinfhuerfana']?></th>               
                                        <?php if($aprobacion){ ?>
	...
	?>
					<tr id="contentColumns" class="row">
						...
						<td class="column borderR" align="center"><input type="text" class="required number" name="suscripcion[]" id="suscripcion_<?PHP echo $row['idclasificacionesinfhuerfana']?>" title="Suscripción" maxlength="3" tabindex="1" autocomplete="off" size="6" style='text-align:center' /></td>
						<td class="column borderR" align="center"><input type="text" class="required number" name="open_access[]" id="open_access_<?PHP echo $row['idclasificacionesinfhuerfana']?>" title="Open access" maxlength="3" tabindex="1" autocomplete="off" size="6" style='text-align:center' /></td>
					</tr>
	<?PHP 
				}
			}
	...
	<input type="submit" value="Guardar cambios" id="enviar10" name="enviar10" class="first" />
</form>
<script type="text/javascript">
var aprobacion = '<?php echo $aprobacion; ?>';  
    getData10("#forma<?PHP echo $_REQUEST['alias']?>");
    
                $('#enviar10').click(function(event) {
                    event.preventDefault();
                    var valido= validateForm("#forma<?PHP echo $_REQUEST['alias']?>");
                    if(valido){
                        sendForm10("#forma<?PHP echo $_REQUEST['alias']?>");
                    }
                });
                                   ...
                                   // $("#id").val(data.message);
                                    for (var i=0;i<data.total;i++){                                  
                                      //  alert(data[i].id+'-'+data[i].idclasificacionesinfhuerfana+'-'+data[i].cantidad+'-->>')
                                        ...
                                        $(formName + " #suscripcion_"+data[i].idclasificacionesinfhuerfana).val(data[i].suscripcion);
                                        $(formName + " #open_access_"+data[i].idclasificacionesinfhuerfana).val(data[i].open_access);
                                        if(data[i].Verificado=="1"){ 
                                           ...
                                           $(formName + " #enviar10").attr('disabled','disabled');
