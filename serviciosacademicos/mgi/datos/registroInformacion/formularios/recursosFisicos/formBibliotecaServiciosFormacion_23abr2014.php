...
<?		
		...
		$utils->getYearsSelect("anio"); 
                 ...
                 $utils->pintarBotonCargar("popup_cargarDocumento(2,6,$('#".$idForm." #mes').val()+'-'+$('#".$idForm." #anio').val())","popup_verDocumentos(2,6,$('#".$idForm." #mes').val()+'-'+$('#".$idForm." #anio').val())");
...
?>
				<tr class="dataColumns category">
					<th colspan="<?php if($aprobacion) echo "4"; else echo "5"; ?>" class="borderR"><?=$row2['clasificacionesinfhuerfana']?></th>               
                                        <?php if($aprobacion){ ?>
	...
	?>
					<tr id="contentColumns" class="row">
						...
						<td class="column borderR" align="center"><input type="text" class="required number" name="induccion_biblioteca[]" id="induccion_biblioteca_<?=$row['idclasificacionesinfhuerfana']?>" title="Inducción Biblioteca" maxlength="3" tabindex="1" autocomplete="off" size="6" style='text-align:center' /></td>
						<td class="column borderR" align="center"><input type="text" class="required number" name="busqueda_informacion[]" id="busqueda_informacion_<?=$row['idclasificacionesinfhuerfana']?>" title="Búsqueda de información" maxlength="3" tabindex="1" autocomplete="off" size="6" style='text-align:center' /></td>
						<td class="column borderR" align="center"><input type="text" class="required number" name="seminario_taller[]" id="seminario_taller_<?=$row['idclasificacionesinfhuerfana']?>" title="Seminario Taller" maxlength="3" tabindex="1" autocomplete="off" size="6" style='text-align:center' /></td>
						<td class="column borderR" align="center"><input type="text" class="required number" name="referenciacion[]" id="referenciacion_<?=$row['idclasificacionesinfhuerfana']?>" title="Referenciación" maxlength="3" tabindex="1" autocomplete="off" size="6" style='text-align:center' /></td>
					</tr>
	<?
				}
			}
	...
	<input type="submit" value="Guardar cambios" id="enviar7" name="enviar7" class="first" />
</form>
<script type="text/javascript">
var aprobacion = '<?php echo $aprobacion; ?>';  
    getData7("#forma<?=$_REQUEST['alias']?>");
    
                $('#enviar7').click(function(event) {
                    event.preventDefault();
                    var valido= validateForm("#forma<?=$_REQUEST['alias']?>");
                    if(valido){
                        sendForm7("#forma<?=$_REQUEST['alias']?>");
                    }
                });
                                   ...
                                   // $("#id").val(data.message);
                                    for (var i=0;i<data.total;i++){                                  
                                      //  alert(data[i].id+'-'+data[i].idclasificacionesinfhuerfana+'-'+data[i].cantidad+'-->>')
                                        ...
                                        $(formName + " #induccion_biblioteca_"+data[i].idclasificacionesinfhuerfana).val(data[i].induccion_biblioteca);
                                        $(formName + " #busqueda_informacion_"+data[i].idclasificacionesinfhuerfana).val(data[i].busqueda_informacion);
                                        $(formName + " #seminario_taller_"+data[i].idclasificacionesinfhuerfana).val(data[i].seminario_taller);
                                        $(formName + " #referenciacion_"+data[i].idclasificacionesinfhuerfana).val(data[i].referenciacion);
                                        ...
                                        if(data[i].Verificado=="1"){ 
                                           ...
                                           $(formName + " #enviar7").attr('disabled','disabled');
