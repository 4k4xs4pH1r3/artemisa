...
if(($_POST['semestre'])) {
	$query_continuada_au="select * from siq_tecnologia where codigoperiodo=".$_POST['semestre']."";
...
?>
<form action="" method="post" id="form1_au">
	<span class="mandatory">* Son campos obligatorios</span>
	<fieldset>  
		<legend>Aulas Virtuales</legend>
		<?php
		$query_papa = "select idclasificacionesinfhuerfana,clasificacionesinfhuerfana from siq_clasificacionesinfhuerfana where aliasclasificacionesinfhuerfana='T_NAV'";
        $papa= $db->Execute($query_papa);
        $totalRows_papa = $papa->RecordCount();
...
$(':submit').click(function(event) {
	...
	var valido= validateForm("#form1_au");
	if(valido){
		sendForm_un();
	}
});
function sendForm_un(){
	$.ajax({
		type: 'POST',
		url: './formularios/recursosFisicos/ViewTecnologiaAulas.php',
		async: false,
		data: $('#form1_au').serialize(),                
