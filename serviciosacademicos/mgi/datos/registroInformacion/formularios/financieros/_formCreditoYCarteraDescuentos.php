                ...
                <table align="center" class="formData last" width="92%" >
                    <thead>            
                        <tr class="dataColumns">
                            <th class="column" colspan="3"><span>Estudiantes que se han Beneficiado por Descuentos</span></th>                                    
                        </tr>
                     ...
                     <tbody>
                         <?php while ($row = $actividades->FetchRow()) { ?>
                            ...
                            <tr class="dataColumns">
                                ...
                                <td><input type="checkbox" id="Valida_<?php echo $row["idsiq_tipoDescuento"]; ?>" name="Valida_<?php echo $row["idsiq_tipoDescuento"]; ?>" onclick="CheckValida(<?php echo $row["idsiq_tipoDescuento"]; ?>)" /></td>
                            </tr>
   
                        <?php } ?> 
                        <input type="hidden" id="DataValida" /> 
...
<script type="text/javascript">
    getDatadescuentos();
       ...
       /* $(document).ready(function(){  
                            ...
                            if (data.success == true){
                               ...
                               var DataValida = $('#DataValida').val();
                                ...
                                $.ajax({//Ajax
                    					  ...
                    					  async: false,
                    					  dataType: 'json',
                    					  data:({actionID:'form_descuentosValidar',Data_LastId:data.data,DataValida:DataValida}),
         ...
         function CheckValida(id){ 
            /*******************************/
            
            if(!$.trim($('#valor_'+id).val())){
                alert('Para Poder Validar Informacion debe estar lleno el valor...!');
                $('#Valida_'+id).attr('checked',false);
                $('#valor_'+id).effect("pulsate", {times:3}, 500);
                $('#valor_'+id).css('border-color','#F00');
                return false;
            }else{
                
                var DataValida = $('#DataValida').val();
            
                if($('#Valida_'+id).is(':checked')){
                    $('#DataValida').val($('#DataValida').val()+'::'+id);
                }else{
                    var Cambiar       = '::'+id;
                    var DelecteValida = DataValida.replace(Cambiar,'');
                    $('#DataValida').val('');
                    $('#DataValida').val(DelecteValida);
                }//if    
