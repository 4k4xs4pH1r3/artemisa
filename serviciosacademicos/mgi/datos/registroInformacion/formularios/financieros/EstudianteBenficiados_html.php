...
switch($_REQUEST['actionID']){
			...
			/***************************************/
				...
				/**********************************************************************/
					...
					*/
					...
					//$SQL_Insert='INSERT INTO estudiantebeneficio(modalidadsic_id,carrera_id,apoyos,paricipacion,egresados,aspirantes,periodo,entrydate,userid) VALUES ("'.$id_modalidad.'","'.$Datos[0].'","'.$Datos[1].'","'.$Datos[2].'","'.$Datos[3].'","'.$Datos[4].'","'.$Periodo.'",NOW(),"'.$userid.'")';
					$SQL_Insert='UPDATE estudiantebeneficio set  modalidadsic_id="'.$id_modalidad.'",carrera_id="'.$Datos[0].'",apoyos="'.$Datos[1].'",paricipacion="'.$Datos[2].'",egresados="'.$Datos[3].'",aspirantes="'.$Datos[4].'",periodo="'.$Periodo.'",entrydate=NOW(),userid="'.$userid.'" where modalidadsic_id="'.$id_modalidad.'" and carrera_id="'.$Datos[0].'" and periodo="'.$Periodo.'" and codigoestado=100';
						if($BecasBeneficio=&$db->Execute($SQL_Insert)===false){
								$a_vectt['val']			='FALSE';
								$a_vectt['descrip']		='Error al Insertar las Becas Beneficio.....'.$SQL_Insert;
								echo json_encode($a_vectt);    
	...
	case 'Buscar':{
		global $userid,$db,$C_EstudianteBenficiados;
			...
			$C_EstudianteBenficiados->DibujaTabla($_GET['id_modalidad'],$_GET['id_periodo']);
		}break;
...
function JsGeneral(){
	...
	function buscarDatos(){
		var modalidadBuscar = $('#Finaciero #id_Modalidad').val();
		var periodoBuscar = $('#Finaciero #Periodo').val();
