...
switch($_REQUEST['actionID']){
			...
			/***************************************/
				...
				/**********************************************************************/
					...
					*/
					...
					//$SQL_Insert='INSERT INTO becasbeneficio(modalidadsic_id,carrera_id,academica,poblaciones,grado,graduandos,periodo,entrydate,userid) VALUES ("'.$id_modalidad.'","'.$Datos[0].'","'.$Datos[1].'","'.$Datos[2].'","'.$Datos[3].'","'.$Datos[4].'","'.$Periodo.'",NOW(),"'.$userid.'")';
					$SQL_Insert='UPDATE becasbeneficio set  modalidadsic_id="'.$id_modalidad.'",carrera_id="'.$Datos[0].'",academica="'.$Datos[1].'",poblaciones="'.$Datos[2].'",grado="'.$Datos[3].'",graduandos="'.$Datos[4].'",periodo="'.$Periodo.'",entrydate=NOW(),userid="'.$userid.'" where modalidadsic_id="'.$id_modalidad.'" and carrera_id="'.$Datos[0].'" and periodo="'.$Periodo.'" and codigoestado=100';
						if($BecasBeneficio=&$db->Execute($SQL_Insert)===false){
								$a_vectt['val']			='FALSE';
								$a_vectt['descrip']		='Error al Insertar las Becas Beneficio.....'.$SQL_Insert;
								echo json_encode($a_vectt);    
				...
				/**************************************************/
					$a_vectt['val']			='TRUE';
					$a_vectt['descrip']		='Se ha Actualizado la Información Correctamente...'.$Periodo;
					echo json_encode($a_vectt);    
					exit;
				/**************************************************/
				}
	...
	case 'Buscar':{
		global $userid,$db,$C_BecasBeneficio;
			...
			$C_BecasBeneficio->DibujaTabla($_GET['id_modalidad'],$_GET['id_periodo']);
		}break;
...
function JsGeneral(){
	...
	function buscarDatos(){
		var modalidadBuscar = $('#xxxxx #id_Modalidad').val();				
		var periodoBuscar = $('#xxxxx #Periodo').val();
