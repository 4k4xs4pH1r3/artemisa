...
<form action="" method="post" id="biblio">
<input type="hidden" name="action" value="biblio" id="action" />
...
<fieldset>
  <?php
  $query_papa = "select idclasificacionesinfhuerfana,clasificacionesinfhuerfana from siq_clasificacionesinfhuerfana where aliasclasificacionesinfhuerfana='P_BIBLIO'";
  ...
  $utils->pintarBotonCargar("popup_cargarDocumento(13,4,$('#biblio #anio').val())","popup_verDocumentos(13,4,$('#biblio #anio').val())"); ?>
  <table align="center" id="estructuraReporte"  class="formData last" width="92%">
    <thead>                   
      <tr id="dataColumns">
        <th class="column borderR" ><span>Material</span></th>
...
<input type="hidden" name="formulario" value="biblio" />
<input type="hidden" name="padre" value="<?php echo $row_papa['idclasificacionesinfhuerfana']; ?>" />
<input type="submit" id="enviabiblio" value="Registrar datos" class="first" />
</form>

<script type="text/javascript">
$(function(){
  $("#biblio input[type='text']").maskMoney({allowZero:true,thousands:',', decimal:'.',precision:0,allowNegative:false, defaultZero:false});
});

//Llama a la función obtener datos al iniciar el formulario
getformulario_biblio("#biblio");
//fin llamado

$('#biblio #anio').bind('change', function(event) {
  getformulario_biblio("#biblio");
});

$('#enviabiblio').click(function(event) {
  event.preventDefault();
  replaceCommas("#biblio");
  var valido= validateForm("#biblio");
  if(valido){
    biblioteca();
  }
});

function getformulario_biblio(forname){
...
function biblioteca(){//$('#form_test').serialize()
...
$.ajax({//Ajax
  ...
  async: false,
  dataType: 'json',
  data:$('#biblio').serialize(),
  error:function(objeto, quepaso, otroobj){alert('Error de Conexión , Favor Vuelva a Intentar');},
  success:function(data){
    if (data.success == true){            
      $('#biblio #msg-success').html('<p>' + data.descrip + '</p>');
      $('#biblio #msg-success').removeClass('msg-error');
      $('#biblio #msg-success').css('display','block');
      $("#biblio #msg-success").delay(5500).fadeOut(800);
    }else{                        
      $('#biblio #msg-success').html('<p>' + data.descrip + '</p>');
      $('#biblio #msg-success').addClass('msg-error');
      $('#biblio #msg-success').css('display','block');
      $("#biblio #msg-success").delay(5500).fadeOut(800);
    }
  }
