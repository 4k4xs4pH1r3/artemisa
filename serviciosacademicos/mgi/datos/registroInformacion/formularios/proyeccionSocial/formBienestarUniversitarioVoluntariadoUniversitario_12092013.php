...
<form action="" method="post" id="forma<?=$_REQUEST['alias']?>">
	...
	<fieldset>  
		...
		<?php //$utils->getSemestresSelect($db,"semestre");
                 ...
                 $utils->pintarBotonCargar("popup_cargarDocumento(3,4,$('#".$idForm." #semestre').val())","popup_verDocumentos(3,4,$('#".$idForm." #semestre').val())"); ?>
...
?>
				<tr class="dataColumns category" style="<?=($row2['aliasclasificacionesinfhuerfana']=='p_evu_ddbu')?'':'border-right:0';?>">
					<th colspan="<?=($row2['aliasclasificacionesinfhuerfana']=='p_evu_ddbu')?8:6;?>" class="borderR"><?=$row2['clasificacionesinfhuerfana']?></th>               
				</tr>
				<tr class="dataColumns" style="<?=($row2['aliasclasificacionesinfhuerfana']=='p_evu_ddbu')?'':'border-right:0';?>">
					<th rowspan="2" class="borderR"><?=($row2['aliasclasificacionesinfhuerfana']=='p_evu_ddbu')?'Proyecto':'Servicio o actividad';?></th>               
...
?>
	...
	?>
					<tr id="contentColumns" class="row" style="<?=($row2['aliasclasificacionesinfhuerfana']=='p_evu_ddbu')?'':'border-right:0';?>">
...
<?
						if($row2['aliasclasificacionesinfhuerfana']=='p_evu_ddbu') {
?>
							<td class="column borderR" align="center"><input type="text" class="required number" name="beneficiarios[<?=$row['idclasificacionesinfhuerfana']?>]" id="beneficiarios[<?=$row['idclasificacionesinfhuerfana']?>]" title="Beneficiarios comunidad" maxlength="10" tabindex="1" autocomplete="off" size="6" style='text-align:center' /></td>
							<td class="column " align="center"><input type="text" class="required number" name="encuentros[<?=$row['idclasificacionesinfhuerfana']?>]" id="encuentros[<?=$row['idclasificacionesinfhuerfana']?>]" title="Nro. de encuentros o presentaciones oficiales" maxlength="10" tabindex="1" autocomplete="off" size="6" style='text-align:center' /></td>
<?
						}
?>
					</tr>
