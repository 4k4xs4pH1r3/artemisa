...
if($_REQUEST['Modificar']==1){
          ...
          while(!$exec->EOF){
            ...
            $query2="UPDATE siq_bienestaruniversitario SET codigoestado='200' 
                where id = '".$id."'";
            $db->Execute($query2);
           ...
           $check = 'SELECT codigoestado FROM siq_bienestaruniversitario WHERE idclasificacionesinfhuerfana="'.$vlrid.'" 
               AND semestre="'.$_REQUEST['semestre'].'"';
           $checkR = $db->GetRow($check);
           if(count($checkR)>0){
            $SQL='UPDATE   siq_bienestaruniversitario
                    ...
                    SET      
                            ...
                            egresados="'.$vlr3.'",
                            docentes="'.$vlr4.'",
                            administrativos="'.$vlr5.'",
                            familiares="'.$vlr6.'",
                            encuentros_presentacion_oficiales="'.$vlr7.'",
                            beneficiarios_comunidades="'.$vlr8.'",
                            lunes="'.$vlr9.'",
                            martes="'.$vlr10.'",
                            miercoles="'.$vlr11.'",
                            jueves="'.$vlr12.'",
                    ...
                    WHERE
                            ...
                            id="'.$id.'"
                            AND
                            idclasificacionesinfhuerfana="'.$vlrid.'"'; 

                    if($UpdateBienestar=&$db->Execute($SQL)===false){
                            echo 'Error en el Modificar....<br>'.$SQL;
                            die;
                    }         
            } else {
    ...
    $a_vectt['val']			='TRUE';
    $a_vectt['message']	    ='Se ha Modificado la Informacion.';
...
if($exec->RecordCount()==0) {
	...
	$mensaje='Información almacenada para el periodo '.$_REQUEST['semestre'];
} else {
	$mensaje='Ya existe información almacenada para el periodo '.$_REQUEST['semestre'];
        $success = false;
}
