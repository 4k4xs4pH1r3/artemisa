             ...
             var mes = $(formName+' #mes').val()
                ...
                var alias = "<?PHP echo $_REQUEST['alias']?>";
                        $.ajax({
                            ...
                            data: { mes: mes, action: "getCategories", anio: anio, alias: alias, tipo: 3 },     
...
<form action="" method="post" id="forma<?PHP echo $_REQUEST['alias']?>">
	...
	<fieldset>  
		...
		<?php //$utils->getSemestresSelect($db,"semestre");
                 ...
                 $utils->pintarBotonCargar("popup_cargarDocumento(3,4,$('#".$idForm." #mes').val()+'-'+$('#".$idForm." #anio').val())","popup_verDocumentos(3,4,$('#".$idForm." #mes').val()+'-'+$('#".$idForm." #anio').val())"); ?>
...
?>
				<tr class="dataColumns category" style="<?PHP echo ($row2['aliasclasificacionesinfhuerfana']=='p_evu_ddbu')?'':'border-right:0';?>">
					<th colspan="<?PHP echo ($row2['aliasclasificacionesinfhuerfana']=='p_evu_ddbu')?8:6;?>" class="borderR"><?PHP echo $row2['clasificacionesinfhuerfana']?></th>               
				</tr>
				<tr class="dataColumns" style="<?PHP echo ($row2['aliasclasificacionesinfhuerfana']=='p_evu_ddbu')?'':'border-right:0';?>">
					<th rowspan="2" class="borderR"><?PHP echo ($row2['aliasclasificacionesinfhuerfana']=='p_evu_ddbu')?'Proyecto':'Servicio o actividad';?></th>               
...
?>
	...
	?>
					<tr id="contentColumns" class="row" style="<?PHP echo ($row2['aliasclasificacionesinfhuerfana']=='p_evu_ddbu')?'':'border-right:0';?>">
...
<?PHP 
						if($row2['aliasclasificacionesinfhuerfana']=='p_evu_ddbu') {
?>
							<td class="column borderR" align="center"><input type="text" class="required number" name="beneficiarios[<?PHP echo $row['idclasificacionesinfhuerfana']?>]" id="beneficiarios[<?PHP echo $row['idclasificacionesinfhuerfana']?>]" title="Beneficiarios comunidad" maxlength="10" tabindex="1" autocomplete="off" size="6" style='text-align:center' /></td>
							<td class="column " align="center"><input type="text" class="required number" name="encuentros[<?PHP echo $row['idclasificacionesinfhuerfana']?>]" id="encuentros[<?PHP echo $row['idclasificacionesinfhuerfana']?>]" title="Nro. de encuentros o presentaciones oficiales" maxlength="10" tabindex="1" autocomplete="off" size="6" style='text-align:center' /></td>
<?PHP 
						}
?>
					</tr>
