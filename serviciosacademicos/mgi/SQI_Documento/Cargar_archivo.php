...
#echo '<br>$Factor_id-->'.$Factor_id.'<br>$caracteristica_id-->'.$caracteristica_id.'<br>$Aspecto_id-->'.$Aspecto_id.'<br>$Inicador_id-->'.$Inicador_id.'<br>$Descripcion-->'.$Descripcion.'<br>$Tipo_Carga-->'.$Tipo_Carga;
##########################################################
...
$userid=$Usario_id->fields['id'];
               ...
               $SQL_Buscar='SELECT 
							...
							siq_documento AS doc INNER JOIN siq_archivo_documento AS archivo ON doc.idsiq_documento=archivo.siq_documento_id AND doc.codigoestado=100 AND archivo.codigoestado=100 AND doc.siqindicador_id="'.$Inicador_id.'"';
							
							if($Existe=&$db->Execute($SQL_Buscar)===false){
									echo 'Error al Buscar si existen mas Archivos Relcionados....<br>'.$SQL_Buscar;
									die;
								}
