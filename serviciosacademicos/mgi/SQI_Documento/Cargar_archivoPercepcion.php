...
$Info = pathinfo($_FILES["file"]["name"]);
	...
	<script>
		alert('No es del Tipo de Documento permitido el formato del documento permitido es Word 2007 en adelante o .docx');
		location.href='Carga_Documento.html.php?actionID=Percepcion&pregunta_id=<?PHP echo $Pregunta_id?>&indicador_id=<?PHP echo $Inicador_id?>';
	</script>
	<?PHP
	die;
	}
...
$SQL_Insert_pregunta='INSERT INTO siq_Apreguntadocumental(idsiq_Apregunta,disiq_documento,usuariocreacion,fechacreacion)VALUES("'.$Pregunta_id.'","'.$Documento_id.'","'.$userid.'",NOW())';

		if($Insert_pregunta=&$db->Execute($SQL_Insert_pregunta)===false){
				echo 'Error en el SQL Al insertar el documento con la Pregunta....<br>'.$SQL_Insert_pregunta;
				die;
			}
