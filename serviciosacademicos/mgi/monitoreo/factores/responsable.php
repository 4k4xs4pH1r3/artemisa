...
$db = writeHeader("Asignar Responsable a Factor", TRUE, $proyectoMonitoreo);
...
if (strcmp($action, "inactivate") == 0) {
    $fields = array();
    $fields["idsiq_responsableFactor"] = $id;
    $data = $utils->processData($_REQUEST["action"], "responsableFactor", $fields);
    ...
    if ($edit) {
        $relacion = $utils->getDataEntity("responsableFactor", $idR);
    ...
    <div id="contenido">
        ...
        <div id="form"> 
            ...
            <form action="save.php" method="post" id="form_test" >
                <input type="hidden" name="entity" value="responsableFactor" />
                <input type="hidden" name="action" value="<?php echo $action; ?>" />
                <span class="mandatory">* Son campos obligatorios</span>
                <fieldset>   
                    <legend>Información del Factor</legend>
                    <label class="grid-2-12">Nombre del Factor: </label>
                    <p><?php echo $data["nombre"]; ?></p>        
                    <input type="hidden" name="idFactor" value="<?php echo $data["idsiq_factor"]; ?>" />                   
                </fieldset>
                <fieldset>   
                    <legend>Información del Responsable</legend>
                    <label for="usuarioResponsable" class="grid-2-12">Responsable: <?php if (!$edit) { ?>   <span class="mandatory">(*)</span><?php } ?></label>
                    <?php if ($edit) { ?>                                
                        ...
                        <input type="hidden" name="idsiq_responsableFactor" value="<?php echo $relacion["idsiq_responsableFactor"]; ?>" />
                    <?php } else { ?>
