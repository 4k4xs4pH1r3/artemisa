 ...
 <script>
    function valor_r(i){
       ...
       // alert(i);
        for(z=0;z<t;z++){
            $("#correcta1_"+z+"").val('0');
            //alert($("#correcta1_"+z+"").val());
        }
        $("#correcta1_"+i+"").val('1');
    }
 </script>
<link rel="stylesheet" href="../../css/styleAutoevaluacion.css" />
 <form action="save.php" method="post" id="form_test" name="form_test">
<div id="container">   
		<div class="demo_jui">
                            ...
                            <table align="center" width="80%" border="0">
                                    ...
                                    <tbody>
                                            <tr id="0"> 
                                                    ...
                                                    <td>
                                                        <input type="hidden" name="valor1[0]" id="valor1_0" value="<?php if(!empty($data[0]['valor'])){ echo $data[0]['valor']; }else{ echo "999"; } ?>" >
                                                        <input type="radio" name="corr" id="corr_0" onclick="valor_r(0)" <?php if ($data[0]['correcta']==1){ ?> checked="checked" <?php } ?>>
                                                        <input type="hidden" name="correcta1[0]" id="correcta1_0" value="<?php if(!empty($data[0]['correcta'])){ echo $data[0]['correcta']; }else{ echo "999"; } ?>" >
                                                    </td>
                                                     ...
                                                     for($i=1;$i<=$can;$i++){
                                                        ?>
                                                            <tr id="<?php echo $i ?>"> 
                                                                    ...
                                                                    <td>
                                                                    <!-- <input type="checkbox" name="valorMR[0]" id="valorMR_0" >--> 
                                                                     <input type="radio" name="corr" id="corr_<?php echo $i ?>" <?php if ($data[$i]['correcta']==1){ ?> checked="checked" <?php } ?>  onclick="valor_r(<?php echo $i ?>)"  >
                                                                     <input type="hidden" name="correcta1[<?php echo $i ?>]" id="correcta1_<?php echo $i ?>" value="<?php if(!empty($data[$i]['correcta'])){ echo $data[$i]['correcta']; }else{ echo "999"; } ?>" >
                                                                     <input type="hidden" name="valor1[<?php echo $i ?>]" id="valor1_<?php echo $i ?>" value="<?php if(!empty($data[$i]['valor'])){ echo $data[$i]['valor']; }else{ echo "999"; } ?>" ></td>
                                                                     <td align="right"><input type="button" value="" class="button medium clsEliminarFilaL"></td>
                                                            </tr>
                                                        <?php
                                                    }
            ...
            $(document).ready(function(){
                  ...
                  //alert($("#total").val());
                    $(document).on('click','.clsAgregarFilaL',function(){
                            var strNueva_FilaL='<tr id="'+i+'">'+
                                    ...
                                    '</td>'+
                                    '<td><input type="radio" name="corr" id="corr_'+i+'" onclick=\'valor_r('+(i)+')\'  >'+
                                         '<input type="hidden" name="correcta1['+i+']" id="correcta1_'+i+'" value="0" >'+
                                         '<input type="hidden" name="valor1['+i+']" id="valor1_'+i+'" value="0"  ></td>'+
                                    '<td align="right"><input type="button" value="" class="button medium clsEliminarFilaL" ></td>'+
                            '</tr>';
             ...
             $(':submit').click(function(event) {
                    ...
                    for (var i=0;i<CantItems;i++){
                       ...
                       var resp=$("#form_test input[name='corr']:radio").is(':checked')
                       //alert(resp)
                       if (value!='' && resp!=false){
                            j++
                       }
