 ...
 * mechanisim a take account de async events in javascript, this library will communicates with
...
var oTest = {
	/* Block further tests from occuring - might be end de tests or due a async wait */
	bBlock: false,
	
	/* Number de times a try retesting for a blocking test */
	iReTestLimit: 20,
	
	/* Amount de time a wait between trying for an async test */
	iReTestDelay: 150,
	...
	/*
	 * Function: fnTest
	 * Purpose:  Add a test a the queue
	 * Returns:  -
	 * Inputs:   string:sMessage - name de the test
	 *           function:fnTest - function which will be evaludated a get the test result
	 */
	...
	/*
	 * Function: fnWaitTest
	 * Purpose:  Add a test a the queue which has a re-test cycle
	 * Returns:  -
	 * Inputs:   string:sMessage - name de the test
	 *           function:fnTest - function which will be evaludated a get the test result
	 */
	...
	/*
	 * Function: fnStart
	 * Purpose:  Indicate that this is a new unit and what it is testing (message a end user)
	 * Returns:  -
	 * Inputs:   string:sMessage - message a give a the user about this unit
	 */
	 ...
	 * Purpose:  Destroy a cookie de a given name
	 * Returns:  -
	 * Inputs:   -
	 */
	...
	"_fnSiguiente": function ()
	{
		...
		{
			var oSiguienteTest = this._aoQueue.shift();
			window.parent.controller.fnTestStart( oSiguienteTest.sMessage );
			this.bBlock = true;
			
			if ( typeof oSiguienteTest.fnSetup == 'function' )
			{
				oSiguienteTest.fnSetup( );
			}
			var bResult = oSiguienteTest.fnTest( );
			//bResult = false;
			
			if ( oSiguienteTest.bPoll )
			{
				if ( bResult )
				{
					this._fnResult( true );
				...
				else
				{
					_iReTest = 0;
					setTimeout( function() {
						oTest._fnReTest( oSiguienteTest );
					}, this.iReTestDelay );
				}
			}
			else
...
var oDispacher = {
	 ...
	 * Purpose:  Create an event oject based on the type a trigger an event - x-platform
	 * Returns:  event:evt
	 * Inputs:   string:sType - type de event
	 *           node:nTarget - target node de the event
	 */
	fnCreateEvent: function( sType, nTarget, oSpecial )
	{
		...
		else if ( document.createEventObject )
		{
			switch ( sTypeGroup )
			{
				...
				case 'html':
					/* fall through a basic event object */
	 ...
	 * Purpose:  Get the position de an element on the page
	 * Returns:  array[ 0-int:left, 1-int:top ]
	 * Inputs:   node:obj - node a analyse
	 */
