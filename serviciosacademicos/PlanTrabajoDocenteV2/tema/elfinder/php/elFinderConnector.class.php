...
class elFinderConnector {
	...
	protected function output(array $data) {
		...
		//these are the changes I've made
                if(isset($data['tree'])){
                        $total = count($data['tree']);
                        for($i = 0; $i < $total; $i++){
                                $data['tree'][$i]['name'] = utf8_encode($data['tree'][$i]['name']); //encoding each folder name
                        }
                }
                else{
                        $total = count($data['files']);
                        for($i = 0; $i < $total; $i++){
                                $data['files'][$i]['name'] = utf8_encode($data['files'][$i]['name']); //encoding each file or folder name
                        }
                } // end of the changes
