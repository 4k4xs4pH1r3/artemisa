...
/**
 * Se modifica las rutas relativas por rutas absolutas de modo que siempre se 
 * conozca en donde se encuentran los archivos referenciados
 * @modified Andres Ariza <andresariza@unbosque.edu.do>.
 * @since 30 de enero de 2019.
 */
 ...
 * Se asigna la referencia sin aporte a una variable para poder utilizarla sin que
 * se sobreescriba con el valor del aporte
 * @since Enero 31, 2019
*/
...
$aporte = $datos->fechas_pago_aporte($_GET['numeroordenpago'], $conexion);
$codigosbarrasaportes = $datos->generar_codigobarras_aportes($aporte);
$titulosbarrasaportes = $datos->generar_titulobarras_aportes($aporte);
...
$factura->Rect(1, 9.6, 11, 2.2, ""); //tercer bloque rectangulo de Universidad y datos basicos mod "10.3"
...
$factura->Rect(1, 14.8, 6, 6.1, ""); // cuadro blanco izquierdo al lado de los cod de barras arriba 
$factura->Rect(1, 20.9, 6, 6.1, ""); // cuadro blanco izquierdo al lado de los cod de barras abajo
/* * ***************************************** */
...
$factura->Rect(19.5, 10.3, 0.20, 0.20, "");
$factura->Rect(19.5, 10.6, 0.20, 0.20, "");
 ...
 * Se modificó la razon social del banco Helm Bank a Itaú de acuerdo a solicitud de Atención al Usuario.
 * @since Junio 30 de 2017
 */
$factura->Cell(4, 0.3, utf8_decode("ITAÚ"), 0, 1, 'L');
//End Caso 91441
$factura->SetFont('Arial', '', 7);
$factura->SetFont('Arial', 'B', 7);
$factura->Cell(4, 0.3, utf8_decode("BANCO DE BOGOTÁ"), 0, 1, 'L');
$factura->SetFont('Arial', '', 7);
$factura->SetFont('Arial', 'B', 7);
...
/**
 * @modified Andres Ariza <arizaandres@unbosque.edu.do>
 * Se cambia la utilizacion de la variable referencia del objeto $datos ya que esta
 * se sobreescribe con la referencia de los pagos con aporte, en su lugar se usa la fariable
 * $referencia que se salvo en la linea 51 de este archivo
 * @since Enero 31, 2019
*/
$factura->Cell(8, 0.65, "REFERENCIA " . $referencia, 0, 1, 'L');
$factura->SetFont('Arial', '', 7);
...
$factura->SetLeftMargin(7.1); // posicion X de los codigos de barras mod "7.4"
$factura->SetFont('code128', '', 28);
$contador_barras = 0;

foreach ($codigosbarrasaportes as $llave => $valor) {
    $factura->SetX(6.2); // posicion X de la fecha de los plazos
    $factura->SetFont('arial', '', 8);
    $factura->Cell(19, 1.0, "               " . $datos->fechas[$contador_barras]['nombreplazo_3'] . "                           " . $datos->fechas[$contador_barras]['fechaordenpago'] . "                           " . money_format('%(#10n', $datos->fechas[$contador_barras]['valorfechaordenpago'] + $aporte), 0, 1, 'L'); //EN ESTA LINEA SE SUMA EL VALOR DEL APORTE A LA MATRICULA
...
$factura->SetX(3.4);
$factura->Cell(21.65, 0.2, "PAGUESE UNICAMENTE EN CHEQUE DE GERENCIA O EFECTIVO NO SE ACEPTAN PAGOS PARCIALES", 0, 0, 'L');
