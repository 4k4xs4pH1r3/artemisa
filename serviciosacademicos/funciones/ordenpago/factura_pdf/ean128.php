	...
	function codificar()
	{
		...
		if(strlen($this->cadena)>0)
		{
			...
			if($this->i > 0)
			{
				...
				for($this->i=1;$this->i <= strlen($ean128); $this->i++)
				{
					//echo "i ",$this->i,"check ",$checksum,"dummy ",$dummy;echo "<br>";
					$dummy=ord(substr($ean128,$this->i-1,1));
					if($dummy < 127 and $dummy)
					{
						$dummy=$dummy-32;
					}
					else
					...
					$checksum=($checksum+($this->i-1) * $dummy) % 103;
					if($checksum<0)
					{
						$checksum=$checksum*-1;
					}
