...
if($usarcondetalleprematricula)
{
	...
	$premainicial1=mysql_query($query_premainicial1, $this->sala) or die("$query_premainicial1");
...
from planestudio";
$semestreplanes=mysql_query($query_semestreplanes, $this->sala) or die("$query_semestreplanes");
...
if(isset($materiaselegidas))
{
	foreach($materiaselegidas as $codigomateriaelectiva2 => $codigomateria2)
	{
		...
		and pee.codigoestudiante = '$this->codigoestudiante'
		...
		//echo "$query_premainicial1<br>";
		$datosmateriaselegidas=mysql_query($query_datosmateriaselegidas, $this->sala) or die("$query_datosmateriaselegidas<br>".mysql_error());
		...
		if($totalRows_datosmateriaselegidas == "")
		{
			...
			where m.codigomateria = '$codigomateria2'
			and lee.codigoestudiante = '$this->codigoestudiante'
			...
			$datosmateriaselegidas=mysql_query($query_datosmateriaselegidas, $this->sala) or die("$query_datosmateriaselegidas".mysql_error());
			$totalRows_datosmateriaselegidas = mysql_num_rows($datosmateriaselegidas);
		...
		if($totalRows_datosmateriaselegidas == "")
		{
			...
			where gm.codigoperiodo = '$this->codigoperiodo'
			...
			$datosmateriapapa = mysql_query($query_datosmateriapapa, $this->sala) or die("$query_datosmateriapapa");
			...
			if($totalRows_datosmateriapapa == "")
			{
				...
				if($row_datosmateriapapa['codigotipomateria'] == '4' && $row_datosmateriapapa['codigoindicadorgrupomateria'] == "200")
				{
					...
					$datosmateriaselegidas = mysql_query($query_datosmateriaselegidas, $this->sala) or die("$query_datosmateriaselegidas");
					$totalRows_datosmateriaselegidas = mysql_num_rows($datosmateriaselegidas);
				...
				else
				{
					...
					where m.codigomateria = '$codigomateria2'
					and m.codigoestadomateria = '01'
					and c.codigoestudiante = '$this->codigoestudiante'
...
if($res_sem[0] == 1 && $maxcreditos == 0)
{
    ...
    $selperidoanterior = mysql_query($query_selperidoanterior, $this->sala) or die("$query_selperidoanterior");
