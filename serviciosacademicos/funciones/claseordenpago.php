...
//													//
    ...
    function insertarordenpago() {
		...
		$query_insordenpago = "INSERT INTO ordenpago(numeroordenpago, codigoestudiante, fechaordenpago, idprematricula, fechaentregaordenpago, codigoperiodo, codigoestadoordenpago, codigoimprimeordenpago, observacionordenpago, codigocopiaordenpago, documentosapordenpago, idsubperiodo, idsubperiododestino, documentocuentaxcobrarsap, documentocuentacompensacionsap, fechapagosapordenpago)
             ...
             * logordenpago con el fin de evidenciar quien crea y anula ordenes de pago. 
             * @since Junio 17, 2019.
            */
			...
			VALUES(0, now(), 'CREACION DE ORDEN', '$this->numeroordenpago', '$idusuario', '" . tomarip() . "')";
              ...
              $query_inslogordenpago = "INSERT INTO logordenpago(idlogordenpago, fechalogordenpago, observacionlogordenpago, numeroordenpago, idusuario, ip)
			VALUES(0, now(), 'CREACION DE ORDEN', '$this->numeroordenpago', '2', '" . tomarip() . "')";
        ...
        if ($totalRows_prematricula != "") {
            ...
            $this->idprematricula = $row_prematricula['idprematricula'];
             ...
             $SQL='SELECT
                    ...
                    AND
                    e.codigoperiodo='.$this->codigoperiodo.'
                    AND
...
if ($cantidaddetalleordenpago == 0) {
 ...
 $query_insdetalleordenpago = "insert into detalleordenpago(numeroordenpago,codigoconcepto,cantidaddetalleordenpago,valorconcepto,codigotipodetalleordenpago)
   ...
   $query_insfechaordenpago = "insert into fechaordenpago(numeroordenpago,fechaordenpago,porcentajefechaordenpago,valorfechaordenpago)
    ...
    }//function insertarfechasordenpago_fechafiancieraEducacion
	 ...
	 * @since  Abril 20, 2017
	 * Funcion nueva que permite insertar concepto temportal en la tabla AportesBecas
	*/
...
<?php
    ...
    function Arreglo_ordenpagoproceso($ruta, $rutaimpresion="") {
        ...
        return $Data_Proceso;
    }//function Arreglo_ordenpagoproceso
   ...
   function mostrar_ordenpagoporpagar($ruta, $rutaimpresion="") {
        ...
        * Cambio de funcion depreciada, eliminacion de codigo comentado y validaciona adicional con estado de orden
        */
        // Ordenes de pago por pagar
        if (preg_match("/^1.+$/", $this->codigoestadoordenpago)) {
            $this->numerodocumento_ordenpago();
            $query_ordenesenproceso = "select * from LogPagos l ".
            " INNER JOIN ordenpago o on (o.numeroordenpago = l.Reference1 and o.codigoestadoordenpago in (60, 61) ) ".
            " where l.StaCode='PENDING' and l.Reference2='".$this->numerodocumento."' ".
            " and l.Reference3='".$this->tipodocumento."' order by 2 desc";
            $ordenesenproceso = mysql_query($query_ordenesenproceso, $this->sala) or die("$query_ordenesenproceso<br>" . mysql_error());
            $row_ordenesenproceso = mysql_fetch_array($ordenesenproceso);
            $totalRows_ordenesenproceso = mysql_num_rows($ordenesenproceso);
                        ...
                        if ($estaapazysalvo and $estasindeudas and ($estasindeudas * 1) <> 8) {			
                            if (!$this->es_ordenplandepagohijo()) {
                                ...
                                if($totalRows_ordenesenproceso>0){
                                    ...
                                    /**
                                    *@modifed  Ivan Quintero  <quinteroivan@unbosque.edu.co>
                                    * Modificacion del link de pagos en linea para la integracion de redebank
                                    * Se agrega validacion del usuario de imngreso al proceso solo para coordinadorsisinfo
                                    * @since Agosto 26 del 2017
                                    *
                                    if($_SESSION['MM_Username'] <> 'coordinadorsisinfo'){
                                    echo "<p>Disculpe las molestias, estamos trabajando para mejorar nuestro servicio.</p>";
                                    }else{*/
                                        ?>
    ...
    }//function mostrar_ordenpagoporpagar end
    /**************************************************/
...
}//function arreglo_ordenpagoporpagar
    ...
    function imprimir_ordenpdf($ruta, $nombre="Imprimir (Pago en bancos)") {
        if (!ereg("^4.+$", $this->codigoestadoordenpago)) {
           ...
           /* 
               ...
               * @since Enero 2, 2018
               * Se comenta el siguiente bloque con el fin de bloquear la opcion para que el estudiante pueda imprimir la orden de pago teniendo deudas
               * pendientes de acuerdo a decisión de vicerrectoria hasta el 19 de enero de 2018.
           */
                    $estaapazysalvo = $this->valida_pazysalvoestudiante();
                    $sindocumentospendientes = $this->valida_documentosestudiante();
                    $tieneevaluaciondocente = $this->valida_evaluaciondocenteestudiante();
...
<?php
                ...
                if (is_array($materiascongrupo)) {
                    ...
                    if ($totalRows_numeroordeninterna != "") {
                        ...
                        if($op==1){
                            $info['val'] = 1;
                            $info['msj'] = "El grupo requiere orden pago interna y no posee una activa";
                           // echo '<pre>';print_r($info);
                            echo json_encode($info);
                            exit;
                        }else{
...
<?php
                    ...
                    if($op==1){
                            $info['val'] = 1;
                            $info['msj'] = "La carrera requiere orden pago interna y no posee una activa";
                           // echo '<pre>';print_r($info);
                            echo json_encode($info);
                            exit;
                    }else{
    ...
    function anular_ordenpago($motivo = 0) {
        ...
        if (isset($usuario)) {
            ...
            */
            ...
            $query_inslogordenpago = "INSERT INTO logordenpago(idlogordenpago, fechalogordenpago, observacionlogordenpago, numeroordenpago, idusuario, ip)
            VALUES(0, now(), 'ANULACION - $motivo', '$this->numeroordenpago','2', '" . tomarip() . "')";
            //echo "<br>uno".$query_inslogordenpago;
            $query_inslogordenpago = mysql_query($query_inslogordenpago, $this->sala) or die("$query_inslogordenpago <br>" . mysql_error());
        }
        ...
        if ($totalRows_seldetalleprematriculaorden != "") {
            while ($row_seldetalleprematriculaorden = mysql_fetch_array($seldetalleprematriculaorden)) {
                ...
                */
				...
				VALUES('" . $row_seldetalleprematriculaorden['idprematricula'] . "','" . $row_seldetalleprematriculaorden['codigomateria'] . "','" . $row_seldetalleprematriculaorden['codigomateriaelectiva'] . "','20','" . $row_seldetalleprematriculaorden['codigotipodetalleprematricula'] . "','" . $row_seldetalleprematriculaorden['idgrupo'] . "','" . $row_seldetalleprematriculaorden['numeroordenpago'] . "','" . date("Y-m-d H:i:s", time()) . "', (SELECT usuario FROM usuario WHERE idusuario = ".$_SESSION['idusuario'].") ,'" . tomarip() . "')";
        ...
        $SQL='SELECT
          ...
          if($C_OrdenEstudiante==false){
            ...
            $json['valorconcepto'] = number_format($C_OrdenEstudiante['valorconcepto'],0,',',',');
             ...
             if (ereg("^1.+$", $C_OrdenEstudiante['codigoestadoordenpago'])) {
                //$estado = "porpagar";
                $json['estado'] = 1;
                $json['mensaje1'] = '';
                $json['texto1'] = '';
                                ...
                                if ($estaapazysalvo and $estasindeudas and ($estasindeudas * 1) <> 8) {
            		                if (!$this->es_ordenplandepagohijo()) {
                                      ...
                                      if($totalRows_ordenesenproceso>0){
                                        ...
                                        //$json['pagos'] = $ruta .'verorden.php?pse='.$fechavencimiento['valorfechaordenpago'] .'&numeroordenpago='.$this->numeroordenpago.'&codigoestudiante='.$this->codigoestudiante.'&codigoperiodo='.$this->codigoperiodo.'&ipimpresora='.$rutaimpresion;
             ...
             if (ereg("^4.+$", $C_OrdenEstudiante['codigoestadoordenpago']) || ereg("^5.+$", $C_OrdenEstudiante['codigoestadoordenpago'])) {
               // $estado = "paga";
               $json['estado'] = 2;
               $json['imprimir'] = '';
               $json['Bloqueo'] = '';
             ...
             /*if (ereg("^5.+$", $C_OrdenEstudiante['codigoestadoordenpago'])) {
                //$estado = "paga";
                $json['estado'] = 2;
             }*/
             if (ereg("^6.+$", $C_OrdenEstudiante['codigoestadoordenpago'])) {
               // $estado = "enproceso";
               $json['estado'] = 3;
               $json['imprimir'] = '';
               $json['Bloqueo'] = '';
