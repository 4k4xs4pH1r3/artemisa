...
class autenticacion
{
	...
	var $passwordPlano;
	var $usuariocreado;
	function autenticacion(&$conexion,$usuario,$password,$autenticarsegundaclave,$modoRespuestaAutenticacion='XML'){
		...
		if(!empty($usuario) and !empty($password)){
			...
			$usuarioBDValido=$this->validarUsuarioBD($this->usuario);

			$_SESSION['MM_Username'] = $this->usuario;
			$_SESSION['key']=$this->password;

			if($this->usuarioValido($this->ip, $this->usuario)) 
			{
				if($usuarioBDValido==true){
					...
					if($correoValido==true){
						if($this->usuariocreado){
							//$this->mensajeUsuario="Usuario Deshabilitado";
							$this->mensajeUsuario="Usted aún no se ha registrado como usuario del sistema SALA, porfavor llene los datos solicitados...";
							$this->urlRedireccionamiento='../userAjax.php?estado=deshabilitado';
						}
						else{
							$this->mensajeUsuario="Usted aún no se ha registrado como usuario del sistema SALA, porfavor llene los datos solicitados...";
			...
			$this->autenticacionAutorizada=false;
			$this->mensajeUsuario="Porfavor digite usuario y clave";
		}
	...
	{
	$msgbox=imap_open ("{172.16.7.109/pop3:110/notls}Inbox", $usuario, $password);
	if($msgbox){
	return true;
	}
	else{
	...
	*/
	...
	function validarCorreo($usuario,$password){
		error_reporting(0);
		$msgbox=imap_open ("{correo.unbosque.edu.co/pop3:110/notls}Inbox", $usuario, $password);
		//$msgbox=imap_open ("{200.31.79.228/pop3:110/notls}Inbox", $usuario, $password);
		//$msgbox=imap_open ("{172.16.7.111/pop3:110/notls}Inbox", $usuario, $password);
		if($msgbox)
		{
			imap_close($msgbox);
			return true;
		...
		{
			//$msgbox=imap_open ("{orion.sistemasunbosque.edu.co/pop3:110/notls}Inbox", $usuario, $password);
			//$msgbox=imap_open ("{200.31.79.233/pop3:110/notls}Inbox", $usuario, $password);
			//$msgbox=imap_open ("{172.16.7.111/pop3:110/notls}Inbox", $usuario, $password);
			if($msgbox)
			{
				imap_close($msgbox);
	...
	**/
	function usuarioValido($ip, $usuario)
	{
		...
		WHERE usuario = '".$user."' 
		and (NOW() between fechainiciousuario and fechavencimientousuario)
		and codigoestadousuario like '1%'";
		...
		if($numRowsOperacion == 1)
		{
			...
			$queryUser = sprintf("SELECT ipaccesousuario FROM usuario 
			...
			else if($ip == $rowOperacion['ipaccesousuario']) 
			{
				$uservalido = true;
			}	
	...
	function validarUsuarioBD($usuario){
		...
		$queryUser ="SELECT * FROM usuario WHERE usuario = '".$user."' 
								and (NOW() between fechainiciousuario and fechavencimientousuario)
								and codigoestadousuario like '1%'";
		$operacion=$this->conexion->query($queryUser);
		$numRowsOperacion=$operacion->numRows();
	...
	function validarExistenciaClave($idusuario,$codigotipoclaveusuario){
		...
		and (codigoestado like '1%' or codigoindicadorclaveusuario like '2%')" ;
		$operacion=$this->conexion->query($query);
		$rowOperacion=$operacion->fetchRow();
	...
	function cargaVariables(){
		...
		else if($rol==2)
		{
			...
			//echo "PERIODO SESION=".$_SESSION['codigoperiodosesion'];
