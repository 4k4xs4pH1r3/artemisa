...
* Ajustes de html en la tablas y clases que se usan.
* Modificado 7 de noviembre del 20172
*/
/**
 ...
 */
	...
	function tabla_arreglo_chulitos()
	{
		...
		echo "<tr><td><table class='table table-bordered' style='font-size:12px;'>
		<tr><td><input class='btn btn-fill-green-XL' name='Exportar_recorte' type='submit' id='Exportar_recorte' value='Exportar'></td>
		<td><input class='btn btn-fill-green-XL' name='Recortar' type='submit' id='Recortar' value='Recortar'></td>
	...
	function escribir_filtros($matriz,$wrap="noWrap")
	{
		...
		{
			echo "<td $wrap>
			<input name='$elemento[0]' type='text' size='15' value='".$_SESSION['get'][$elemento[0]]."'><br>
	...
	function listar($matriz="",$texto="",$link="",$filtros="si",$numerar="no",$totalizar="no",$campos_edicion="no",$wrap="noWrap",$align="center",$nbsp="")
	{
		...
		if($texto!="")
		{
			echo "<caption align=TOP><h2>$texto</h2></caption>";
		}
...
if(isset($_SESSION['sesionFecha_Proximo_Contacto']) && !isset($_REQUEST['Filtrar']))
{
?>
...
<?php
	...
	function botones()
	{		
		echo "<br><table style='font-size:12px;'>
	  	<tr>
    	<td><input class='btn btn-fill-green-XL' name='Filtrar' type='submit' id='Filtrar' value='Filtrar'></td>
	    <td><input class='btn btn-fill-green-XL' name='Restablecer' type='submit' id='Restablecer' value='Restablecer'></td>
	    <td><input class='btn btn-fill-green-XL' name='Regresar' type='button' id='Regresar' value='Regresar' onClick='javascript:history.back()'></td>
	    <td><input class='btn btn-fill-green-XL' name='Recargar' type='submit' id='Recargar' value='Recargar'></td>
	    <td><input class='btn btn-fill-green-XL' name='Exportar' type='submit' id='Exportar' value='Exportar'></td>\n";
		echo '
    	<td><div align="center">
            <label>XLS</label>
            <input name="tipo_exp" type="radio" value="xls" checked>        
    	</div></td>
	    <td></td>
	    <td><div align="center"><input name="tipo_exp" type="radio" value="doc"></div></td>
    	<td><div align="center">DOC</div>
    	</td>
        </tr>
		</table><br>';
	...
	<!--<script language="javascript">window.open("funciones/exportar.php?nombre=<?php echo $nombrearchivo?>&formato=<?php echo $formato?>&arreglo=<?php echo $arreglo?>");</script>-->
<?php 
 $REQUEST_URI ="funciones/exportar.php?nombre=".$nombrearchivo."&formato=".$formato."&arreglo=".$arreglo;
echo "<META HTTP-EQUIV='Refresh' CONTENT='0;URL=$REQUEST_URI'>";
?>
	...
	function DibujarTablaNormal($matriz,$texto="")
	{
		if(is_array($matriz))
		{
			echo "<table class='table' border=1 style='font-size:12px;' align=center>\n";
	...
	function TablaArregloChulitosFilas($matriz,$col_id,$texto="")
	{
		if(is_array($matriz))
		{
			echo "<table id='TablaArregloChulitosFilas' class='table' border='1' style='font-size:12px;'>\n";
			echo "<caption align=TOP>$texto</caption>";
