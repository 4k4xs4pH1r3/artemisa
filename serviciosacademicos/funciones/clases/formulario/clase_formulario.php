...
/*
 * Se hace reorganizacion de codigo y se eliminan sentencias else innecesarias
...
class formulario extends ADODB_Active_Record {
    ...
    function validacion($nombrevar, $validacion) {
        ...
        switch ($validacion) {
            ...
            case "fechanacimiento":
                ...
                //$regs = array();
                if ($regs == null) {
                    list($dd, $mm, $yyyy) = explode('/', $nombrevar);
                } else {
                    $dd = $regs[2];
                    $mm = $regs[1];
                    $yyyy = $regs[3];
    ...
    function valida_formularioLocalizado($lang = "es-es") {
        include_once("../../../../aspirantes/usuarioaspirante/localization.php");
        $mensajes = "";
        /* echo "this->array_validacion<pre>";
          print_r($this->array_validacion);
          echo "<pre>"; die; */
        ...
        if (is_array($this->array_validacion)) {
            foreach ($this->array_validacion as $llave => $valor) {
                if ($valor['valido'] == '0') {
                    if ($valor['tipo'] == 'requerido') {
                        $mensajes = $mensajes . $valor['mensaje'] . ', ' . localize('es', $lang) . ' ' . localize($valor['tipo'], $lang) . '\n';
                    } else {
                        $mensajes = $mensajes . $valor['mensaje'] . ', ' . localize('debe ser del tipo', $lang) . ' ' . localize($valor['tipo'], $lang) . '\n';
                    }
                    $this->validaciongeneral = false;
                }
            }
    ...
    function trace($var, $file, $line, $exit = false) {
        ...
        $id = md5(microtime());
        ?>
        <div class="sa_trace_start"><a href="javascript:;" class="sa_trace_start_link" onClick="MM_changeProp('var_<?= $id ?>', '', 'style.visibility', MM_toggleVisibility('var_<?= $id ?>'), 'DIV');
                            " title="click here to view the output of var_dump(<?= gettype($var) ?>)">:: Traza <span style="color: #ff6600"><?= $file ?></span> on line <?= $line ?> ::</a></div>
