 ...
 */ 
 ...
 class Carrera{
	 ...
	 */
        /**
         *Mododfied DIego RIvera <riveradiego@unbosque.edu.co>
         *Se añade codigo de modalidad 600 (educacion a distanca)
         *Since May 31,2018
         */
	public function consultarCarrera( $idFacultad ){
		...
		$sql = "SELECT C.codigocarrera, C.nombrecarrera
				...
				AND C.codigomodalidadacademica IN (200,300,600,800)
	...
	public function consultarCarreraNotificar( ){
		...
		$sql = "SELECT DISTINCT C.codigocarrera , F.codigofacultad, C.codigocentrobeneficio
				FROM carrera C
				INNER JOIN facultad F ON ( F.codigofacultad = C.codigofacultad )
				WHERE C.codigomodalidadacademica IN (200,300,800) 
	...
	public function buscarTituloProfesion( ){
		/**
                ...
                * se añade campo  nombreTituloGenero en consulta sql para deteminar el nombre del titulo a obtener dependiendo el genero del estudiante
                *@since January 15,2019
                */	
		$sql = "SELECT
                            T.codigotitulo, T.nombretitulo , T.nombreTituloGenero
                        FROM
                            titulo T
		...
		if( $this->persistencia->getNext( ) ){
			...
			$tituloProfesion->setNombreTituloGenero( $this->persistencia->getParametro( "nombreTituloGenero" ) );
	...
	public function consultarTodaCarrera( $modalidad , $facultad ){
		$carreras = array( );
		$sql = "SELECT 
						C.codigocarrera,
						C.nombrecarrera
				FROM 
						carrera C
				INNER JOIN 
						facultad F ON ( C.codigofacultad = F.codigofacultad )
				WHERE 
						F.codigofacultad = ?
						AND C.codigomodalidadacademica = ?
						AND C.fechavencimientocarrera >= YEAR(NOW())
				ORDER BY 
						C.codigomodalidadacademica, C.nombrecarrera ASC";
				
		$this->persistencia->crearSentenciaSQL( $sql );
		$this->persistencia->setParametro( 0 , $facultad , false );
		$this->persistencia->setParametro( 1 , $modalidad , false );
		$this->persistencia->ejecutarConsulta( );
	...
	public function carreraSnies( $carrera ){


		$sql = "SELECT
					cr.codigosniescarreraregistro
				FROM
					carreraregistro cr
		...
		$this->persistencia->crearSentenciaSQL( $sql );
		$this->persistencia->setParametro( 0 , $carrera , false );
		//echo $this->persistencia->getSQLListo( );
		$this->persistencia->ejecutarConsulta( );
		if( $this->persistencia->getNext( ) ){
			$this->setCodigoCarrera( $this->persistencia->getParametro( "codigosniescarreraregistro" ) );
		}
