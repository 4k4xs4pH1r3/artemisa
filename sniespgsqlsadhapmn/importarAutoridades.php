...
$data->read("archivos/autoridades.xls");
...
{
	$fila2['ies_code']=1729;
	$fila2['cargo']=$valor[12];//$valor[6];
	$fila2['tipo_identificacion']=$valor[7];
	$fila2['num_documento']=$valor[1];
	$fila2['email']=$valor[9];
	$fila2['direccion']='Cra 7B Bis No. 132-11';
	$fila2['fax']=$valor[8];
	$fila2['tel_extension']=$valor[10];
	$fila2['tel_cod_pais']=57;
	$fila2['tel_cod_area']=1;
		
	echo "<pre>";
	print_r($fila2);
	echo "</pre>";
	if($_GET['insertar']=='si'){
		insertar_fila_bd($snies_conexion,'AUTORIDAD',$fila2);
	}
	unset($fila);
}
