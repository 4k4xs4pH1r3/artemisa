...
if(!empty($_POST['annio']) and !empty($_POST['periodo'])){
	...
	if($_POST['accion']=='2'){
		...
		foreach ($array_codigoestudiante_estudiante as $llave => $valor)
		{
			...
			//$annio=$datosestudiante["codigoperiodo"][0].$datosestudiante["codigoperiodo"][1].
			//$datosestudiante["codigoperiodo"][2].$datosestudiante["codigoperiodo"][3]
			$periodo="0".$datosestudiante["codigoperiodo"][4];
			...
			$fila['anio']=$datosestudiante["codigoperiodo"][0].$datosestudiante["codigoperiodo"][1].
			$datosestudiante["codigoperiodo"][2].$datosestudiante["codigoperiodo"][3];
			$fila['semestre']="0".$datosestudiante["codigoperiodo"][4];

			if($datosestudiante["codigotipoestudiante"]=='11'){
			$fila['es_transferencia']="SI";			
			}
			else{
			$fila['es_transferencia']="NO";			
			}
			...
			{
				echo "<h1>falta carrera registro, se asigna por defecto 111111, esto se debe revisar en la actualizacion de la plataforma<br>codigoestudiante : ".$valor['codigoestudiante']."<br>codigocarrera: ".$valor['codigocarrera']."</h1>";
			...
			if(($fila['anio']==$_POST['annio'])&&($fila['semestre']==$_POST['periodo'])){
			$snies->insertar_fila_bd($snies_conexion,'estudiante_programa',$fila);
			}
			unset($fila);
			unset($participante);
		}
		...
		echo $query="SELECT count(*) as cantidad FROM estudiante_programa";
