...
if(!empty($_POST['annio']) and !empty($_POST['periodo'])){
	$query="
	select programa.*,egresado.* from egresado 
	inner join programa on programa.pro_consecutivo=egresado.pro_consecutivo
...
$motor = new matriz($arrayInterno,'Egresados SNIES','listadoEgresados.php','si','si','','',false,'si','../../../');
$motor->jsVarios();
$motor->mostrar();
?>