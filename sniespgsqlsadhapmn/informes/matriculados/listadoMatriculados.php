...
if(!empty($_POST['annio']) and !empty($_POST['periodo'])){
	...
	if($_POST['tipo']=='02'){
		$query="
	select
	programa.*,matriculado.* from matriculado
	inner join programa on programa.pro_consecutivo=matriculado.pro_consecutivo
	inner join participante on matriculado.codigo_unico=participante.codigo_unico
	where est_annio='".$_POST['annio']."' and est_semestre='".$_POST['periodo']."'";
	}
	else{
		$query="
		select
		count(matriculado.*) as count, programa.prog_nombre from matriculado
		inner join programa on programa.pro_consecutivo=matriculado.pro_consecutivo
		inner join participante on matriculado.codigo_unico=participante.codigo_unico
		where est_annio='".$_POST['annio']."' and est_semestre='".$_POST['periodo']."'
...
$motor = new matriz($arrayInterno,'Matriculados SNIES','listadoMatriculados.php','si','si','','',false,'si','../../../');
$motor->jsVarios();
$motor->mostrar();
?>