...
require_once 'Google/Service/Groupssettings.php';
    ...
    $emailList="pruebatecnologia@unbosque.edu.co";
    $password="T3cn0C0rr30";
    $apellido="Prueba";
    $nombres="Creaco";
    
    
    $body2 = new Google_Service_Directory_UserName();
    $body2->setFamilyName($apellido);
    $body2->setGivenName($nombres);
    $body = new Google_Service_Directory_User();
    $body->setName($body2);
    $body->setPassword($password);
    $body->setPrimaryEmail($emailList);
   ...
   /* $condicion = " codigomodalidadacademica in ('200','300')
	...
	if($client->getAuth()->isAccessTokenExpired()) {
	  ...
	  $_SESSION['service_token'] = $client->getAccessToken();
        ...
        $emailList = "estudiantes" . $nombrecarrera;        
	...
	sleep(10);
        try {
            $listausuario = $objusuario->listaUsuarioEstudianteMatriculado($codigocarrera);
		  ...
		  $listaemailpregrado[$ipregrado]["Correo_Institucional"] = $emailList;
		  $ipregrado++;
	      }
	      if ($datomodalidad["codigomodalidadacademica"] == "300") {
		  $listaemailpostgrado[$ipostgrado]["Correo_Institucional"] = $emailList;
		  $ipostgrado++;
	      }
	      unset($fila);	      
	      $tabla = "carreraemail";
   ...
   /* $listaestudiantespostgrado = "estudiantespostgrados";
    $listaestudiantespregrado = "estudiantespregrado";
    
    try {
	  eliminaLista($service, $listaestudiantespregrado);
          echo "<br><h5>Elimino la lista= " . $listaestudiantespregrado . "</h5>";
    } catch (Google_Service_Exception $miExcepcion) {
        
        echo "<br><h5>No exite el grupo y por tanto no se puede eliminar: ". $listaestudiantespregrado ."</h5>";
        
    } catch(Exception $e){
	  //lo demas no importa
    }
     
    creaLista($service, $listaestudiantespregrado);        
    
    agregarCorreosLista($service, $service2, $listaestudiantespregrado, $listaemailpregrado);  
    
    try {
	  eliminaLista($service, $listaestudiantespostgrado);
          echo "<br><h5>Elimino la lista= " . $listaestudiantespostgrado . "</h5>";
        } catch (Google_Service_Exception $miExcepcion) {
        
        echo "<br><h5>No exite el grupo y por tanto no se puede eliminar: ". $listaestudiantespostgrado ."</h5>";
        
    } catch(Exception $e){
	  //lo demas no importa
    }
     
    creaLista($service, $listaestudiantespostgrado);        
    
    agregarCorreosLista($service, $service2, $listaestudiantespostgrado, $listaemailpostgrado);     

        
    $listaestudiantes[0]["Correo_Institucional"] = $listaestudiantespregrado;
    $listaestudiantes[1]["Correo_Institucional"] = $listaestudiantespostgrado;
    $estudiante="estudiantes";
    
    try {
	  eliminaLista($service, $estudiante);
          echo "<br><h5>Elimino la lista= " . $estudiante . "</h5>";
    } catch (Google_Service_Exception $miExcepcion) {
        
        echo "<br><h5>No exite el grupo y por tanto no se puede eliminar: ". $estudiante ."</h5>";
        
    } catch(Exception $e){
	  //lo demas no importa
    }
     
    creaLista($service, $estudiante);        
    
    agregarCorreosLista($service, $service2, $estudiante, $listaestudiantes);      
