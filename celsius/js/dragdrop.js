...
const spacer = 'transparentpixel.gif'

// window.onerror = new Function('return true;');

// Optional commands passed to SET_DHTML() on the html-page (g: may be applied globally, i: individually)
const CLONE = 'C10nE' // i  img	  clone image
const COPY = 'C0pY' // i  img	  create copies
const DETACH_CHILDREN = 'd37aCH' // i  lyr	  detach images
const HORIZONTAL = 'H0r1Z' // i  img,lyr  horizontally draggable only
const MAXHEIGHT = 'm7x8I' // i  img,lyr  maximum height limit, "
const MAXOFFBOTTOM = 'm7xd0wN' // i  img,lyr  downward offset limit
const MAXOFFLEFT = 'm7x23Ft' // i  img,lyr  leftward offset limit
const MAXOFFRIGHT = 'm7x0Ff8' // i  img,lyr  rightward offset limit
const MAXOFFTOP = 'm7xu9' // i  img,lyr  upward offset limit
const MAXWIDTH = 'm7xW1' // i  img,lyr  maximum width limit, use with resizable or scalable
const MINWIDTH = 'm1nw1' // i  img,lyr  minimum width limit, "
const MINHEIGHT = 'm1n8I' // i  img,lyr  minimum height limit, "
const NO_ALT = 'no81T' // gi img	  disable alt and title attributes
const NO_DRAG = 'N0d4Ag' // i  img,lyr  disable draggability
const RESET_Z = 'r35E7z' // gi img,lyr  reset z-index when dropped
const RESIZABLE = 'r5IZbl' // gi img,lyr  resizable if <ctrl> or <shift> pressed
const SCALABLE = 'SCLbl' // gi img,lyr  scalable		   "
const SCROLL = 'sC8lL' // gi img,lyr  enable auto scroll functionality
const TRANSPARENT = 'dIApHAn' // gi img,lyr  translucent while dragged
const VERTICAL = 'V3Rt1C' // i  img,lyr  vertically draggable only
const dd_cursors = new Array(
  'c:default',
  'c:crosshair',
  'c:e-resize',
  'c:hand',
...
for (let dd_i = dd_cursors.length; dd_i;) {
  --dd_i
  eval(
    'var CURSOR_' +
...
dd.getDiv = function (d_x, d_d) {
  ...
  if (dd.n4) {
    ...
    for (let d_i = d_d.layers.length; d_i;) {
      const d_y = dd.getDiv(d_x, d_d.layers[--d_i].document)
      if (d_y) return d_y
    }
  }
...
dd.getImg = function (d_o, d_nm, d_xy, d_w) {
  ...
  if (dd.n4) {
    for (let d_i = d_w.document.layers.length; d_i;) {
      const d_y = dd.getImg(d_o, d_nm, d_xy, d_w.document.layers[--d_i])
      if (d_y) return d_y
    }
  }
  return null
...
dd.getParent = function (d_o, d_p) {
  if (dd.n4) {
    for (let d_i = dd.elements.length; d_i;) {
      if (
        !(d_p = dd.elements[--d_i]).is_image &&
        d_p.div &&
        (d_p.div.document.layers[d_o.name] ||
          (d_o.oimg && d_p.div.document.images[d_o.oimg.name]))
      ) {
...
dd.getCmd = function (d_o, d_cmd, d_cmdStr) {
  const d_i = d_o.id.indexOf(d_cmd)
  let d_j
  const d_y = (d_i >= 0) * 1
  if (d_y) {
...
dd.addProps = function (d_o) {
  if (d_o.is_image) {
    ...
    d_o.nimg =
      dd.n4 && d_o.div
        ? d_o.div.document.images[0]
        : document.images[d_o.id + 'NI1m6G'] || null
    ...
    d_o.bgColor = dd
      .getCssProp(d_o.div, 'background-color', 'backgroundColor', 'bgColor')
      .toLowerCase()
    if (dd.n6 && d_o.div) {
      let d_c = d_o.bgColor.toLowerCase()
      if (d_c.indexOf('rgb') >= 0) {
        d_c = d_c.substring(4, d_c.length - 1).split(',')
        d_o.bgColor = '#'
        for (let d_l = d_c.length, d_i = 0; d_i < d_l; d_i++) {
...
dd.initz = function () {
  ...
  const d_drag = document.onmousemove == DRAG
  const d_resize = document.onmousemove == RESIZE
...
dd.finlz = function () {
  if (dd.ie && dd.elements) {
    for (let d_i = dd.elements.length; d_i;) dd.elements[--d_i].del()
  }
...
dd.recalc = function (d_x) {
  for (var d_o, d_i = dd.elements.length; d_i;) {
    if (!(d_o = dd.elements[--d_i]).is_image && d_o.div) {
      ...
      if (d_o.div.pos_rel) {
        dd.getPageXY(d_o.div)
        const d_dx = dd.x - d_o.x
        const d_dy = dd.y - d_o.y
        d_o.defx += d_dx
        d_o.x += d_dx
        d_o.defy += d_dy
        d_o.y += d_dy
        for (var d_p, d_j = d_o.children.length; d_j;) {
          if (
            !(d_p = d_o.children[--d_j]).detached &&
            (d_o != d_p.defparent ||
              !(d_p.is_image && dd.getImg(d_p, d_p.oimg.name, 1)))
          ) {
            d_p.defx += d_dx
            d_p.defy += d_dy
      ...
      if (dd.n6 && d_x && !d_o.defw) {
        d_o.resizeTo(
          (d_o.defw = dd.getImgW(d_o.oimg)),
          (d_o.defh = dd.getImgH(d_o.oimg))
        )
      }
      const d_defx = d_o.defx
      const d_defy = d_o.defy
      if (
        !(d_o.parent && d_o.parent != d_o.defparent) &&
...
function DDObj (d_o, d_i) {
  ...
  if (this.oimg) {
    ...
    this.dw = this.dh = 0
    this.defz =
      dd.Int(dd.getCssProp(this.oimg, 'z-index', 'zIndex', 'zIndex')) || 1
...
DDObj.prototype.moveBy = function (d_x, d_y, d_kds, d_o) {
  ...
  for (let d_i = this.children.length; d_i;) {
...
DDObj.prototype.hide = function (d_m, d_o, d_p) {
  if (this.div && this.visible) {
    ...
    if (d_m && !dd.n4) {
      ...
      if (this.oimg) {
        this.oimg.display = dd.getCssProp(
          this.oimg,
          'display',
          'display',
          'display'
...
DDObj.prototype.resizeTo = function (d_w, d_h, d_o) {
  ...
  if (dd.n4) {
    ...
    if (this.is_image) {
      this.write(
        '<img src="' + this.src + '" width="' + d_w + '" height="' + d_h + '">'
      );
...
DDObj.prototype.swapImage = function (d_x, d_cp) {
  ...
  if (d_cp) {
    for (let d_i = this.copies.length; d_i;) {
      --d_i
...
DDObj.prototype.copy = function (d_n, d_p, d_b) {
  if (!this.oimg) return
  d_b =
    dd.ie && document.all.tags('body')
      ? document.all.tags('body')[0]
      : document.getElementsByTagName
        ? document.getElementsByTagName('body')[0] || dd.db
        : dd.db
  for (d_n = d_n || 1; d_n > 0; --d_n) {
    const d_l = this.copies.length
    const d_o = new DDObj(this.name + this.cmd, d_l + 1)
    if (dd.n4) {
...
DDObj.prototype.addChild = function (d_kd, detach, defp) {
  ...
  if (
    (d_kd.parent && d_kd.parent == this) ||
    d_kd == this ||
    (!d_kd.is_image && d_kd.defparent && !defp)
  ) {
    return
  }
...
DDObj.prototype._removeChild = function (d_kd, d_newp) {
  ...
  const d_oc = this.children
  const d_nc = new Array()
  for (let d_i = 0; d_i < d_oc.length; d_i++) {
...
DDObj.prototype.setZ = function (d_x, d_kds, d_o) {
  if (d_kds) {
    for (let d_dz = d_x - this.z, d_i = this.children.length; d_i;) {
      if (!(d_o = this.children[--d_i]).detached) d_o.setZ(d_o.z + d_dz, 1)
    }
  }
  dd.z = Math.max(
    dd.z,
    (this.z = this.div ? ((this.css || this.div).zIndex = d_x) : 0)
  )
}
...
DDObj.prototype._free = function () {
  for (const d_i in this) this[d_i] = null
...
dd.embedVis = function (d_vis) {
  const d_o = new Array('iframe', 'applet', 'embed', 'object')
  for (let d_i = d_o.length; d_i;) {
    --d_i
    const d_p = dd.ie
      ...
      : document.getElementsByTagName
        ...
        : null
    if (d_p) {
      for (let d_j = d_p.length; d_j;) {
        let d_q = d_p[--d_j]
        while (d_q.offsetParent || d_q.parentNode) {
          if (
            (d_q = d_q.parentNode || d_q.offsetParent || null) == dd.obj.div
          ) {
...
dd.maxOffX = function (d_x, d_y) {
  return (dd.obj.maxoffl + 1 &&
    (d_y = dd.obj.defx - dd.obj.maxoffl) - d_x > 0) ||
    (dd.obj.maxoffr + 1 && (d_y = dd.obj.defx + dd.obj.maxoffr) - d_x < 0)
    ? d_y
    : d_x
}
dd.maxOffY = function (d_x, d_y) {
  return (dd.obj.maxofft + 1 &&
    (d_y = dd.obj.defy - dd.obj.maxofft) - d_x > 0) ||
    (dd.obj.maxoffb + 1 && (d_y = dd.obj.defy + dd.obj.maxoffb) - d_x < 0)
    ? d_y
    : d_x
}
dd.inWndW = function (d_x, d_y) {
  const d_wx = dd.getScrollX()
  const d_ww = dd.getWndW()
  return (d_y = d_wx + 2) - d_x > 0 ||
    (d_y = d_wx + d_ww + dd.obj.w - 2) - d_x < 0
    ? d_y
    : d_x
}
dd.inWndH = function (d_x, d_y) {
  const d_wy = dd.getScrollY()
  const d_wh = dd.getWndH()
  return (d_y = d_wy + 2) - d_x > 0 ||
    (d_y = d_wy + d_wh + dd.obj.h - 2) - d_x < 0
    ? d_y
    : d_x
}
...
function DDScroll () {
  ...
  if (dd.msmoved) {
    ...
    let d_y
    dd.scrx =
      (d_y = dd.e.x - d_ww - d_wx + d_bnd) > 0
        ? (d_y >>= 2) * d_y
        : (d_y = d_wx + d_bnd - dd.e.x) > 0
            ? -(d_y >>= 2) * d_y
            : 0
    dd.scry =
      (d_y = dd.e.y - d_wh - d_wy + d_bnd) > 0
        ? (d_y >>= 2) * d_y
  ...
  if (dd.scrx || dd.scry) {
    ...
    dd.obj.is_dragged
      ? dd.obj.moveTo(
          dd.obj.x + dd.getScrollX() - d_wx,
          dd.obj.y + dd.getScrollY() - d_wy
        )
      : dd.reszTo(
        dd.obj.w + dd.getScrollX() - d_wx,
        dd.obj.h + dd.getScrollY() - d_wy
      )
  }
  dd.msmoved = 0
...
function PICK (d_ev) {
  ...
  if (
    dd.e.x >= dd.getWndW() + dd.getScrollX() ||
    dd.e.y >= dd.getWndH() + dd.getScrollY()
  ) {
    return true
  } // on scrollbar
  ...
  if (dd.obj) {
    if (dd.obj.nodrag) dd.obj = null
      ...
      const d_rsz = dd.e.modifKey && (dd.obj.resizable || dd.obj.scalable)
      if (dd.op) {
...
function DRAG (d_ev) {
  ...
  dd.obj.moveTo(
    dd.obj.vertical
      ? dd.obj.x
      : dd.maxOffX(dd.inWndW(dd.ofx + dd.e.x) - dd.obj.w),
    dd.obj.horizontal
      ? dd.obj.y
      : dd.maxOffY(dd.inWndH(dd.ofy + dd.e.y) - dd.obj.h)
  )
...
function RESIZE (d_ev) {
  ...
  dd.msmoved = 1
  let d_w = dd.limW(dd.inWndW(dd.ofx + dd.e.x) - dd.obj.x)
  let d_h
  if (!dd.whratio) d_h = dd.limH(dd.inWndH(dd.ofy + dd.e.y) - dd.obj.y)
  else {
    d_h = dd.limH(
      dd.inWndH(Math.round(d_w / dd.whratio) + dd.obj.y) - dd.obj.y
    )
  ...
  while (d_i) {
    if (!(d_ai = d_a[--d_i]).indexOf('c:')) dd.cursor = d_ai.substring(2)
      ...
      if (d_o.oimg && d_o.cpy_n) {
        for (let d_l = d_o.cpy_n, d_j = 0; d_j < d_l;) {
          const d_p = new DDObj(d_o.name + d_o.cmd, ++d_j)
          dd.addElt(d_p, d_o)
...
const RESET_ZINDEX = RESET_Z // < 3.44
const KEYDOWN_RESIZE = RESIZABLE // < 4.43
const CURSOR_POINTER = CURSOR_HAND // < 4.44
const NO_SCROLL = '' // < v. 4.49
