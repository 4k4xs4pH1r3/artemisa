...
function Determinar_Estado($Evento)
{
  ...
  switch ($Evento)
  {
    ...
    case 13: //Evento de autorizado para bajarse el pdf
      return 11; //pasa a estado PDF enviado
      break;  
   }   
}
...
function Devolver_Opciones ($Estado,$Idioma)
{
  ...
  switch ($Estado)
  {
    ...
    case 2: // Ya esta en busqueda, puede ser solicitado o confirmado
      return array($Idioma["Evento2"]=>3,$Idioma["Evento3"]=>4,$Idioma["Evento8"]=>9,$Idioma["Evento7"]=>8,$Idioma["Evento9"]=>10); //,$Idioma["Evento12"]=>12//,$Idioma["Evento11"]=>11
    case 3: // Ya fue solicitado, puede ser recibido, entregado, redirigido o vuelto a poner en búsqueda o confirmar
      return array($Idioma["Evento5"]=>6,$Idioma["Evento10"]=>3,$Idioma["Evento8"]=>9,$Idioma["Evento7"]=>8,$Idioma["Evento9"]=>10,$Idioma["Evento1"]=>2,$Idioma["Evento3"]=>4);//,$Idioma["Evento12"]=>12);//,$Idioma["Evento11"]=>11
    ...
    case 5: // Ya fue confirmado
      return array($Idioma["Evento2"]=>3,$Idioma["Evento8"]=>9,$Idioma["Evento7"]=>8,$Idioma["Evento9"]=>10,$Idioma["Evento3"]=>4,$Idioma["Evento1"]=>2);//,$Idioma["Evento11"]=>11
...
function Devolver_Todos_Eventos_Mails ($Idioma)
{
  ...
  // que no se mezclen
      ...
      return array($Idioma["Evento12"]=>Devuelve_Evento_PDFEnviado(), $Idioma["Eventos_Mail_1"]=>100,$Idioma["Eventos_Mail_2"]=>101,$Idioma["Eventos_Mail_3"]=>102);
}
...
function devolverBusqueda($id,$Mensajes)
{
 
 $instruccion2="select count(*)as cantidad from Busquedas where Id_Pedido='".$id."'";
//  echo $instruccion2;
  $result2=mysql_query($instruccion2);
  $row2=mysql_fetch_array($result2);

$instruccion3="SELECT Resultado, count( Resultado ) AS Cant FROM Busquedas WHERE Id_Pedido = '".$id."' GROUP BY Resultado";
$result3=mysql_query($instruccion3);
 
 $bus='<tr bgcolor="#C0C0C0"><td width="80%" colspan="2"><div align="center" class="style22"><b>'.$Mensajes["bb-01"].':&nbsp;&nbsp;'.$row2["cantidad"].'</b></div></td></td></tr>';
 while ($row3=mysql_fetch_array($result3))
	 {

    switch ($row3["Resultado"])
		 {
          case 1:{
              $bus.='<tr bgcolor="#0099CC"><td width="80%" colspan="2"><div align="center" class="style22">'.$Mensajes["bb-02"].':&nbsp;&nbsp;'.$row3["Cant"].'</div></td></td></tr>';
				 
				 break;
                  }
          case 2:{
              $bus.=' <tr bgcolor="#0099CC"><td width="80%" colspan="2"><div align="center" class="style22">'.$Mensajes["bb-03"].':&nbsp;&nbsp;'.$row3["Cant"].'</div></td></td></tr>';
				 break;
                  }
	      case 3:{
               $bus.='<tr bgcolor="#0099CC"><td width="80%" colspan="2"><div align="center" class="style22">'.$Mensajes["bb-04"].':&nbsp;&nbsp;'.$row3["Cant"].'</div></td></td></tr>';
				 break;
                  }
...
function mostrarIntersecion($row)
{
  if ($row[16]!='')
 ...
 //  $query1 .= "WHERE (Titulo_Revista LIKE '%".$expresion."%'  or Titulo_Libro LIKE '%".$expresion."%' or Titulo_Articulo LIKE '%".$expresion."%' or  TituloTesis  LIKE '%".$expresion."%'  or TituloCongreso LIKE '%".$expresion."%' or PonenciaActa like '%".$expresion."%')";
     $query1 .= "WHERE (Titulo_Articulo LIKE '%".$row[16]."%')";
   $query1 .= " AND Codigo_Usuario=".$row[50];
...
function armar_expresion_busqueda()
{
   ...
   $expresion .= ",Pedidos.Capitulo_Libro,Unidades.Nombre "; // 52,53 agregador por Gonzalo
   ...
   $expresion.=" LEFT JOIN Unidades AS Unidades ON Clientes.Codigo_Unidad=Unidades.Id ";
   return $expresion;
}
           ...
           switch ($autorizado)
			{
				...
				case -1: //el archivo está disponible, pero el usuario posee problemas (ej. cta. cte. en rojo)
				   $renglon.= " <img alt='Archivo no disponible para bajar' border=0 src='../images/pdf.gif' width='20'> ";
				  break;
			}  //switch
		}
		else
...
function devolverFormaDePago($id_usuario)
{
 $inst="select Codigo_FormaPago from Usuarios where id='".$id_usuario."'";
 //echo $inst;
	          ...
	          {	   		if ($dedonde!=1) 
	          		{
	          			if ($sinhiper==1)
	          			{
	          			  $renglon ="<span class='style29'><strong>".$Mens['tf-1'].":</strong></span> <b>".$row[14]."</b>";	
	          			}
	          			else
	          			{ 
	          		      $renglon ="<span class='style29'><strong>".$Mens['tf-1'] .":</strong></span> <b><a href=\"conshallados.php?Id_Col=$row[45]&Vol=$row[17]&Numero=$row[18]&Anio=$row[19]&Id=$row[1]\"><font color='#000000'>".$row[14]."</font></a></b>";
						}	
	          		
					}
                 else {$renglon ="<span class='style29'><strong>".$Mens['tf-1'] .": </strong></span><b><a href=\"conshallados.php?Id_Col=$row[46]&Vol=$row[17]&Numero=$row[18]&Anio=$row[19]&Id=$row[1]\"><font color='#000000'>".$row[14]."</font></a></b>";}  
           	}
	 			else
				{
					if ($dedonde!=1) {$renglon ="<span class='style29'><strong>".$Mens['tf-1'].":</strong></span> <font color='#800000'>".$row[15]."</font>";}
					else {$renglon ="<span class='style29'><strong>".$Mens['tf-1'].":</strong></span> ".$row[15];}       			
 				}
				$renglon=$renglon." <br> <span class='style29'><strong>".$Mens['tf-2'].":</strong></span> ".$row[19]." &nbsp;&nbsp;&nbsp;<span class='style29'><strong> Vol:</strong></span>".$row[17]." &nbsp;&nbsp;&nbsp; <span class='style29'><strong>".$Mens['tf-3'].":</strong></span> ".$row[18];
				if ($dedonde!=1){ $renglon=$renglon." &nbsp;&nbsp;&nbsp;  <span class='style29'><strong>".$Mens['tf-4'].":</strong></span> ".$row[20]."-".$row[21]."";
				if ($row[16])
					$renglon .= "<br><span class='style29'><strong>Art:</strong></span> ".$row[16];
				}
				else {$renglon=$renglon." &nbsp;&nbsp;&nbsp; <span class='style29'><strong>".$Mens['tf-4'].":</strong></span> ".$row[20]."-".$row[21];}
				$renglon .= '<br><br>';
				break;
				
	  case 2: // Libros
	          $renglon = "<span class='style29'><strong> ".$Mens['tf-5'].":</strong></span> <b> ".$row[5]." </b>   <!-- titulo libro-->
			  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			  <br><span class='style29'><strong>".$Mens['tf-6'].":</span></strong> ".$row[6]." <!-- autor libro -->
&nbsp;&nbsp;&nbsp;
			  <span align='left' class='style29'><strong>".$Mens['tf-8'].":</strong></span> ".$row[9]."</span></td></tr>"; //año 
			  $renglon = $renglon."<tr valign='top' class='style33'><td width='750' colspan=3 align='left'>";
			  if ($row[52])
			    $renglon .= "<span class='style29'><strong>Cap:</strong></span> ".$row[52]." ";
			  $renglon .= "&nbsp;</td><td colspan='1' align='right' width=489>";
             if (($row[21]) || ($row[20])) //pag. desde o pag. hasta
			     $renglon .= "<span class='style29'><strong>pag.</strong></span> ".$row[20]."-".$row[21]."";

             if ($row[8]==1)
             { //solicita índice
               $renglon = $renglon."<span class='style29'><strong>".$Mens['tf-9']."</strong></span>"; 
             }
			 
			 $renglon .= "</td> </tr> <tr> <td> &nbsp; </td> </tr>";
             break;
             
	  case 3:// Patentes
             $renglon = "<span class='style29'><strong>".$Mens['tf-10'].":</strong></span> ".$row[10]." /  <span class='style29'><strong>".$Mens['tf-11'].":</strong></span>";
             if ($row[11]==null && strlen($row[12])>0)
             {
               $renglon = $renglon.$row[12];
             }  
             ...
             $renglon = $renglon. " / <span class='style29'><strong>".$Mens['tf-12'].":</strong></span>".$row[13]."<br><br>";
             break;
             
	  case 4:// Tesis
	         $renglon = " <span class='style29'><strong>".$Mens['tf-13'].":</strong></span>".$row[25]." /  <span class='style29'><strong>".$Mens['tf-14'].":</strong>".$row[26]." / ".$Mens['tf-15'].":".$row[27];
	         $renglon = $renglon." /  <span class='style29'><strong>".$Mens['tf-16'].":</strong></span>".$row[28]." /  <span class='style29'><strong>".$Mens['tf-17'].":</strong></span>".$row[29]." /  <span class='style29'><strong>".$Mens['tf-18'].":</strong></span>".$row[30]."<br><br>";
	         break;
	         
	  case 5:// Actas o Proceeding Congresos
	         $renglon = "<span class='style29'><strong>".$Mens['tf-13'].":</strong></span>".$row[31]." / <span class='style29'><strong>".$Mens['tf-19'].":</strong></span>".$row[32]." / <span class='style29'><strong> ".$Mens['tf-16'].":</strong></span> ".$row[33]." / <span class='style29'><strong> ".$Mens['tf-18'].":</strong></span>".$row[34]."<br><br>";
...
function Busqueda_Pedido_Por_Codigo($Codigo , $dedonde , $Id_usuario ,$VectorIdioma , $Mensajes, $Bibliotecario = 0, $esAdmin=0){
	...
	//$caja=1;
	$query = armar_expresion_busqueda();
	$query .=" WHERE Pedidos.Id='".addslashes($Codigo)."'";
	if ($dedonde==1){
			 $query2 .= " AND Codigo_Usuario= ".$Id_usuario;
		}
	$result = mysql_query($query);   
	...
	if (mysql_num_rows($result)==0)
		{
			...
			$query1 .= "WHERE PedAnula.Id='".addslashes($Codigo)."'";
			...
			if (mysql_num_rows($result)==0)
				{
					...
					$query2 .= "WHERE PedHist.Id='".addslashes($Codigo)."'";
