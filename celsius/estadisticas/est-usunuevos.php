...
$Instruccion = "SELECT YEAR(Fecha_Alta) AS p, MONTH(Fecha_Alta) AS m, COUNT(*) ";
$Instruccion .= "FROM usuarios ";
$Instruccion .= "WHERE YEAR(Fecha_Alta)>=" . $Anio . " AND YEAR(Fecha_Alta)<=" . $AnioFinal;
...
// Recorro el resultado limitando hasta un desplazamiento
// por 5 años
...
<body topmargin="0">
<div align="left">
<form method="POST" action="est-usunuevos.php">
        ...
        <td bgcolor="#E4E4E4">
            ...
            <table width="576" border="0" style="margin-bottom: 0; margin-top:0; border-collapse:collapse" bordercolor="#111111" cellpadding="0" cellspacing="0">
              <tr>
                ...
                <table width="100%"  border="0" cellpadding="0" cellspacing="0" class="style43" class="table-form">
                <tr>
                ...
                <tr>
                    ...
                    <tr>
                        <th bgcolor="#CCCCCC" class="style43">
                        ...
                        <th align="right" bgcolor="#CCCCCC" class="style43">
...
$buffer = '';
$Titulo .= $Mensajes["ec-52"];
?>
           <tr valign="top">
             <td colspan="2" align="left" class="style45">
             <div align="left">
             <p><span class="style54"><img src="../images/sa6.gif" width="8" height="10"><? echo $Titulo." - ".$Mensajes["ec-51"]; ?>.</span></p>
             </div>
    <?
...
function retornar_valores($Modalidad) {
	...
	if ($Modalidad == 1) {
		for ($j = 1; $j <= $filas; $j++) {
			...
			$devolucion .= "<param name=sampleValues_" . $indice . " value=" . $cadena . ">";
		}
	} else {
		$cadena = "";
		for ($j = 1; $j <= $filas; $j++) {
		...
		$cadena = substr($cadena, 0, strlen($cadena) - 1);
		$devolucion .= "<param name=sampleValues_0 value=" . $cadena . ">";
	}
...
function retornar_labels($Modalidad) {
	...
	if ($Modalidad == 1) {
		...
		$cadena = substr($cadena, 0, strlen($cadena) - 1);
		$devolucion .= "<param name=seriesLabels value=" . $cadena . ">";
		$cadena = "";
		for ($i = 1; $i <= 12; $i++) {
			$cadena .= TraduccionesUtils :: Traducir_Mes($i, $VectorIdioma) . ",";
		}
		$cadena = substr($cadena, 0, strlen($cadena) - 1);
		$devolucion .= "<param name=sampleLabels value=" . $cadena . ">";
	} else {
		for ($j = 1; $j <= $filas; $j++) {
			$cadena .= $row[$j][0] . ",";
		}
...
?>
    ...
    <tr valign="top">
     <td colspan="2" align="left" class="style45"><p class="style54"><img src="../images/sa6.gif" width="8" height="10"> <? echo $Mensajes["mensaje.usuariosNuevosPorMes"]; ?> </p>
...
<form name="formExport" action="exp_est.php">
  ...
  <input type='hidden' name='mifilename' value='usunuevos.csv'>
