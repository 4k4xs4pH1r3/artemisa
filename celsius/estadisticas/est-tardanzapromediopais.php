...
function Calcular() {
	...
	while ($rowg = mysql_fetch_row($result)) {
		...
		if ($rowg[3] > 0) {
			...
			$cantidades[$filacorrecta][$rowg[0] - $Anio +1] += $rowg[4];
			$sumacantidades[$filacorrecta] += $rowg[4];
...
function Armar_Instruccion($dedonde) {
	...
	$Instruccion = "SELECT YEAR(Fecha_Alta_Pedido) AS p, paises.Nombre, AVG(Tardanza_Recepcion), paises.Id , COUNT(*)";
	if ($dedonde == 1) {
		$Instruccion .= "FROM pedhist ";
		$Instruccion .= "LEFT JOIN paises ON pedhist.Ultimo_Pais_Solicitado=paises.Id ";
	} else {
		$Instruccion .= "FROM pedidos ";
		$Instruccion .= "LEFT JOIN paises ON pedidos.Ultimo_Pais_Solicitado=paises.Id ";
	}
	...
	$Instruccion .= " GROUP BY p,paises.Id";
	$Instruccion .= " ORDER BY paises.Nombre";
...
<form method="POST" action="est-tardanzapromediopais.php">
<div align="left">
      ...
      <hr color="#E4E4E4" size="1">      <div align="center"><center>
        <table width="760" border="0" bgcolor="#E4E4E4" style="border-collapse: collapse" bordercolor="#111111" cellpadding="0" cellspacing="5">
      	<tr bgcolor="#EFEFEF">
            ...
            <div align="center">
              <center>
            <table width="576" border="0" style="margin-bottom: 0; margin-top:0; border-collapse:collapse" bordercolor="#111111" cellpadding="0" cellspacing="0" class="table-form">
              <tr>
                <td valign="top" bgcolor="#E4E4E4"><table width="100%"  border="0" cellpadding="0" cellspacing="0" class="style43">
                  ...
                  <tr>
                   ...
                   <tr>
                        ...
                        <td width="179" align="left">
                        	<select name="Modalidad" class="style54" size="1" style="width:190px">
                   ...
                   <tr>
                        ...
                        <td width="179" align="left">
                        	<select name="Caja" class="style54" size="1" style="width:190px">
...
// Obtengo la tabla con todos los países 
...
$buffer = '';
$Titulo .= " - " . $Mensajes["tf-21"];
echo $Titulo . " - " . $Mensajes["tf-18"];
?>
  ...
  <tr bordercolor="#CCCCCC" bgcolor="#006699" class="style28">
      <td align="right"><span class="style66"><? echo $Mensajes["ec-53"]; $buffer .=$Mensajes["ec-53"]; ?></span></td>
      <?
...
?>
        ...
        <td align="center" class="style28"><? echo $Mensajes["tf-18"]; $buffer .= ", ".$Mensajes["tf-18"];; ?></td>
...
for ($i = 1; $i <= $tope; $i++) {
	//    echo $SumaPaises[$i];
	if ($SumaPaises[$i] >= 0 && $sumacantidades[$i] > 0) {
		$buffer .= "\n";
?> 
...
?>
               <td align="center" bgcolor="#ECECEC"><? echo number_format($row[$i][$j],2)." - (".$cantidades[$i][$j].")"; $buffer .= ",".number_format($row[$i][$j],2)." - (".$cantidades[$i][$j].")";?></td>
     	     <?
...
?>
		...
		$numero = $SumaPaises[$i] / $columnastope;
		echo number_format($numero, 2) . " - (" . $sumacantidades[$i] . ")";
		$buffer .= ',' . number_format($numero, 2) . " - (" . $sumacantidades[$i] . ")";
?></td>
...
function retornar_series() {
	...
	$cadena = "";
	for ($j = 1; $j <= $tope; $j++) {
		if ($SumaPaises[$j] >= 0 && $sumacantidades[$j] > 0) {
			$cadena .= $row[$j][0] . ",";
		}
	}
	$cadena = substr($cadena, 0, strlen($cadena) - 1);
