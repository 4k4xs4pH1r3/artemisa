...
<?
  ...
  if (!isset($CantAutor))		$CantAutor=0;
...
.style54 {font-family: verdana; font-size: 11px; color: #000000; }
.style55 {
	font-size: 11px;
	color: #000000;
	font-family: Verdana;
...
//alert(<? echo $adonde;?>);
   ...
   document.getElementById('form1').action="agrega_col.php?CantAutor=<?echo $CantAutor; ?>&Id_Col=0";
   //document.forms[0].action = "agrega_col.php?CantAutor=<?echo $CantAutor; ?>&Id_Col=0";
	<? } else { ?>
document.getElementById('form1').action= "verped_col.php?CantAutor=<?echo $CantAutor; ?>&Id_Col=0";
//     document.forms[0].action = "verped_col.php?CantAutor=<?echo $CantAutor; ?>&Id_Col=0";
	<? } ?>
...
<div align="left">
<a name="arriba">
<form method="POST" name="form1" id="form1" onSubmit="recargo1();">
                      ...
                      <table width="95%"  border="0" align="center" cellpadding="0" cellspacing="0">
			            ...
			            <option value=2 <? if ($tipobusqueda==2) { echo " selected"; }  ?>><? echo $Mensajes["sel-2"]; ?></option>
             			<option value=3 <? if ($tipobusqueda==3) { echo " selected"; }  ?>><? echo $Mensajes["sel-3"]; ?></option>
                                  ...
                                  <br>
                                  <input name="Submit2" type="submit" class="style43" value="<? echo $Mensajes["bot-2"]; ?>" OnClick="recargo1();"></td>
                              </tr>
                            </table>                            </td>
                          <td valign="top">
                          <div align="center">
                            ...
                            <div align="center">                                
                              <p>
                                <!--<input name="Submit" type="submit" class="style43" value="Volver sin seleccionar" OnClick="retornaSin();">-->
                                 ...
                                 <? if ($adonde==1)
                                { ?>
                                <a href="agrega_col.php?Alias_Id=<?echo $Alias_Id;?>&CantAutor=<?echo $CantAutor; ?>&Id_Col=0">
                               Volver Sin Seleccionar
								</a>
                       	<? } else { ?>
						   <a href="verped_col.php?dedonde=<? echo $dedonde;?>&Id=<? echo $Id; ?>&Tabla=<? echo $Tabla; ?>&CantAutor=<?echo $CantAutor; ?>&Id_Col=0">
                               Volver Sin Seleccionar
							</a>
                    	<? } ?>
...
*/	
   ...
   if ($pg > 1)
					{
						$url = $pg - 1;
						echo "<td class='style43'><a href='lookup_col.php?adonde=$adonde&Id=$Id&Tabla=$Tabla&Alias_Id=$Alias_Id&Id_Col".$Id_Col."&dedonde=".$dedonde."&Letra=".$Letra."&pg=".$url."'> &lt;&lt;&nbsp;</a></td>";
					}
   for ($i = $desde; $i<=$hasta ; $i++) {
					if ($i == $pg)  
						...
						else 
						{
							...
							echo "<td class='style43' ><a  href='lookup_col.php?adonde=$adonde&Id=$Id&Tabla=$Tabla&Alias_Id=$Alias_Id&Id_Col".$Id_Col."&dedonde=".$dedonde."&Letra=".$Letra."&pg=".$i."'>".$i."</a>&nbsp;</td>";
...
?>			
					  ...
					  <? if ($adonde==1)
                         { ?>
                            <a href="agrega_col.php?CantAutor=<? echo $CantAutor; ?>&Id_Col=<? echo $row[3]; ?>&Alias_Id=<? echo $Alias_Id; ?>&Alias_Comunidad=<? echo $Alias_Comunidad; ?>&Bandeja=<? echo $Bandeja; ?>&Instit_Alias=<? echo $Instit_Alias; ?>&Tabla=<? echo $Tabla; ?>"><?echo $row[0] ; ?></a>
                          <? } else {
                              ?>
                            <a href="verped_col.php?&Alias_Id=<? echo $Alias_Id; ?>&CantAutor=<? echo $CantAutor; ?>&Id_Col=<? echo $row[3]; ?>&Id=<? echo $Id; ?>&dedonde=<? echo $dedonde; ?>&adonde=<? echo $adonde; ?>&Titulo_Recien_Llegado=<? echo $row[0]; ?>&Tabla=<? echo $Tabla; ?>"><?echo $row[0] ; ?></a>
                           <? } ?>
                              </td>
                        </tr>
                      <tr><td>&nbsp;</td>
