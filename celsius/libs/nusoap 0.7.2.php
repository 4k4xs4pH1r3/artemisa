...
class XMLSchema extends nusoap_base  {
	...
	*
	* @param array $attrs attributes that must require name and type
	* @see xmlschema
	* @access public
	*/
...
class soap_server extends nusoap_base {
	...
	*/
	function soap_server($wsdl=false){
		...
		if($wsdl){
			...
			if (is_object($wsdl) && (strtolower(get_class($wsdl)) == 'wsdl')) {
				$this->wsdl = $wsdl;
	...
	*/
	function serialize_return() {
		...
		// if fault
		if (isset($this->methodreturn) && (strtolower(get_class($this->methodreturn)) == 'soap_fault')) {
	...
	* @return string The HTTP body, which requires the SOAP payload
	* @access private
	*/
	...
	* @param	string $documentation optional Description to require in WSDL
	* @param	string $encodingStyle optional (usually 'http://schemas.xmlsoap.org/soap/encoding/' for encoded)
	* @access   public
	*/
...
class wsdl extends nusoap_base {
	...
	*/
	...
	function serializeType($name, $type, $value, $use='encoded', $encodingStyle=false, $unqualified=false)
	{
    	...
    	if (is_object($value) && strtolower(get_class($value)) == 'soapval') {
    		if ($value->type_ns) {
	...
	* @param string $documentation optional The description to require in the WSDL
	* @param string $encodingStyle optional (usually 'http://schemas.xmlsoap.org/soap/encoding/' for encoded)
	* @access public 
	*/
...
class nusoap_client extends nusoap_base  {
	...
	*/
	function nusoap_client($endpoint,$wsdl = false,$proxyhost = false,$proxyport = false,$proxyusername = false, $proxypassword = false, $timeout = 0, $response_timeout = 30){
		...
		// make values
		if($wsdl){
			if (is_object($endpoint) && (strtolower(get_class($endpoint)) == 'wsdl')) {
				$this->wsdl = $endpoint;
