...
<a href="local_provision.php"> Recargar página </a> <br>
<?php
  ...
  if (isset($setAll))
  {
 	$updQuery = "UPDATE PedHist Set Tipo_Pedido = 1 WHERE Pedidos.Id like '$local%' 
			     and Pedidos.Tipo_Pedido = 2";
  ...
  elseif (isset($Id_pedido)) //quiere decir que hay algún pedido para actualizar
    {
	$updQuery = "UPDATE PedHist Set Tipo_Pedido = 1 WHERE Id = '".$Id_pedido."'";
	$resu = mysql_query($updQuery);
	echo mysql_error();
	if (mysql_affected_rows() > 0)
  ...
  $query = "SELECT PedHist.Id as Id_Pedido, CONCAT(Usuarios.Apellido,', ',Usuarios.Nombres) 
			...
			Where PedHist.Id like '$local%' 
			     and PedHist.Tipo_Pedido = 2 
			     and PedHist.Codigo_Usuario = Usuarios.Id";
	$result = mysql_query($query);
	    echo mysql_error();
	    
	echo "<center> <h2> Pedidos Históricos locales que son provision </h2> ";	
	echo "<table>";	
	while($row = mysql_fetch_row($result))
	  { echo "<tr height=18> <td> <table bgcolor='#99CCFF'> <tr height=18> ";
	    ...
	    echo "<td bgcolor='#000080' align='center' width='130'> 
	             <form name='updateForm' action='local_provision.php'>
	             <input type='submit' style='background-color:#68C9CE;color:#FFFFCC;size:8pt;font-face:MS Comic Sans' value='Pasar a búsqueda'>
   ...
   echo "</table> <br> <br>";
   echo "<form name='updtateForAll' action='local_provision.php'>
          <input type='submit' style='background-color:#B7CFE1;color:navy;size:8pt;font-weight:bold;font-face:MS Comic Sans;height:15;width:25' value='Pasar todos a búsqueda'>
