...
{
	document.forms.form1.Id1.value=<? if (isset($Id1)){ if ($Id1=="") { $Id1=0; } echo $Id1;}else{ echo 0; } ?>; 
	document.forms.form1.adonde.value=1;
	document.forms.form1.action = "tesauro.php?Letra=A";
	document.forms.form1.submit();
}

function buscolookup2()
{
	document.forms.form1.Id2.value=<? if  (isset($Id2)){ if ($Id2=="" || $Id1==$Id2) { $Id2=0; } echo $Id2;}else { echo 0; } ?>;
...
<?
 ...
 ?> 
							...
							<form method="POST" name="form1" action="union_pub.php?definido=1">  
                            <table width="95%"  border="0" cellpadding="1" cellspacing="1">
                              <tr>
                                <td colspan="2" bgcolor="#006699" class="style45"><img src="../images/square-w.gif" width="8" height="8"><? echo $Mensajes["tf-14"]; ?></td>
                                </tr>
                              <tr>
                                <td width="150" class="style49"><div align="right"><? echo $Mensajes["ec-11"]; ?></div></td>
                              ...
                              <tr>
                                <td width="150" class="style49"><div align="right"><? echo $Mensajes["ec-12"]; ?></div></td>
                                <td class="style43"><div align="left">
...
<? } else {
   ...
   $expresion = "SELECT Nombre FROM Titulos_Colecciones WHERE Id=".$Id2;
   $result = mysql_query($expresion);
   echo mysql_error();
   $rowg = mysql_fetch_row($result);
      
   $expresion = "UPDATE Pedidos SET Codigo_Titulo_Revista=".$Id2.",Titulo_Revista='".AddSlashes($rowg[0])."' WHERE Codigo_Titulo_Revista=".$Id1;
   $result = mysql_query($expresion);
   echo mysql_error();
   $num1 = mysql_affected_rows();
   
   $expresion = "UPDATE PedHist SET Codigo_Titulo_Revista=".$Id2.",Titulo_Revista='".AddSlashes($rowg[0])."' WHERE Codigo_Titulo_Revista=".$Id1;
   $result = mysql_query($expresion);
   echo mysql_error();
   $num2 = mysql_affected_rows();
   
   $expresion = "UPDATE PedAnula SET Codigo_Titulo_Revista=".$Id2.",Titulo_Revista='".AddSlashes($rowg[0])."' WHERE Codigo_Titulo_Revista=".$Id1;
   $result = mysql_query($expresion);
   echo mysql_error();
   $num3 = mysql_affected_rows();

 
   $expresion = "DELETE FROM Titulos_Colecciones WHERE Id=".$Id1;
   $result = mysql_query($expresion);
   echo mysql_error();
   
      //nuevo Log
   global $Id_usuario;
   $expresion = "INSERT INTO Log (idOperador,fecha,tipoEvento,idNuevo) VALUES(".$Id_usuario.",NOW(),4,".$Id2.")";
   $result = mysql_query($expresion);
   echo mysql_error();
 ...
 ?>
					<table width="95%"  border="0" cellpadding="1" cellspacing="1">
                              <tr>
								...
								<tr>
								<td width="50%" class="style49"><div align="right"><? echo $Mensajes["ec-11"]; ?></div></td>
								<td width="50%" class="style43"><div align="left"> <? echo $Nombre1; ?></div></td>
								</tr>
								<tr>
								<td width="50%" class="style49"><div align="right"><? echo $Mensajes["ec-12"]; ?></div></td>
								<td width="50%" class="style43"><div align="left"> <? echo $Nombre2; ?></div></td>
								</tr>
								<tr>
								<td colspan=2 class="style49"><div align="center"><?
								echo "Ped/PedHist/PedAnula:".$num1."/".$num2."/".$num3; ?></div></td>
								</tr>
